...
The default scorer is deterministic and training-free. The B6 scorer uses a
versioned multi-task MLP checkpoint with continuous IoU and P15/P25/P50 heads.
`box_stability` measures Top-K lifted-box center/log-size dispersion, and
training/validation splits are disjoint by `scene_ids`.
...
B4  + identity refit control
B5  + object-local BoxRefiner
B6  + learned multi-view IoU-aware quality score
B7  + track-aware Soft-NMS
```
