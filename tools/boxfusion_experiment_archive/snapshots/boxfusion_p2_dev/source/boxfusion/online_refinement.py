...
def resolve_online_refinement_config(
    ...
    if p_stage not in {"DISABLED", "P0", "P1", "P2"}:
        raise ValueError(
            "p_ablation_stage must be disabled, P0, P1, or P2"
        )
    expected_p_profile = {
        "DISABLED": "disabled",
        "P0": "p0_frozen_b6",
    ...
    if p1_enabled != (p_stage in {"P1", "P2"}):
        raise ValueError(
            "residual_proposal.enabled disagrees with P stage"
        )
    if p2_enabled != (p_stage == "P2"):
    ...
    if p1_enabled and resolved["ablation_profile"] != "quality_only":
        raise ValueError(
            "P1/P2 must be layered on the frozen quality_only B6 profile"
        )
    if not isinstance(
