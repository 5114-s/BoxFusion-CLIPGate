...
DEFAULT_OBJECT_MEMORY_CONFIG = {
    ...
    # YiDu A1: disabled by default so every historical profile remains
    # bit-exact.  Dedicated secondary-memory profiles enable it explicitly.
    "adaptive_mask_erosion": False,
    ...
    # YiDu A2: DFU3D-inspired local/global filtering.  These switches affect
    # only the isolated secondary SAM3 memory in the released ablations.
...
def resolve_object_memory_config(
    ...
    for key in (
        "enabled",
        "adaptive_mask_erosion",
        "radius_filter_enabled",
        "statistical_filter_enabled",
    ):
        if not isinstance(resolved[key], (bool, np.bool_)):
            raise ValueError(f"object_memory.{key} must be a boolean")
    ...
    for key in (
        ...
        "mask_threshold",
        "voxel_size",
        "adaptive_erosion_radius_fraction",
        "adaptive_erosion_depth_edge_weight",
        "radius_filter_radius",
        "statistical_filter_std_ratio",
    ...
    for key, minimum in (
        ("adaptive_erosion_min_margin", 0),
        ("adaptive_erosion_max_margin", 0),
        ("radius_filter_min_neighbors", 1),
        ("statistical_filter_k", 1),
        ("point_filter_max_points", 1),
        ("point_filter_min_points", 1),
    ):
    ...
    if (
        resolved["adaptive_erosion_max_margin"]
        < resolved["adaptive_erosion_min_margin"]
    ):
        raise ValueError(
            "object_memory adaptive erosion maximum margin is below minimum"
        )
    if resolved["adaptive_erosion_radius_fraction"] < 0.0:
        raise ValueError(
            "object_memory.adaptive_erosion_radius_fraction must be "
            "non-negative"
        )
    if resolved["adaptive_erosion_depth_edge_weight"] < 0.0:
        raise ValueError(
            "object_memory.adaptive_erosion_depth_edge_weight must be "
            "non-negative"
        )
    if resolved["radius_filter_radius"] <= 0.0:
        raise ValueError(
            "object_memory.radius_filter_radius must be positive"
        )
    if resolved["statistical_filter_std_ratio"] < 0.0:
        raise ValueError(
            "object_memory.statistical_filter_std_ratio must be non-negative"
        )
    if (
        resolved["point_filter_min_points"]
        > resolved["point_filter_max_points"]
    ):
        raise ValueError(
            "object_memory.point_filter_min_points exceeds "
            "point_filter_max_points"
        )
...
class DepthPointObservation:
    ...
    erosion_margin: int = 0
    radius_retained_count: int = 0
    statistical_retained_count: int = 0
    radius_filter_fallback: bool = False
    statistical_filter_fallback: bool = False
...
def extract_masked_world_points(
    ...
    erosion_margin = int(cfg["mask_edge_margin"])
    if bool(cfg["adaptive_mask_erosion"]):
        erosion_margin = adaptive_erosion_margin(
            resized_mask,
            depth_meters,
            minimum_margin=int(cfg["adaptive_erosion_min_margin"]),
            maximum_margin=int(cfg["adaptive_erosion_max_margin"]),
            radius_fraction=float(
                cfg["adaptive_erosion_radius_fraction"]
            ),
            depth_edge_weight=float(
                cfg["adaptive_erosion_depth_edge_weight"]
            ),
            depth_edge_threshold=float(
                cfg["depth_edge_threshold"]
                if cfg["depth_edge_threshold"] is not None
                else 0.15
            ),
        )
    filtered_mask = erode_mask_edges(resized_mask, erosion_margin)
    ...
    if accepted_depth.shape[0] == 0:
        ...
        return DepthPointObservation(
            ...
            radius_retained_count=0,
            statistical_retained_count=0,
        )
    ...
    cleaning = clean_mask_rgbd_points(
        voxel_points,
        {
            "radius_filter_enabled": bool(
                cfg["radius_filter_enabled"]
            ),
            "radius": float(cfg["radius_filter_radius"]),
            "minimum_neighbors": int(
                cfg["radius_filter_min_neighbors"]
            ),
            "statistical_filter_enabled": bool(
                cfg["statistical_filter_enabled"]
            ),
            "statistical_k": int(cfg["statistical_filter_k"]),
            "statistical_std_ratio": float(
                cfg["statistical_filter_std_ratio"]
            ),
            "maximum_input_points": int(
                cfg["point_filter_max_points"]
            ),
            "minimum_points": int(cfg["point_filter_min_points"]),
        },
    )
    ...
    return DepthPointObservation(
        ...
        erosion_margin=erosion_margin,
        radius_retained_count=int(cleaning.radius_retained_count),
        statistical_retained_count=int(
            cleaning.statistical_retained_count
        ),
        radius_filter_fallback=bool(cleaning.radius_fallback),
        statistical_filter_fallback=bool(
            cleaning.statistical_fallback
        ),
    )
