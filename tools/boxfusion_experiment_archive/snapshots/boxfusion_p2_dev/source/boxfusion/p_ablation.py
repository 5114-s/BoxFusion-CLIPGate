"""Strict P0/P1/P2 profiles for the residual-proposal research route.

P0, P1, and P2 share the exact frozen B6 output path.  P1 adds a read-only
residual RGB-D proposal stream; P2 adds only foreground-occupancy Top-K
selection on the frozen P1 stream.  This helper aggressively disables every
older experimental mutation so stale YAML values cannot turn a P observer
run into an undocumented module combination.
"""
...
P_STAGE_MODULE_MATRIX = {
    "P0": {"residual_proposal": False, "occupancy_topk": False},
    "P1": {"residual_proposal": True, "occupancy_topk": False},
    "P2": {"residual_proposal": True, "occupancy_topk": True},
}
...
def apply_p_ablation(
    ...
    """Return a deep-copied, output-safe P0, P1, or P2 configuration."""
    ...
    if canonical not in P_STAGE_TO_PROFILE:
        raise ValueError("P stage must be P0, P1, or P2")
    ...
    residual["enabled"] = canonical in {"P1", "P2"}
    residual["observer_only"] = True
    residual["mutate"] = False
    residual["collect_diagnostics"] = canonical in {"P1", "P2"}
    ...
    occupancy["enabled"] = canonical == "P2"
    occupancy["observer_only"] = True
    occupancy["mutate"] = False
    occupancy["collect_diagnostics"] = canonical == "P2"
