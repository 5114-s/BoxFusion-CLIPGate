...
def test_public_p0_p1_p2_profiles_and_matrix_are_incremental():
...
def test_p_stages_are_deep_copied_children_of_quality_only_b6(stage):
    ...
    assert residual["enabled"] is (stage in {"P1", "P2"})
    assert residual["observer_only"] is True
    assert residual["mutate"] is False
    assert residual["collect_diagnostics"] is (stage in {"P1", "P2"})
    ...
    assert occupancy["enabled"] is (stage == "P2")
    assert occupancy["observer_only"] is True
    assert occupancy["mutate"] is False
    assert occupancy["collect_diagnostics"] is (stage == "P2")
