...
# Strict B5-v2 + frozen-B6 ablation. B5-v2 changes only object-local geometry
# and uses its learned quality only for the geometry gate. B6 ranks detections
# with original-geometry features, its training-time refiner_quality=0.50,
# and an explicit detector blend (0.40 by default). Supplemental output and
# Soft-NMS stay disabled.
#
# Usage:
...
RUN_TAG="${BOXFUSION_B5V2_RUN_TAG:-b5v2_frozen_b6_v2_blend040_extent040_ablation10}"
...
echo "Frozen B6 input contract: feature_geometry=original, refiner_quality=0.50"
