...
if [[ "$ABLATION_PROFILE" == "p1_residual_proposal_observer" \
      || "$ABLATION_PROFILE" == "p2_occupancy_topk_observer" ]]; then
    ...
    echo "P1 residual options require a P1/P2 profile" >&2
    exit 1
fi
if [[ "$ABLATION_PROFILE" == "p2_occupancy_topk_observer" ]]; then
    if [[ "$P1_RESIDUAL_MODE" != "infer" ]]; then
        echo "P2 requires P1 infer mode" >&2
        exit 1
    fi
    if [[ -z "$P2_OCCUPANCY_CHECKPOINT" \
          || ! -f "$P2_OCCUPANCY_CHECKPOINT" ]]; then
        echo "P2 requires a valid BOXFUSION_P2_OCCUPANCY_CHECKPOINT" >&2
        exit 1
    fi
elif [[ -n "$P2_OCCUPANCY_CHECKPOINT" ]]; then
    echo "P2 occupancy options require p2_occupancy_topk_observer" >&2
    exit 1
fi
if [[ "$ABLATION_PROFILE" == "p0_frozen_b6" ]]; then
    ...
    echo "BOXFUSION_P_STAGE is valid only for a P0/P1/P2 profile" >&2
    exit 1
fi
