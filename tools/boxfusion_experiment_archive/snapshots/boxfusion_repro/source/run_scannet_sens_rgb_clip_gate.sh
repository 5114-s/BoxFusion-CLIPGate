...
CONFIG="$WORK_ROOT/scannet_sens_rgb_clip_gate.yaml"
PRED_ROOT="$WORK_ROOT/results/scannet_sens_rgb_clip_gate"
LOG_ROOT="$WORK_ROOT/logs/scannet_sens_rgb_clip_gate"
...
echo "[$(date '+%F %T')] Starting CLIP-gated inference for $total ScanNet scenes"
echo "[$(date '+%F %T')] Host GPU: $HOST_GPU"
echo "[$(date '+%F %T')] Code commit: $(git -C "$CODE" rev-parse HEAD) + appearance-gate working tree"
...
while IFS= read -r scene || [[ -n "$scene" ]]; do
    ...
    echo "[$(date '+%F %T')] [$completed/$total] Completed $scene $gate_summary"
done < "$META"
...
CUDA_VISIBLE_DEVICES="$HOST_GPU" PYTHONDONTWRITEBYTECODE=1 \
    ...
    --dump_dir "$WORK_ROOT/evaluation/scannet_sens_rgb_clip_gate" \
    --num_point 40000 \
...
echo "[$(date '+%F %T')] CLIP-gated inference and evaluation completed"
