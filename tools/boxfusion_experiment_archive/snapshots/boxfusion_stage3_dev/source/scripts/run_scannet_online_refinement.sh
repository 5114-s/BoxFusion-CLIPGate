...
if [[ "$QUALITY_MODE" != "linear" && "$QUALITY_MODE" != "mlp" ]]; then
    echo "BOXFUSION_QUALITY_MODE must be linear or mlp" >&2
    exit 1
fi
...
if [[ ! -d "$LIVE_ROOT/upstream_clean/scannet_readme_frames" ]]; then
    echo "Missing score-preserving ScanNet frames under $LIVE_ROOT" >&2
    exit 1
fi
if [[ ! -d "$GT_ROOT" ]]; then
