...
def resolve_online_refinement_config(
    ...
    if p_stage not in {"DISABLED", "P0", "P1"}:
        raise ValueError("p_ablation_stage must be disabled, P0, or P1")
    expected_p_profile = {
        "DISABLED": "disabled",
        "P0": "p0_frozen_b6",
    ...
    expected_added = None if p_stage != "P1" else "residual_proposal"
    ...
    if p1_enabled != (p_stage == "P1"):
        raise ValueError(
            "residual_proposal.enabled disagrees with P0/P1 stage"
        )
    if p1_enabled and resolved["ablation_profile"] != "quality_only":
        raise ValueError(
            "P1 must be layered on the frozen quality_only B6 profile"
        )
    if not isinstance(
    ...
    if p1_enabled and (
        ...
        raise ValueError(
            "P1 observer requires diagnostics.enabled and "
    ...
    def __init__(
        ...
        *,
        ...
        if residual_cfg["enabled"]:
            ...
            self.residual_proposal_observer = P1ResidualProposalObserver(
                residual_cfg,
                head=residual_proposal_head,
                device=residual_cfg.get("device") or self.device,
