...
if [[ -n "$ABLATION_PROFILE" ]]; then
    case "$ABLATION_PROFILE" in
        observer|quality_observer|refit_only|supplemental_only|supplemental_conservative|quality_only|b3_memory_observer|b3_topk_refit_only|b3_b6|b3v2_memory_observer|b3v2_visibility_refit_only|b3v2_b6|missing_mask_graph_c1_recovery|missing_mask_graph_c2_geometry_observer|missing_mask_graph_c2_geometry|missing_mask_graph_c3_stitch_observer|full) ;;
        *)
...
if [[ -n "$SCANNET_MIN_EXTENT" ]]; then
    optional_args+=(--scannet-min-extent "$SCANNET_MIN_EXTENT")
fi
CUDA_VISIBLE_DEVICES="$GPU" \
PYTHONDONTWRITEBYTECODE=1 \
PYTHONUNBUFFERED=1 \
