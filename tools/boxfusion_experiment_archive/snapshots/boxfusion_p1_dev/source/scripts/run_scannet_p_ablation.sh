...
#   BOXFUSION_P_FULL100=1 ... bash scripts/run_scannet_p_ablation.sh P1 0,1
...
case "${STAGE^^}" in
    ...
    *) echo "Stage must be P0 or P1" >&2; exit 2 ;;
esac
...
if [[ "$STAGE" == "P1" && ! -f "$P1_CHECKPOINT" ]]; then
    echo "P1 requires BOXFUSION_P1_RESIDUAL_CHECKPOINT from train-only data" >&2
    exit 1
fi
if [[ "$STAGE" == "P0" ]]; then
    P1_CHECKPOINT=""
...
for output in outputs:
    try:
        ...
        raise SystemExit("P output roots must remain inside boxfusion_p1_dev")
PY
