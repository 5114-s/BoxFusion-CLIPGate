...
def test_pair_compatibility_missing_pair_rejects_geometry_match():
    manager = object_memory.CandidateTrackManager(manager_config())
    manager.update([observation([0, 0, 0])], frame_id=0)

    result = manager.update(
        [observation([0.02, 0, 0])],
        frame_id=1,
        pair_compatibility={},
    )
    ...
    assert manager.tracks[0].hit_count == 1
    assert manager.tracks[1].hit_count == 1


def test_pair_compatibility_bonus_changes_deterministic_track_ranking():
    ...
    result = manager.update(
        ...
        frame_id=1,
        pair_compatibility={(0, 0): 0.0, (1, 0): 0.25},
    )

    assert result.assignments == {0: 1}
    assert result.created_track_ids == ()
    assert manager.tracks[0].hit_count == 1
    assert manager.tracks[1].hit_count == 2


@pytest.mark.parametrize("value", [np.nan, np.inf, -np.inf])
def test_pair_compatibility_requires_finite_values_without_mutation(value):
    manager = object_memory.CandidateTrackManager(manager_config())
    manager.update([observation([0, 0, 0])], frame_id=0)

    with pytest.raises(ValueError, match="pair_compatibility value"):
        manager.update(
            [observation([0.02, 0, 0])],
            frame_id=1,
            pair_compatibility={(0, 0): value},
        )

    assert manager.last_update_frame == 0
    assert manager.last_lifecycle_step == 0
    assert sorted(manager.tracks) == [0]
    assert manager.tracks[0].hit_count == 1
    assert manager.tracks[0].memory.observation_count == 1


def test_pair_compatibility_prevents_same_frame_track_reuse():
    manager = object_memory.CandidateTrackManager(manager_config())
    manager.update([observation([0, 0, 0])], frame_id=0)
...
def test_pair_compatibility_rejects_malformed_mappings(
    pair_compatibility,
):
    manager = object_memory.CandidateTrackManager(manager_config())
    with pytest.raises(ValueError, match="pair_compatibility"):
        manager.update(
            [observation([0, 0, 0])],
            frame_id=0,
            pair_compatibility=pair_compatibility,
        )
    assert manager.last_update_frame is None
