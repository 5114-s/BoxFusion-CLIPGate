...
def _write_grouped_quality_dataset(path):
    features, targets = _write_quality_dataset(path)
    with np.load(path, allow_pickle=False) as archive:
        arrays = {key: archive[key].copy() for key in archive.files}
    arrays["scene_ids"] = np.asarray(
        [f"scene{index // 10:04d}_00" for index in range(len(features))]
    )
    np.savez(path, **arrays)
    return features, targets, arrays["scene_ids"]
...
def test_quality_scene_group_split_has_no_scene_leakage(tmp_path):
    path = tmp_path / "grouped.npz"
    _, _, scene_ids = _write_grouped_quality_dataset(path)
    data = load_quality_dataset(path)
    assert data.scene_ids is not None
    training, validation = deterministic_group_split(
        data.scene_ids, 0.25, 17
    )
    assert not (
        set(scene_ids[training]) & set(scene_ids[validation])
    )
...
def test_iou_aware_quality_training_writes_runtime_checkpoint(tmp_path):
    pytest.importorskip("torch")
    dataset = tmp_path / "grouped_iou.npz"
    output = tmp_path / "iou_mlp.npz"
    _write_grouped_quality_dataset(dataset)
    result = train_quality_calibrator(
        dataset,
        output,
        model="iou_mlp",
    ...
    assert result["model"] == "iou_mlp"
    assert result["split_kind"] == "scene"
    assert result["train_scenes"] + result["validation_scenes"] == 8
    assert np.isfinite(result["validation"]["mae_iou"])
    scorer = load_quality_scorer(output, method="iou_mlp")
    assert isinstance(scorer, IoUAwareMLPQualityScorer)
