...
def _p1_provenance(
    ...
    *,
    ...
    expected_head_architecture: str,
    expected_target_assignment_scope: str,
) -> dict:
    import torch
    ...
    legacy_contract = (
        ...
        and expected_target_assignment_scope == "scene_global"
    )
    if model_config is None and training_config is None and legacy_contract:
        # Released historical P1/P2-chain checkpoints predate both mappings.
        model_config = {}
        training_config = {}
    elif not isinstance(model_config, Mapping) or not isinstance(
        training_config, Mapping
    ):
        raise ValueError("P1 checkpoint lacks model/training contract")
    # The historical/per-voxel head stores ``head_architecture`` while the
    # strict native sparse head deliberately uses ``architecture`` in the
    # exact mapping consumed by ``from_model_config``.  Do not require the
    # sparse producer to add an unknown compatibility key.
    explicit_head = model_config.get("head_architecture")
    native_head = model_config.get("architecture")
    if (
        ...
        raise ValueError(
            "P1 checkpoint has conflicting head architecture fields"
        )
    head_architecture = (
        explicit_head if explicit_head is not None else native_head
    )
    target_assignment_scope = training_config.get(
        "target_assignment_scope"
    )
    # Historical P1 checkpoints predate the explicit contract fields.  Their
    # schema has only one possible interpretation.  P1R/P1S never receive
    # this compatibility treatment and must carry both fields explicitly.
    if (
        head_architecture is None
        and legacy_contract
    ):
        head_architecture = "per_voxel_mlp"
    if (
        target_assignment_scope is None
        and legacy_contract
    ):
        target_assignment_scope = "scene_global"
    if head_architecture != expected_head_architecture:
        raise ValueError(
            "P1 checkpoint head_architecture disagrees with the P stage"
        )
    if target_assignment_scope != expected_target_assignment_scope:
        raise ValueError(
            "P1 checkpoint target_assignment_scope disagrees with the P stage"
        )
    ...
    if overlap:
        raise ValueError(
            "P1 checkpoint train scenes overlap canonical validation: "
            + ", ".join(overlap[:8])
        )
    return {
        ...
        "head_architecture": str(head_architecture),
        "target_assignment_scope": str(target_assignment_scope),
    }
...
def build_manifest(args: argparse.Namespace) -> dict:
    ...
    if (
        args.stage in {"P1", "P1R", "P1S", "P1G", "P2", "P2V2", "P2V3"}
        and args.p1_checkpoint is None
    ):
        raise ValueError(
            "P1/P1R/P1S/P1G/P2/P2V2/P2V3 require a train-only P1 checkpoint"
        )
    ...
    if (
        ...
        raise ValueError(
            "P1/P1R/P1S/P1G/P2/P2V2/P2V3 require a canonical forbidden scene list"
        )
    required = {
    ...
    if args.stage in {"P1", "P1R", "P1S", "P1G", "P2", "P2V2", "P2V3"}:
    ...
    p1_contract = _P1_STAGE_CONTRACT.get(
        args.stage, _P1_STAGE_CONTRACT["P1"]
    )
    p1_training_provenance = (
        _p1_provenance(
            ...
            expected_head_architecture=p1_contract[0],
            expected_target_assignment_scope=p1_contract[1],
        )
        if args.stage in {"P1", "P1R", "P1S", "P1G", "P2", "P2V2", "P2V3"}
        else None
    )
        ...
        **{key: str(value) for key, value in output_roots.items()},
        ...
        "code_sha256": {
            ...
            in {
                ...
                root / "scripts" / "run_scannet_p1v2_ablation.sh",
                root / "tools" / "build_p_run_manifest.py",
                root / "tools" / "validate_p1v2_run_artifacts.py",
                root / "tools" / "report_p1v2_recall.py",
            }
        },
    }
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--stage",
        required=True,
        choices=("P0", "P1", "P1R", "P1S", "P1G", "P2", "P2V2", "P2V3"),
    )
