...
P1R_HEAD_SCHEMA = "boxfusion.p1r_snapshot_residual_head.v1"
P1S_HEAD_SCHEMA = "boxfusion.p1s_native_sparse_residual_head.v1"
P1_FEATURE_NAMES = (
    "log_point_count",
    "mean_red",
    "mean_green",
...
class ResidualProposalConfig:
    ...
    hidden_dim: int = 64
    head_architecture: str = "per_voxel_mlp"
    target_assignment_scope: str = "scene_global"
    ...
    def validated(self) -> "ResidualProposalConfig":
        ...
        head_architecture = str(self.head_architecture).strip().lower()
        if head_architecture not in {
            "per_voxel_mlp",
            "native_sparse_context_v1",
        }:
            raise ValueError(
                "residual_proposal.head_architecture must be "
                "per_voxel_mlp or native_sparse_context_v1"
            )
        target_assignment_scope = str(
            self.target_assignment_scope
        ).strip().lower()
        if target_assignment_scope not in {
            "scene_global",
            "snapshot_inside_only",
        }:
            raise ValueError(
                "residual_proposal.target_assignment_scope must be "
                "scene_global or snapshot_inside_only"
            )
    ...
    def _validate_relations(self) -> "ResidualProposalConfig":
        ...
        if (
            self.head_architecture == "native_sparse_context_v1"
            and self.target_assignment_scope != "snapshot_inside_only"
        ):
            raise ValueError(
                "native_sparse_context_v1 requires "
...
class ResidualObservation:
    ...
    error: str = ""
    geometry_points_world: Optional[np.ndarray] = None
    camera_position: Optional[np.ndarray] = None

    def __post_init__(self) -> None:
        points = (
            np.empty((0, 3), dtype=np.float32)
            if self.geometry_points_world is None
            else np.asarray(self.geometry_points_world, dtype=np.float32)
        )
        if (
            points.ndim != 2
            or points.shape[1] != 3
            ...
            raise ValueError(
                "geometry_points_world must be finite with shape [N,3]"
            )
        points = np.array(points, dtype=np.float32, copy=True)
        points.setflags(write=False)
        object.__setattr__(self, "geometry_points_world", points)
        ...
        if camera is not None:
            ...
            if camera.shape != (3,) or not np.isfinite(camera).all():
                raise ValueError(
                    "camera_position must be finite with shape [3]"
                )
            camera = np.array(camera, dtype=np.float32, copy=True)
            camera.setflags(write=False)
            object.__setattr__(self, "camera_position", camera)
...
def load_residual_proposal_head(
    ...
    *,
    ...
    architecture = str(expected_config.head_architecture).strip().lower()
    target_scope = str(
        expected_config.target_assignment_scope
    ).strip().lower()
    expected_schema = {
        ("per_voxel_mlp", "scene_global"): P1_HEAD_SCHEMA,
        (
            "per_voxel_mlp",
            "snapshot_inside_only",
        ): P1R_HEAD_SCHEMA,
    ...
    }.get((architecture, target_scope))
    if expected_schema is None:
        raise ValueError(
            "unsupported P1 head architecture/target-assignment pairing"
        )
    ...
    checkpoint_architecture = str(
        model_config.get(
            "head_architecture",
            model_config.get("architecture", "per_voxel_mlp"),
        )
    ).strip().lower()
    if checkpoint_architecture != architecture:
        raise ValueError(
            "P1 checkpoint head_architecture disagrees with runtime config"
        )
    training_config = payload.get("training_config")
    if not isinstance(training_config, Mapping):
        if expected_schema != P1_HEAD_SCHEMA:
            raise ValueError("P1 checkpoint lacks training_config")
        # Historical P1 checkpoints predate this explicit metadata.  Their
        # legacy schema has only the per-voxel/scene-global interpretation.
        checkpoint_target_scope = "scene_global"
    else:
        checkpoint_target_scope = str(
            training_config.get(
                "target_assignment_scope", "scene_global"
            )
        ).strip().lower()
    if checkpoint_target_scope != target_scope:
        raise ValueError(
            "P1 checkpoint target_assignment_scope disagrees with "
            "runtime config"
        )
    ...
    with torch.random.fork_rng(devices=fork_devices, enabled=True):
        if architecture == "native_sparse_context_v1":
            ...
            if callable(factory):
                ...
                model = NativeSparseResidualProposalHead(
                    ...
                    regression_dim=int(
                        model_config.get("regression_dim", -1)
                    ),
                )
        else:
...
class P1ResidualProposalObserver:
    ...
    def _run_head(
        ...
        """Run either the legacy MLP or coordinate-aware sparse P1 head."""

        if self.head is None:
            raise RuntimeError("P1 infer mode has no loaded head")
        coordinate_aware = (
            self.config.head_architecture
            == "native_sparse_context_v1"
        )
        if torch is not None and isinstance(self.head, nn.Module):
            ...
            with torch.inference_mode():
                if coordinate_aware:
                    coordinate_tensor = torch.as_tensor(
                        np.asarray(batch.coordinates),
                        dtype=torch.int64,
                        device=self.device,
                    )
                    logits_tensor, regression_tensor = self.head(
                        feature_tensor, coordinate_tensor
                    )
                else:
                    logits_tensor, regression_tensor = self.head(
                        feature_tensor
            ...
            return (
                ...
                regression_tensor.detach().cpu().numpy(),
            )
        if coordinate_aware:
            logits, regression = self.head(
                batch.features, batch.coordinates
            )
        else:
    ...
    def observe(
        ...
        *,
        ...
        if self.config.mode == "infer" and len(batch.centers):
            ...
            logits, regression = self._run_head(batch)
        ...
        observation = ResidualObservation(
            ...
            geometry_points_world=residual_points,
            camera_position=pose[:3, 3],
        )
    ...
    def diagnostic_payload(self) -> dict[str, np.ndarray]:
        ...
        if (
            ...
            self.config.target_assignment_scope
            == "snapshot_inside_only"
        ):
            stage = "P1R"
            ...
            stage = "P1"
            profile = "p1_residual_proposal_observer"
        return {
