...
DEFAULT_ONLINE_REFINEMENT_CONFIG: Dict[str, Any] = {
    ...
    "residual_proposal": ResidualProposalConfig().to_dict(),
    "p1_multiview_geometry": (
        P1MultiViewGeometryConfig().to_dict()
    ),
    "occupancy_topk": OccupancyTopKConfig().to_dict(),
    "p2_local_mask_geometry": P2LocalMaskGeometryConfig().to_dict(),
    "p2_reliability_fusion": P2ReliabilityFusionConfig().to_dict(),
    "appearance_memory": {
        "enabled": True,
        "masked_crop": True,
    },
...
def resolve_online_refinement_config(
    ...
    p_stage = str(resolved["p_ablation_stage"]).strip().upper()
    p_profile = str(resolved["p_ablation_profile"]).strip()
    if p_stage not in {
        "DISABLED",
        "P0",
        "P1",
        ...
        raise ValueError(
            "p_ablation_stage must be disabled, P0, P1, P1R, P1S, "
            "P1G, P2, P2V2, or P2V3"
        )
    expected_p_profile = {
        "DISABLED": "disabled",
    ...
    if p_profile != expected_p_profile:
        raise ValueError(
            "p_ablation_profile disagrees with p_ablation_stage"
        )
    expected_added = {
        "DISABLED": None,
        "P0": None,
    ...
    if resolved["p_added_module"] != expected_added:
        raise ValueError("p_added_module disagrees with p_ablation_stage")
    resolved["p_ablation_stage"] = (
        "disabled" if p_stage == "DISABLED" else p_stage
    )
    resolved["p_ablation_profile"] = p_profile
    resolved["residual_proposal"] = resolve_residual_proposal_config(
        resolved["residual_proposal"]
    ).to_dict()
    resolved["p1_multiview_geometry"] = (
        resolve_p1_multiview_geometry_config(
            resolved["p1_multiview_geometry"]
        ).to_dict()
    )
    resolved["occupancy_topk"] = resolve_occupancy_topk_config(
        resolved["occupancy_topk"]
    ).to_dict()
    resolved["p2_local_mask_geometry"] = (
        resolve_p2_local_mask_geometry_config(
            resolved["p2_local_mask_geometry"]
        ).to_dict()
    )
    resolved["p2_reliability_fusion"] = (
        resolve_p2_reliability_fusion_config(
            resolved["p2_reliability_fusion"]
        ).to_dict()
    )
    p1_enabled = bool(resolved["residual_proposal"]["enabled"])
    p1g_enabled = bool(
        resolved["p1_multiview_geometry"]["enabled"]
    )
    p2_enabled = bool(resolved["occupancy_topk"]["enabled"])
    p2v2_enabled = bool(
        resolved["p2_local_mask_geometry"]["enabled"]
    )
    p2v3_enabled = bool(
        resolved["p2_reliability_fusion"]["enabled"]
    )
    p1_stages = {
        "P1",
        "P1R",
    ...
    if p1_enabled != (p_stage in p1_stages):
        raise ValueError(
            "residual_proposal.enabled disagrees with P stage"
        )
    if p1g_enabled != (p_stage == "P1G"):
        raise ValueError(
            "p1_multiview_geometry.enabled disagrees with P stage"
        )
    if p2_enabled != (p_stage in {"P2", "P2V2", "P2V3"}):
        raise ValueError("occupancy_topk.enabled disagrees with P stage")
    if p2v2_enabled != (p_stage in {"P2V2", "P2V3"}):
        raise ValueError(
            "p2_local_mask_geometry.enabled disagrees with P stage"
        )
    if p2v3_enabled != (p_stage == "P2V3"):
        raise ValueError(
            "p2_reliability_fusion.enabled disagrees with P stage"
        )
    if p2_enabled and not p1_enabled:
        raise ValueError("P2 occupancy Top-K requires frozen P1")
    if p1g_enabled and not p1_enabled:
        raise ValueError(
            "P1G multi-view geometry requires frozen P1S"
        )
    if p2v2_enabled and not p2_enabled:
        raise ValueError(
            "P2-v2 local Mask-RGBD geometry requires frozen P2"
        )
    if p2v3_enabled and not p2v2_enabled:
        raise ValueError(
            "P2-v3 reliability fusion requires frozen P2-v2"
        )
    if p2v2_enabled and not np.isclose(
        float(
            resolved["p2_local_mask_geometry"][
                "occupancy_voxel_size"
            ]
        ),
        float(resolved["residual_proposal"]["voxel_size"]),
        rtol=0.0,
        atol=1e-12,
    ):
        raise ValueError(
            "P2-v2 occupancy_voxel_size must match the frozen P1 "
            "residual voxel_size"
        )
    if p2_enabled and resolved["residual_proposal"]["mode"] != "infer":
        raise ValueError("P2 occupancy Top-K requires P1 infer mode")
    if p1g_enabled and resolved["residual_proposal"]["mode"] != "infer":
        raise ValueError(
            "P1G multi-view geometry requires P1S infer mode"
        )
    expected_p1_contract = {
    ...
    if expected_p1_contract is not None:
        actual_p1_contract = (
            resolved["residual_proposal"]["head_architecture"],
            resolved["residual_proposal"]["target_assignment_scope"],
        )
        if actual_p1_contract != expected_p1_contract:
            raise ValueError(
                "residual_proposal architecture/target scope disagrees "
                f"with {p_stage}"
            )
    if p1_enabled and resolved["ablation_profile"] != "quality_only":
        raise ValueError(
            "P1/P1R/P1S/P1G/P2/P2-v2/P2-v3 must be layered on the "
            "frozen quality_only B6 profile"
        )
    if not isinstance(
    ...
    if (p1_enabled or p2_enabled) and (
        not diagnostics["enabled"] or not diagnostics["dump_track_memory"]
    ):
        raise ValueError(
            "P observers require diagnostics.enabled and "
            "diagnostics.dump_track_memory"
        )
    return resolved
...
def _empty_runtime_stats() -> Dict[str, Any]:
    return {
        ...
        "p1_calls": 0,
        "p1_input_points": 0,
        "p1_explained_points": 0,
        "p1_residual_points": 0,
        "p1_voxels": 0,
        "p1_raw_candidates": 0,
        "p1_voxelize_seconds": 0.0,
        "p1_head_seconds": 0.0,
        "p1_nms_seconds": 0.0,
        "p2_calls": 0,
        "p2_input_voxels": 0,
        "p2_eligible_voxels": 0,
        "p2_selected_voxels": 0,
        "p2_step_candidates": 0,
        "p2_seconds": 0.0,
        "p2v2_calls": 0,
        "p2v2_mask_observations": 0,
        "p2v2_mask_components": 0,
        "p2v2_eligible_pairs": 0,
        "p2v2_step_candidates": 0,
        "p2v2_seconds": 0.0,
        "p2v2_failures": 0,
        "p2v3_calls": 0,
        "p2v3_input_candidates": 0,
        "p2v3_eligible_candidates": 0,
        "p2v3_step_candidates": 0,
        "p2v3_seconds": 0.0,
        "p2v3_failures": 0,
        "rejected": Counter(),
    }
...
class OnlineRefinementController:
    ...
    def __init__(
        ...
        *,
        ...
        residual_proposal_head: Any = None,
        occupancy_topk_head: Any = None,
        ...
        self.residual_proposal_observer: Optional[
            P1ResidualProposalObserver
        ] = None
        self.p1_multiview_geometry_observer: Optional[
            P1MultiViewGeometryObserver
        ] = None
        self._p1g_payload_cache: Optional[
            Dict[str, np.ndarray]
        ] = None
        self._p1g_payload_scene_id: Optional[str] = None
        self.p2_local_mask_geometry_observer: Optional[
            P2LocalMaskGeometryObserver
        ] = None
        self.p2_reliability_fusion_observer: Optional[
            P2ReliabilityFusionObserver
        ] = None
        ...
        residual_cfg = self.config["residual_proposal"]
        occupancy_cfg = self.config["occupancy_topk"]
        if residual_cfg["enabled"]:
            expected_b6_sha = None
            if (
                residual_cfg["mode"] == "infer"
                and residual_proposal_head is None
            ):
                quality_checkpoint = self.config["quality"].get("checkpoint")
                if not quality_checkpoint:
                    raise ValueError(
                        "P1 inference requires the frozen B6 quality "
                        "checkpoint for provenance verification"
                    )
                expected_b6_sha = sha256_file(quality_checkpoint)
            if occupancy_cfg["enabled"]:
                p1_checkpoint = residual_cfg.get("checkpoint")
                if not p1_checkpoint and residual_proposal_head is None:
                    raise ValueError("P2 requires the frozen P1 checkpoint")
                expected_p1_sha = (
                    sha256_file(p1_checkpoint)
                    if p1_checkpoint
                    else None
                )
                self.residual_proposal_observer = (
                    P2OccupancyTopKObserver(
                        residual_cfg,
                        occupancy_cfg,
                        p1_head=residual_proposal_head,
                        occupancy_head=occupancy_topk_head,
                        p1_device=(
                            residual_cfg.get("device") or "cpu"
                        ),
                        expected_b6_checkpoint_sha256=expected_b6_sha,
                        expected_p1_checkpoint_sha256=expected_p1_sha,
                    )
                )
            else:
                self.residual_proposal_observer = (
                    P1ResidualProposalObserver(
                        residual_cfg,
                        head=residual_proposal_head,
                        device=residual_cfg.get("device") or "cpu",
                        expected_b6_checkpoint_sha256=expected_b6_sha,
                    )
                )
        elif residual_proposal_head is not None:
            raise ValueError(
                "injected residual_proposal_head requires P1 enabled"
            )
        elif occupancy_cfg["enabled"] or occupancy_topk_head is not None:
            raise ValueError("P2 occupancy Top-K requires frozen P1")

        p1g_cfg = self.config["p1_multiview_geometry"]
        if p1g_cfg["enabled"]:
            if self.residual_proposal_observer is None:
                raise ValueError(
                    "P1G multi-view geometry requires the frozen P1S "
                    "observer"
                )
            parent_checkpoint_sha256 = str(
                self.residual_proposal_observer.checkpoint_sha256
            ).strip()
            if not parent_checkpoint_sha256:
                raise ValueError(
                    "P1G requires an identified frozen P1S checkpoint"
                )
            self.p1_multiview_geometry_observer = (
                P1MultiViewGeometryObserver(
                    p1g_cfg,
        ...
        p2v2_cfg = self.config["p2_local_mask_geometry"]
        if p2v2_cfg["enabled"]:
            if not isinstance(
                self.residual_proposal_observer,
                P2OccupancyTopKObserver,
                ...
                raise ValueError(
                    "P2-v2 requires the frozen P2 occupancy observer"
                )
            self.p2_local_mask_geometry_observer = (
                P2LocalMaskGeometryObserver(
                    p2v2_cfg,
                    parent_p2_checkpoint_sha256=(
                        self.residual_proposal_observer
                        .occupancy_checkpoint_sha256
                    ),
                    provider_name=str(
                        proposals_cfg.get(
        ...
        p2v3_cfg = self.config["p2_reliability_fusion"]
        if p2v3_cfg["enabled"]:
            if (
                self.p2_local_mask_geometry_observer is None
                or not isinstance(
                    self.residual_proposal_observer,
                    P2OccupancyTopKObserver,
                )
                ...
                raise ValueError(
                    "P2-v3 requires the frozen P2-v2 observer chain"
                )
            self.p2_reliability_fusion_observer = (
                P2ReliabilityFusionObserver(
                    p2v3_cfg,
                    parent_p2_checkpoint_sha256=(
                        self.residual_proposal_observer
    ...
    def reset_scene(self, scene_id: str) -> None:
        ...
        self._fragment_stitch_refresh_complete = False
        self._last_fragment_stitch_error = ""
        self._p1g_payload_cache = None
        self._p1g_payload_scene_id = None
        if self.residual_proposal_observer is not None:
            self.residual_proposal_observer.reset(self.scene_id)
        if self.p1_multiview_geometry_observer is not None:
            self.p1_multiview_geometry_observer.reset(self.scene_id)
        if self.p2_local_mask_geometry_observer is not None:
            self.p2_local_mask_geometry_observer.reset(self.scene_id)
        if self.p2_reliability_fusion_observer is not None:
            self.p2_reliability_fusion_observer.reset(self.scene_id)
        if self.enabled:
            self.track_manager = CandidateTrackManager(
                self.object_config,
                archive_confirmed=self.config["candidate_lifecycle"][
    ...
    def _process_generic_geometry_keyframe(
        ...
        *,
        ...
        """Replay one SAM3 cache frame into the isolated C4 global memory."""
    ...
    def process_keyframe(
        ...
        *,
        ...
        missing_candidates = [
            item for _, item in missing_candidate_rows
        ]
        ...
        if self.residual_proposal_observer is not None:
            p1_observation = self.residual_proposal_observer.observe(
                image=image,
                depth=depth,
                intrinsics=intrinsics,
                camera_to_world=camera_to_world,
                global_corners=np.asarray(
                    global_corners, dtype=np.float32
                ),
            ...
            # P1G is finalized lazily from the complete frozen P1S
            # observation stream.  A new P1S step invalidates only its
            # detached diagnostic cache; it never touches formal outputs.
            self._p1g_payload_cache = None
            self._p1g_payload_scene_id = None
            p1_batch = p1_observation.voxel_batch
            self.stats["p1_calls"] += 1
            self.stats["p1_input_points"] += int(
                p1_batch.input_point_count
            )
            self.stats["p1_explained_points"] += int(
                p1_batch.explained_point_count
            )
            self.stats["p1_residual_points"] += int(
                p1_batch.residual_point_count
            )
            self.stats["p1_voxels"] += len(p1_batch.centers)
            self.stats["p1_raw_candidates"] += len(
                p1_observation.proposals
            )
            self.stats["p1_voxelize_seconds"] += float(
                p1_observation.voxelize_seconds
            )
            self.stats["p1_head_seconds"] += float(
                p1_observation.head_seconds
            )
            self.stats["p1_nms_seconds"] += float(
                p1_observation.nms_seconds
            )
            if isinstance(
                self.residual_proposal_observer,
                P2OccupancyTopKObserver,
                ...
                p2_observation = (
                    self.residual_proposal_observer.p2_observations[-1]
                )
                self.stats["p2_calls"] += 1
                self.stats["p2_input_voxels"] += len(
                    p2_observation.base.voxel_batch.features
                )
                self.stats["p2_eligible_voxels"] += int(
                    p2_observation.eligible_voxels
                )
                self.stats["p2_selected_voxels"] += int(
                    p2_observation.selected_voxels
                )
                self.stats["p2_step_candidates"] += len(
                    p2_observation.selected
                )
                self.stats["p2_seconds"] += float(
                    p2_observation.occupancy_seconds
                )
                p2v2_observer = (
                    self.p2_local_mask_geometry_observer
                )
                if p2v2_observer is not None:
                    p2v2_started = time.perf_counter()
                    mask_inputs: list[P2MaskRGBDInput] = []
                    try:
                        mask_inputs = [
                            P2MaskRGBDInput(
                                source_id=(
                                    f"{provider_cache_key}:lifted:"
                                    f"{int(index):04d}"
                                ),
                                frame_index=frame_index,
                                provider_step=provider_step,
                                score=float(item.proposal.score),
                            ...
                            for index, item in missing_candidate_rows
                        ]
                        p2v2_step = p2v2_observer.observe(
                            scene_id=requested_scene,
                            p2_observation=p2_observation,
                            masks=mask_inputs,
                        )
                        self.stats["p2v2_calls"] += 1
                        self.stats[
                            "p2v2_mask_observations"
                        ] += p2v2_step.mask_observation_count
                        self.stats[
                            "p2v2_mask_components"
                        ] += p2v2_step.mask_component_count
                        self.stats[
                            "p2v2_eligible_pairs"
                        ] += p2v2_step.eligible_pair_count
                        self.stats[
                            "p2v2_step_candidates"
                        ] += len(p2v2_step.candidates)
                        self.stats["p2v2_seconds"] += float(
                            p2v2_step.seconds
                        )
                        p2v3_observer = (
                            self.p2_reliability_fusion_observer
                        )
                        if p2v3_observer is not None:
                            p2v3_started = time.perf_counter()
                            try:
                                p2v3_step = p2v3_observer.observe(
                                    scene_id=requested_scene,
                                    p2v2_step=p2v2_step,
                                )
                                self.stats["p2v3_calls"] += 1
                                self.stats[
                                    "p2v3_input_candidates"
                                ] += int(
                                    p2v3_step.input_candidate_count
                                )
                                self.stats[
                                    "p2v3_eligible_candidates"
                                ] += int(
                                    p2v3_step.eligible_candidate_count
                                )
                                self.stats[
                                    "p2v3_step_candidates"
                                ] += len(p2v3_step.candidates)
                                self.stats["p2v3_seconds"] += float(
                                    p2v3_step.seconds
                                )
                            except Exception as p2v3_error:
                                elapsed = float(
                                    time.perf_counter()
                                    - p2v3_started
                                )
                                ...
                                self.stats["p2v3_failures"] += 1
                                self.stats["p2v3_seconds"] += elapsed
                                p2v3_observer.record_failure(
                                    scene_id=requested_scene,
                                    frame_index=frame_index,
                                    provider_step=provider_step,
                                    input_candidate_count=len(
                                        p2v2_step.candidates
                                    ),
                        ...
                        self.stats["p2v2_calls"] += 1
                        self.stats["p2v2_failures"] += 1
                        self.stats[
                            "p2v2_mask_observations"
                        ] += len(mask_inputs)
                        p2v2_observer.record_failure(
                            scene_id=requested_scene,
                            frame_index=frame_index,
                            provider_step=provider_step,
                            ...
                            mask_observation_count=len(mask_inputs),
                            elapsed_seconds=float(
                                time.perf_counter() - p2v2_started
                            ),
                            error=error,
                        )
                        self.stats["p2v2_seconds"] += float(
    ...
    def finalize(
        ...
        *,
        ...
        # P1G consumes only the complete, frozen P1S scene candidates and
        # observations.  Formal BoxFusion output arrays above are already
        # assembled, and P1G has no mutation API.
        self._p1g_diagnostic_payload(scene_id=scene_id)
    ...
    def _p1_diagnostic_payload(self) -> Dict[str, np.ndarray]:
        observer = self.residual_proposal_observer
        if observer is not None:
            return observer.diagnostic_payload()
        config_json = json.dumps(
            self.config["residual_proposal"],
            sort_keys=True,
            separators=(",", ":"),
            default=str,
        )
        return {
            ...
            "p1_stage": np.asarray(
                self.config.get("p_ablation_stage", "disabled")
            ),
            "p1_profile": np.asarray(
                self.config.get("p_ablation_profile", "disabled")
            ),
            "p1_enabled": np.asarray(False, dtype=bool),
            ...
            "p1_head_architecture": np.asarray(
                self.config["residual_proposal"][
                    "head_architecture"
                ]
            ),
            ...
            "p1_step_frame_ids": np.empty((0,), dtype=np.int64),
            "p1_step_provider_steps": np.empty((0,), dtype=np.int64),
            "p1_step_input_point_counts": np.empty(
                (0,), dtype=np.int64
            ),
            "p1_step_explained_point_counts": np.empty(
                (0,), dtype=np.int64
            ),
            "p1_step_residual_point_counts": np.empty(
                (0,), dtype=np.int64
            ),
            "p1_step_voxel_counts": np.empty((0,), dtype=np.int64),
            "p1_step_candidate_counts": np.empty((0,), dtype=np.int64),
            "p1_step_voxelize_seconds": np.empty(
                (0,), dtype=np.float64
            ),
            "p1_step_head_seconds": np.empty((0,), dtype=np.float64),
            "p1_step_nms_seconds": np.empty((0,), dtype=np.float64),
            "p1_voxel_offsets": np.asarray([0], dtype=np.int64),
            "p1_voxel_coords": np.empty((0, 3), dtype=np.int32),
            "p1_voxel_centers": np.empty((0, 3), dtype=np.float32),
            "p1_voxel_features": np.empty(
                (0, len(P1_FEATURE_NAMES)), dtype=np.float32
            ),
            "p1_voxel_point_counts": np.empty((0,), dtype=np.int32),
            "p1_candidate_ids": np.empty((0,), dtype=np.str_),
            "p1_candidate_frame_ids": np.empty((0,), dtype=np.int64),
            "p1_candidate_provider_steps": np.empty(
                (0,), dtype=np.int64
            ),
            "p1_candidate_boxes": np.empty((0, 6), dtype=np.float32),
            "p1_candidate_corners": np.empty(
                (0, 8, 3), dtype=np.float32
            ),
            "p1_candidate_scores": np.empty((0,), dtype=np.float32),
            "p1_candidate_objectness": np.empty(
                (0,), dtype=np.float32
            ),
            "p1_candidate_residual_point_counts": np.empty(
                (0,), dtype=np.int64
            ),
            "p1_candidate_nearest_b6_stable_ids": np.empty(
                (0,), dtype=np.int64
            ),
            "p1_candidate_nearest_b6_iou": np.empty(
                (0,), dtype=np.float32
            ),
            "p1_raw_candidate_ids": np.empty((0,), dtype=np.str_),
            "p1_raw_candidate_boxes": np.empty(
                (0, 6), dtype=np.float32
            ),
            "p1_raw_candidate_scores": np.empty(
                (0,), dtype=np.float32
            ),
        }

    def _p1g_diagnostic_payload(
        self, *, scene_id: Optional[str] = None
    ) -> Dict[str, np.ndarray]:
        """Finalize and cache the detached P1G scene observer payload."""

        observer = self.p1_multiview_geometry_observer
        if observer is None:
            return {}
        selected_scene = scene_id or self.scene_id
        if selected_scene is None:
            raise RuntimeError(
                "P1G diagnostics require an initialized scene"
            )
        if (
            self._p1g_payload_cache is None
            or self._p1g_payload_scene_id != selected_scene
        ):
            parent = self.residual_proposal_observer
            if parent is None:
                raise RuntimeError(
                    "P1G observer lost its frozen P1S parent"
                )
            anchors = parent.scene_candidates()
            observations = tuple(parent.observations)
            ...
            if str(np.asarray(payload["p1g_schema"]).item()) != (
                P1G_DIAGNOSTIC_SCHEMA
            ):
                raise RuntimeError("P1G diagnostic schema drifted")
            if str(np.asarray(payload["p1g_stage"]).item()) != "P1G":
                raise RuntimeError("P1G diagnostic stage drifted")
            if str(np.asarray(payload["p1g_profile"]).item()) != (
                P1G_PROFILE
            ):
                raise RuntimeError("P1G diagnostic profile drifted")
            if str(
                np.asarray(
                    payload["p1g_parent_checkpoint_sha256"]
                ).item()
            ) != str(parent.checkpoint_sha256):
                raise RuntimeError(
                    "P1G parent checkpoint provenance drifted"
                )
            if bool(
                np.asarray(payload["p1g_mutation_enabled"]).item()
            ) or int(
                np.asarray(payload["p1g_applied_count"]).item()
            ) != 0:
                raise RuntimeError(
                    "P1G violated its observer-only output contract"
                )
            if len(payload["p1g_parent_candidate_ids"]) != len(
                anchors
            ):
                raise RuntimeError(
                    "P1G candidates are not one-to-one with frozen "
                    "P1S anchors"
                )
            self._p1g_payload_cache = {
                key: np.array(value, copy=True)
                for key, value in payload.items()
            }
            self._p1g_payload_scene_id = selected_scene
        return self._p1g_payload_cache

    def _p2v2_diagnostic_payload(self) -> Dict[str, np.ndarray]:
        observer = self.p2_local_mask_geometry_observer
        if observer is None:
            return {}
        payload = observer.diagnostic_payload()
        if str(np.asarray(payload["p2v2_schema"]).item()) != (
            P2V2_DIAGNOSTIC_SCHEMA
        ):
            raise RuntimeError("P2-v2 diagnostic schema drifted")
        return payload

    def _p2v3_diagnostic_payload(self) -> Dict[str, np.ndarray]:
        observer = self.p2_reliability_fusion_observer
        if observer is None:
            return {}
        payload = observer.diagnostic_payload()
        if str(np.asarray(payload["p2v3_schema"]).item()) != (
            P2V3_DIAGNOSTIC_SCHEMA
        ):
            raise RuntimeError("P2-v3 diagnostic schema drifted")
        return payload
    ...
    def _dump_diagnostics(
        ...
        p1_payload = self._p1_diagnostic_payload()
        p1g_payload = self._p1g_diagnostic_payload(scene_id=scene_id)
        p2v2_payload = self._p2v2_diagnostic_payload()
        p2v3_payload = self._p2v3_diagnostic_payload()
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "p_ablation_stage": self.config["p_ablation_stage"],
            "p_ablation_profile": self.config["p_ablation_profile"],
            "p1_enabled": bool(
                self.config["residual_proposal"]["enabled"]
            ),
            "p1_head_architecture": self.config[
                "residual_proposal"
            ]["head_architecture"],
            "p1_target_assignment_scope": self.config[
                "residual_proposal"
            ]["target_assignment_scope"],
            "p1_mutation_enabled": False,
            "p1_applied_count": 0,
            "p1_calls": int(self.stats["p1_calls"]),
            "p1_input_points": int(self.stats["p1_input_points"]),
            "p1_explained_points": int(
                self.stats["p1_explained_points"]
            ),
            "p1_residual_points": int(
                self.stats["p1_residual_points"]
            ),
            "p1_voxels": int(self.stats["p1_voxels"]),
            "p1_raw_candidates": int(
                self.stats["p1_raw_candidates"]
            ),
            "p1_scene_candidates": (
                len(self.residual_proposal_observer.scene_candidates())
                if self.residual_proposal_observer is not None
                else 0
            ),
            "p1_voxelize_seconds": float(
                self.stats["p1_voxelize_seconds"]
            ),
            "p1_head_seconds": float(self.stats["p1_head_seconds"]),
            "p1_nms_seconds": float(self.stats["p1_nms_seconds"]),
            "p1_config_json": json.dumps(
                self.config["residual_proposal"],
                sort_keys=True,
                separators=(",", ":"),
                default=str,
            ...
            "p1g_enabled": bool(
                self.config["p1_multiview_geometry"]["enabled"]
            ),
            "p1g_observer_only": True,
            "p1g_mutation_enabled": False,
            "p1g_applied_count": 0,
            "p1g_scene_candidates": (
                len(self.p1_multiview_geometry_observer.candidates)
                if self.p1_multiview_geometry_observer is not None
                else 0
            ),
            "p1g_failure_count": (
                int(self.p1_multiview_geometry_observer.failure_count)
                if self.p1_multiview_geometry_observer is not None
                else 0
            ),
            "p1g_seconds": (
                float(
                    self.p1_multiview_geometry_observer.runtime_seconds
                )
                if self.p1_multiview_geometry_observer is not None
                else 0.0
            ),
            "p1g_config_json": json.dumps(
                self.config["p1_multiview_geometry"],
                sort_keys=True,
                separators=(",", ":"),
                default=str,
            ),
            "p2_enabled": bool(self.config["occupancy_topk"]["enabled"]),
            "p2_mutation_enabled": False,
            "p2_applied_count": 0,
            "p2_calls": int(self.stats["p2_calls"]),
            "p2_input_voxels": int(self.stats["p2_input_voxels"]),
            "p2_eligible_voxels": int(
                self.stats["p2_eligible_voxels"]
            ),
            "p2_selected_voxels": int(
                self.stats["p2_selected_voxels"]
            ),
            "p2_step_candidates": int(
                self.stats["p2_step_candidates"]
            ),
            "p2_scene_candidates": (
                len(self.residual_proposal_observer.p2_scene_candidates())
                if isinstance(
                    self.residual_proposal_observer,
                    P2OccupancyTopKObserver,
                )
            ...
            "p2_seconds": float(self.stats["p2_seconds"]),
            "p2_config_json": json.dumps(
                self.config["occupancy_topk"],
                sort_keys=True,
                separators=(",", ":"),
                default=str,
            ),
            "p2v2_enabled": bool(
                self.config["p2_local_mask_geometry"]["enabled"]
            ),
            "p2v2_mutation_enabled": False,
            "p2v2_applied_count": 0,
            "p2v2_calls": int(self.stats["p2v2_calls"]),
            "p2v2_mask_observations": int(
                self.stats["p2v2_mask_observations"]
            ),
            "p2v2_mask_components": int(
                self.stats["p2v2_mask_components"]
            ),
            "p2v2_eligible_pairs": int(
                self.stats["p2v2_eligible_pairs"]
            ),
            "p2v2_step_candidates": int(
                self.stats["p2v2_step_candidates"]
            ),
            "p2v2_scene_candidates": (
                len(
                    self.p2_local_mask_geometry_observer
                    .scene_candidates()
                )
            ...
            "p2v2_failures": int(self.stats["p2v2_failures"]),
            "p2v2_seconds": float(self.stats["p2v2_seconds"]),
            "p2v2_config_json": json.dumps(
                self.config["p2_local_mask_geometry"],
                sort_keys=True,
                separators=(",", ":"),
                default=str,
            ),
            "p2v3_enabled": bool(
                self.config["p2_reliability_fusion"]["enabled"]
            ),
            "p2v3_mutation_enabled": False,
            "p2v3_applied_count": 0,
            "p2v3_calls": int(self.stats["p2v3_calls"]),
            "p2v3_input_candidates": int(
                self.stats["p2v3_input_candidates"]
            ),
            "p2v3_eligible_candidates": int(
                self.stats["p2v3_eligible_candidates"]
            ),
            "p2v3_step_candidates": int(
                self.stats["p2v3_step_candidates"]
            ),
            "p2v3_scene_candidates": (
                len(
                    self.p2_reliability_fusion_observer
                    .scene_candidates()
                )
            ...
            "p2v3_failures": int(self.stats["p2v3_failures"]),
            "p2v3_seconds": float(self.stats["p2v3_seconds"]),
            "p2v3_config_json": json.dumps(
                self.config["p2_reliability_fusion"],
                sort_keys=True,
                separators=(",", ":"),
                default=str,
            ),
    ...
    def summary_text(self) -> str:
        ...
        return (
            ...
            "p1(calls/voxels/raw/scene)="
            f"{summary['p1_calls']}/{summary['p1_voxels']}/"
            f"{summary['p1_raw_candidates']}/"
            f"{summary['p1_scene_candidates']} "
            "(mutate/applied="
            f"{int(summary['p1_mutation_enabled'])}/"
            f"{summary['p1_applied_count']}), "
            "p2(calls/voxels/topk/scene)="
            f"{summary['p2_calls']}/{summary['p2_input_voxels']}/"
            f"{summary['p2_selected_voxels']}/"
            f"{summary['p2_scene_candidates']} "
            "(mutate/applied="
            f"{int(summary['p2_mutation_enabled'])}/"
            f"{summary['p2_applied_count']}), "
            "p2v2(calls/masks/components/pairs/step/scene)="
            f"{summary['p2v2_calls']}/"
            f"{summary['p2v2_mask_observations']}/"
            f"{summary['p2v2_mask_components']}/"
            f"{summary['p2v2_eligible_pairs']}/"
            f"{summary['p2v2_step_candidates']}/"
            f"{summary['p2v2_scene_candidates']} "
            "(mutate/applied/failures="
            f"{int(summary['p2v2_mutation_enabled'])}/"
            f"{summary['p2v2_applied_count']}/"
            f"{summary['p2v2_failures']}), "
            "p2v3(calls/input/eligible/step/scene)="
            f"{summary['p2v3_calls']}/"
            f"{summary['p2v3_input_candidates']}/"
            f"{summary['p2v3_eligible_candidates']}/"
            f"{summary['p2v3_step_candidates']}/"
            f"{summary['p2v3_scene_candidates']} "
            "(mutate/applied/failures="
            f"{int(summary['p2v3_mutation_enabled'])}/"
            f"{summary['p2v3_applied_count']}/"
            f"{summary['p2v3_failures']}), "
            ...
            f"p2v2_s={summary['p2v2_seconds']:.3f}, "
            f"p2v3_s={summary['p2v3_seconds']:.3f}, "
