...
under the isolated checkout. They do not modify the validated
`boxfusion_b6_dev` checkout.
...
For the leakage-safe multi-task B6 scorer, pure quality-only ablation, and
minimum-extent diagnostic, see [b6_iou_quality.md](b6_iou_quality.md).

For the per-view Top-K Mask-RGBD B3 memory, isolated identity/refit/B3+B6
profiles, and fixed10/full100 commands, see
[b3_topk_mask_rgbd.md](b3_topk_mask_rgbd.md).
