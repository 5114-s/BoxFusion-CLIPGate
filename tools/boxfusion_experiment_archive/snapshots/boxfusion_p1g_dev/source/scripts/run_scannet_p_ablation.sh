...
# Frozen-B6 P0/P1/P2/P2V2/P2V3 protocol.
#
# Fixed ten scenes:
...
case "${STAGE^^}" in
    ...
    P2V2) STAGE=P2V2; PROFILE=p2v2_local_component_mask_rgbd_observer ;;
    P2V3) STAGE=P2V3; PROFILE=p2v3_reliability_geometry_fusion_observer ;;
    *) echo "Stage must be P0, P1, P2, P2V2, or P2V3" >&2; exit 2 ;;
esac
...
if [[ "$STAGE" != "P0" && ! -f "$P1_CHECKPOINT" ]]; then
    echo "P1/P2/P2V2/P2V3 require BOXFUSION_P1_RESIDUAL_CHECKPOINT" >&2
    exit 1
fi
if [[ ( "$STAGE" == "P2" || "$STAGE" == "P2V2" \
        || "$STAGE" == "P2V3" ) \
      && ! -f "$P2_CHECKPOINT" ]]; then
    echo "P2/P2V2/P2V3 require BOXFUSION_P2_OCCUPANCY_CHECKPOINT" >&2
    exit 1
fi
if [[ "$STAGE" == "P0" ]]; then
    P1_CHECKPOINT=""
...
for output in outputs:
    try:
        ...
        raise SystemExit("P output roots must remain inside boxfusion_p2v3_dev")
PY
