...
P2_OCCUPANCY_CHECKPOINT="${BOXFUSION_P2_OCCUPANCY_CHECKPOINT:-}"
...
if [[ "$ABLATION_PROFILE" == "p1_residual_proposal_observer" \
      || "$ABLATION_PROFILE" == "p1r_snapshot_target_residual_observer" \
      || "$ABLATION_PROFILE" == "p1s_native_sparse_context_observer" \
      || "$ABLATION_PROFILE" == "p1g_multiview_occupancy_msr_observer" \
      || "$ABLATION_PROFILE" == "p2_occupancy_topk_observer" \
      || "$ABLATION_PROFILE" == "p2v2_local_component_mask_rgbd_observer" \
      || "$ABLATION_PROFILE" == "p2v3_reliability_geometry_fusion_observer" ]]; then
    ...
    echo "P1 residual options require a P1/P1R/P1S/P1G/P2/P2V2/P2V3 profile" >&2
    exit 1
fi
if [[ "$ABLATION_PROFILE" == "p2_occupancy_topk_observer" \
      || "$ABLATION_PROFILE" == "p2v2_local_component_mask_rgbd_observer" \
      || "$ABLATION_PROFILE" == "p2v3_reliability_geometry_fusion_observer" ]]; then
    if [[ "$P1_RESIDUAL_MODE" != "infer" ]]; then
        echo "P2/P2V2/P2V3 require P1 infer mode" >&2
        exit 1
    fi
    if [[ -z "$P2_OCCUPANCY_CHECKPOINT" \
          || ! -f "$P2_OCCUPANCY_CHECKPOINT" ]]; then
        echo "P2/P2V2/P2V3 require a valid BOXFUSION_P2_OCCUPANCY_CHECKPOINT" >&2
        exit 1
    fi
elif [[ -n "$P2_OCCUPANCY_CHECKPOINT" ]]; then
    echo "P2 occupancy options require a P2/P2V2/P2V3 profile" >&2
    exit 1
fi
...
elif [[ "$ABLATION_PROFILE" == "p1r_snapshot_target_residual_observer" ]]; then
    if [[ "$P1_RESIDUAL_MODE" != "infer" || "$P_STAGE" != "P1R" ]]; then
        echo "P1R profile requires infer mode and BOXFUSION_P_STAGE=P1R" >&2
        exit 1
    fi
elif [[ "$ABLATION_PROFILE" == "p1s_native_sparse_context_observer" ]]; then
    if [[ "$P1_RESIDUAL_MODE" != "infer" || "$P_STAGE" != "P1S" ]]; then
        echo "P1S profile requires infer mode and BOXFUSION_P_STAGE=P1S" >&2
        exit 1
    fi
elif [[ "$ABLATION_PROFILE" == "p1g_multiview_occupancy_msr_observer" ]]; then
    if [[ "$P1_RESIDUAL_MODE" != "infer" || "$P_STAGE" != "P1G" ]]; then
        echo "P1G profile requires infer mode and BOXFUSION_P_STAGE=P1G" >&2
        exit 1
    fi
elif [[ "$ABLATION_PROFILE" == "p2_occupancy_topk_observer" ]]; then
    if [[ "$P_STAGE" != "P2" ]]; then
        echo "P2 profile requires BOXFUSION_P_STAGE=P2" >&2
        exit 1
    fi
elif [[ "$ABLATION_PROFILE" == "p2v2_local_component_mask_rgbd_observer" ]]; then
    if [[ "$P_STAGE" != "P2V2" ]]; then
        echo "P2V2 profile requires BOXFUSION_P_STAGE=P2V2" >&2
        exit 1
    fi
elif [[ "$ABLATION_PROFILE" == "p2v3_reliability_geometry_fusion_observer" ]]; then
    if [[ "$P_STAGE" != "P2V3" ]]; then
        echo "P2V3 profile requires BOXFUSION_P_STAGE=P2V3" >&2
        exit 1
    fi
elif [[ -n "$P_STAGE" ]]; then
    echo "BOXFUSION_P_STAGE is valid only for a P0/P1/P1R/P1S/P1G/P2/P2V2/P2V3 profile" >&2
    exit 1
fi
...
elif [[ "$ABLATION_PROFILE" == "p1r_snapshot_target_residual_observer" \
        || "$ABLATION_PROFILE" == "p1s_native_sparse_context_observer" ]]; then
    required_files+=("$ROOT/tools/validate_p1v2_run_artifacts.py")
elif [[ "$ABLATION_PROFILE" == "p1g_multiview_occupancy_msr_observer" ]]; then
    required_files+=("$ROOT/tools/validate_p1g_run_artifacts.py")
elif [[ "$ABLATION_PROFILE" == "p2_occupancy_topk_observer" ]]; then
    required_files+=("$ROOT/tools/validate_p2_run_artifacts.py")
elif [[ "$ABLATION_PROFILE" == "p2v2_local_component_mask_rgbd_observer" ]]; then
    required_files+=("$ROOT/tools/validate_p2v2_run_artifacts.py")
elif [[ "$ABLATION_PROFILE" == "p2v3_reliability_geometry_fusion_observer" ]]; then
    required_files+=("$ROOT/tools/validate_p2v3_run_artifacts.py")
fi
...
if [[ -n "$P_STAGE" ]]; then
    ...
    if [[ "$P_STAGE" == "P1" || "$P_STAGE" == "P1R" \
          || "$P_STAGE" == "P1S" || "$P_STAGE" == "P1G" \
          || "$P_STAGE" == "P2" \
          || "$P_STAGE" == "P2V2" || "$P_STAGE" == "P2V3" ]]; then
        p_manifest_args+=(
    ...
    if [[ "$P_STAGE" == "P2" || "$P_STAGE" == "P2V2" \
          || "$P_STAGE" == "P2V3" ]]; then
        p_manifest_args+=(
            --p2-checkpoint "$P2_OCCUPANCY_CHECKPOINT"
        )
    fi
...
echo "[$(date '+%F %T')] P2 occupancy checkpoint: ${P2_OCCUPANCY_CHECKPOINT:-disabled}"
    ...
    while IFS= read -r scene || [[ -n "$scene" ]]; do
        ...
        if [[ -s "$prediction" \
              && ( "$ABLATION_PROFILE" != "p1_residual_proposal_observer" \
                   ...
                   && "$ABLATION_PROFILE" != "p2_occupancy_topk_observer" \
                   && "$ABLATION_PROFILE" != "p2v2_local_component_mask_rgbd_observer" \
                   && "$ABLATION_PROFILE" != "p2v3_reliability_geometry_fusion_observer" \
                   || -s "$diagnostic" ) ]]; then
        ...
        if [[ -n "$P2_OCCUPANCY_CHECKPOINT" ]]; then
            optional_args+=(
                --p2-occupancy-checkpoint "$P2_OCCUPANCY_CHECKPOINT"
            )
        fi
...
if [[ "$ABLATION_PROFILE" == "p1_residual_proposal_observer" ]]; then
    ...
    "$PYTHON" "$ROOT/tools/validate_p1g_run_artifacts.py" \
        --scene-list "$META" \
        --prediction-root "$PRED_ROOT" \
        --diagnostics-root "$DIAGNOSTICS_ROOT" \
        --expected-p1s-checkpoint "$P1_RESIDUAL_CHECKPOINT"
elif [[ "$ABLATION_PROFILE" == "p2_occupancy_topk_observer" ]]; then
    "$PYTHON" "$ROOT/tools/validate_p2_run_artifacts.py" \
        --scene-list "$META" \
        --prediction-root "$PRED_ROOT" \
        --diagnostics-root "$DIAGNOSTICS_ROOT" \
        --expected-p1-checkpoint "$P1_RESIDUAL_CHECKPOINT" \
        --expected-p2-checkpoint "$P2_OCCUPANCY_CHECKPOINT"
elif [[ "$ABLATION_PROFILE" == "p2v2_local_component_mask_rgbd_observer" ]]; then
    "$PYTHON" "$ROOT/tools/validate_p2v2_run_artifacts.py" \
        --scene-list "$META" \
        --prediction-root "$PRED_ROOT" \
        --diagnostics-root "$DIAGNOSTICS_ROOT" \
        --expected-p1-checkpoint "$P1_RESIDUAL_CHECKPOINT" \
