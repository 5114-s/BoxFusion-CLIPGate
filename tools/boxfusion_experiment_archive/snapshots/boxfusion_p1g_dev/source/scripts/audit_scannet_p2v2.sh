...
# Validate and report a completed observer-only P2-v2 run.  Ground truth is
...
RUN_TAG="${BOXFUSION_P2V2_RUN_TAG:-${BOXFUSION_P_RUN_TAG:-p2v2_${SCOPE}_b6frozen_v1}}"
PRED_ROOT="${BOXFUSION_P2V2_PRED_ROOT:-${BOXFUSION_P_PRED_ROOT:-$ROOT/results/p_ablation/$RUN_TAG}}"
DIAGNOSTICS_ROOT="${BOXFUSION_P2V2_DIAGNOSTICS_ROOT:-${BOXFUSION_P_DIAGNOSTICS_ROOT:-$ROOT/diagnostics/p_ablation/$RUN_TAG}}"
REPORT_ROOT="${BOXFUSION_P2V2_REPORT_ROOT:-${BOXFUSION_P_REPORT_ROOT:-$ROOT/reports/p_ablation/$RUN_TAG}}"
...
GT_ROOT="${BOXFUSION_P2V2_GT_ROOT:-/data/ZhaoX/BoxFusion/evaluation/data_util/scannet_train_detection_data}"
SCANS_ROOT="${BOXFUSION_P2V2_SCANS_ROOT:-/extra/ZhaoX/scannet_data/scans}"
MIN_R25_PP="${BOXFUSION_P2V2_MIN_DELTA_R25_PP:-3.0}"
MIN_R50_PP="${BOXFUSION_P2V2_MIN_DELTA_R50_PP:-1.0}"
REQUIRE_GO="${BOXFUSION_P2V2_REQUIRE_GO:-0}"
ENV_ROOT="${BOXFUSION_ENV_ROOT:-${CONDA_PREFIX:-/home/admin1/miniconda3/envs/boxfusion2}}"
PYTHON="$ENV_ROOT/bin/python"

case "$REQUIRE_GO" in
    0|1) ;;
    *) echo "BOXFUSION_P2V2_REQUIRE_GO must be 0 or 1" >&2; exit 2 ;;
esac
...
for path in \
    ...
    "$P1_CHECKPOINT" \
    "$P2_CHECKPOINT" \
    "$ROOT/tools/validate_p2v2_run_artifacts.py" \
    "$ROOT/tools/report_p2v2_local_geometry_recall.py"; do
    if [[ ! -f "$path" ]]; then
        echo "Missing P2-v2 audit input: $path" >&2
        exit 1
    fi
done
for directory in \
    ...
    "$SCANS_ROOT"; do
    if [[ ! -d "$directory" ]]; then
        echo "Missing P2-v2 audit directory: $directory" >&2
        exit 1
    fi
done

mkdir -p "$REPORT_ROOT"
echo "P2-v2 audit: scope=$SCOPE, tag=$RUN_TAG"
...
"$PYTHON" "$ROOT/tools/report_p2v2_local_geometry_recall.py" \
    --scene-list "$SCENE_LIST" \
    --prediction-root "$PRED_ROOT" \
    --diagnostics-root "$DIAGNOSTICS_ROOT" \
    --gt-root "$GT_ROOT" \
...
"$PYTHON" -c \
    'import json,sys; r=json.load(open(sys.argv[1], encoding="utf-8")); g=r["go_no_go"]; print("P2-v2 decision:", g["decision"]); print("  delta R@0.25: %.4f pp" % g["observed_delta_recall_at_025_percentage_points"]); print("  delta R@0.50: %.4f pp" % g["observed_delta_recall_at_050_percentage_points"]); raise SystemExit(0 if g["passed"] or sys.argv[2] == "0" else 3)' \
    "$REPORT_ROOT/recall.json" "$REQUIRE_GO"

echo "P2-v2 audit completed"
