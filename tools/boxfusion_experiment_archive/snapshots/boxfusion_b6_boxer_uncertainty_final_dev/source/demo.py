    ...
    for sample in dataset:
        ...
        if count == len(dataset)-1 or (count+gap)>len(dataset)-1:
            ...
            if cfg['data']['output_dir'] is not None and cfg["eval"]:
                frozen_global_xyzlwh = (
                    all_pred_box.pred_boxes_3d.tensor.detach()
                    .cpu()
                    .numpy()
                    .copy()
                )
                frozen_global_rotations = (
                    all_pred_box.pred_boxes_3d.R.detach()
                    .cpu()
                    .numpy()
                    .copy()
                )
                frozen_fusion_groups = tuple(
                ...
                if online_refiner.enabled:
                    ...
                    final_source_indices = (
                        refinement_result.source_indices.copy()
                    )
                    final_stable_ids = refinement_result.stable_ids.copy()
                    frozen_global_stable_ids = stable_ids.copy()
                    ...
                    scores = all_pred_box.scores.detach().cpu().numpy()
                    frozen_global_stable_ids = resolve_fusion_stable_ids(
                        box_manager.fusion_list
                    )
                    final_source_indices = np.arange(
                        boxes_3d.shape[0], dtype=np.int64
                    )
                    final_stable_ids = frozen_global_stable_ids.copy()
                minimum_extent = 0.0
                if cfg['dataset'] == 'scannet':
                    ...
                    final_source_indices = final_source_indices[valid_mask]
                    final_stable_ids = final_stable_ids[valid_mask]

                if (
                    Box_Fuser.final_boxer_uncertainty_cfg["mode"]
                    != "disabled"
                ):
                    protected_scores = scores.copy()
                    protected_sources = final_source_indices.copy()
                    protected_ids = final_stable_ids.copy()
                    boxes_3d = Box_Fuser.apply_final_boxer_uncertainty(
                        baseline_corners=boxes_3d,
                        scores=scores,
                        source_indices=final_source_indices,
                        stable_ids=final_stable_ids,
                        global_xyzlwh=frozen_global_xyzlwh,
                        global_rotations=frozen_global_rotations,
                        global_stable_ids=frozen_global_stable_ids,
                        frozen_fusion_groups=frozen_fusion_groups,
                        minimum_extent=minimum_extent,
                    )
                    if not np.array_equal(scores, protected_scores):
                        raise RuntimeError(
                            "Final Boxer uncertainty changed B6 scores"
                        )
                    if not np.array_equal(
                        final_source_indices, protected_sources
                    ):
                        raise RuntimeError(
                            "Final Boxer uncertainty changed source order"
                        )
                    if not np.array_equal(final_stable_ids, protected_ids):
                        raise RuntimeError(
                            "Final Boxer uncertainty changed stable-ID order"
                        )
            ...
            # Persist the fusion diagnostic only after the prediction export
            # (and optional cache finalization) has succeeded.  This avoids an
            # orphan diagnostic when a later finalization step fails.
            if Box_Fuser.boxer_uncertainty_cfg["mode"] != "disabled":
                print(Box_Fuser.boxer_uncertainty_summary())
            if (
                Box_Fuser.final_boxer_uncertainty_cfg["mode"]
                != "disabled"
            ):
                print(Box_Fuser.final_boxer_uncertainty_summary())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--boxer-uncertainty-mode", choices=["disabled", "observer", "active"], default=None, help="Override Boxer uncertainty-aware reliable-view fusion mode")
    parser.add_argument("--boxer-uncertainty-diagnostics-root", default=None, type=str, help="Override Boxer uncertainty-fusion diagnostics root")
    parser.add_argument("--boxer-final-uncertainty-mode", choices=["disabled", "observer", "active"], default=None, help="Override post-B6 fixed-Top-K Boxer uncertainty mode")
    parser.add_argument("--boxer-final-uncertainty-diagnostics-root", default=None, type=str, help="Override post-B6 Boxer uncertainty diagnostics root")
    ...
    if dataset_path.lower() in ["scannet", "ca1m", 'online']:
        ...
        if (
            args.boxer_uncertainty_mode is not None
            or args.boxer_uncertainty_diagnostics_root is not None
        ):
            lifting_cfg = cfg.setdefault("lifting", {})
            boxer_cfg = lifting_cfg.setdefault("boxer", {})
            reliable_cfg = cfg.setdefault("box_fusion", {}).setdefault(
                "reliable_views", {}
            )
            uncertainty_cfg = reliable_cfg.setdefault(
                "boxer_uncertainty", {}
            )
            if args.boxer_uncertainty_mode is not None:
                uncertainty_cfg["mode"] = args.boxer_uncertainty_mode
            if args.boxer_uncertainty_diagnostics_root is not None:
                uncertainty_cfg["diagnostics_dir"] = (
                    args.boxer_uncertainty_diagnostics_root
                )
            if uncertainty_cfg.get("mode", "disabled") != "disabled":
                if lifting_cfg.get("backend") != "boxer":
                    parser.error(
                        "Boxer uncertainty fusion requires "
                        "lifting.backend=boxer"
                    )
                if boxer_cfg.get("mode") != "active":
                    parser.error(
                        "Boxer uncertainty fusion requires active Boxer "
                        "lifting; observer applies only to the fusion module"
                    )
                if not boxer_cfg.get("selective_gate", {}).get(
                    "enabled", False
                ):
                    parser.error(
                        "Boxer uncertainty fusion requires the frozen "
                        "Selective Boxer gate"
                    )
                if not reliable_cfg.get("enabled", False):
                    parser.error(
                        "Boxer uncertainty fusion requires reliable views"
                    )
                if not uncertainty_cfg.get("diagnostics_dir"):
                    parser.error(
                        "Boxer uncertainty observer/active mode requires a "
                        "diagnostics root"
                    )
            print(
                "Boxer uncertainty fusion override:",
                uncertainty_cfg.get("mode", "disabled"),
            )
        if (
            args.boxer_final_uncertainty_mode is not None
            or args.boxer_final_uncertainty_diagnostics_root is not None
        ):
            lifting_cfg = cfg.setdefault("lifting", {})
            boxer_cfg = lifting_cfg.setdefault("boxer", {})
            reliable_cfg = cfg.setdefault("box_fusion", {}).setdefault(
            ...
            final_cfg = reliable_cfg.setdefault(
                "final_boxer_uncertainty", {}
            )
            if args.boxer_final_uncertainty_mode is not None:
                final_cfg["mode"] = args.boxer_final_uncertainty_mode
            if args.boxer_final_uncertainty_diagnostics_root is not None:
                final_cfg["diagnostics_dir"] = (
                    args.boxer_final_uncertainty_diagnostics_root
                )
            if final_cfg.get("mode", "disabled") != "disabled":
                if lifting_cfg.get("backend") != "boxer":
                    parser.error(
                        "Final Boxer uncertainty requires "
                        "lifting.backend=boxer"
                    )
                if boxer_cfg.get("mode") != "active":
                    parser.error(
                        "Final Boxer uncertainty requires active Boxer "
                        "lifting"
                    )
                if not boxer_cfg.get("selective_gate", {}).get(
                    "enabled", False
                    ...
                    parser.error(
                        "Final Boxer uncertainty requires the frozen "
                        "Selective Boxer gate"
                    )
                if not reliable_cfg.get("enabled", False):
                    parser.error(
                        "Final Boxer uncertainty requires reliable views"
                    )
                if reliable_cfg.get("boxer_uncertainty", {}).get(
                    "mode", "disabled"
                ) != "disabled":
                    parser.error(
                        "Final-only and online Boxer uncertainty modes are "
                        "mutually exclusive"
                    )
                if not final_cfg.get("diagnostics_dir"):
                    parser.error(
                        "Final Boxer uncertainty observer/active mode "
                        "requires a diagnostics root"
                    )
                if not online_cfg.get("enabled", False):
                    parser.error(
                        "Final Boxer uncertainty requires B6 online "
                        "refinement to be enabled"
                    )
            print(
                "Final-only Boxer uncertainty override:",
                final_cfg.get("mode", "disabled"),
            )
