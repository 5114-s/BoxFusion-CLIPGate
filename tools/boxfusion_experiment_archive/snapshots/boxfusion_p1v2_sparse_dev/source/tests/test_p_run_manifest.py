"""Immutable-manifest contracts for the isolated P-stage runner."""
...
def _root(tmp_path: Path) -> Path:
    ...
    for relative in (
        ...
        "boxfusion/p2_local_mask_geometry.py",
        "boxfusion/p2_reliability_fusion.py",
...
def test_p2v2_manifest_binds_frozen_p1_p2_chain_and_local_code(tmp_path):
    args = _args(tmp_path, stage="P2V2")
    payload = _MODULE.build_manifest(args)

    assert payload["stage"] == "P2V2"
    ...
    assert "boxfusion/p2_local_mask_geometry.py" in payload["code_sha256"]


def test_p2v3_manifest_binds_same_heads_and_reliability_code(tmp_path):
    args = _args(tmp_path, stage="P2V3")
    payload = _MODULE.build_manifest(args)

    assert payload["stage"] == "P2V3"
    assert payload["profile"] == (
    ...
    assert "boxfusion/p2_local_mask_geometry.py" in payload["code_sha256"]
    assert "boxfusion/p2_reliability_fusion.py" in payload["code_sha256"]
