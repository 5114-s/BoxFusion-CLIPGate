...
class OccupancyTopKObservation:
    ...
    eligible_voxels: int
    selected_voxels: int
    selected_voxel_indices: np.ndarray
    selected_voxel_scores: np.ndarray
    occupancy_seconds: float

    def __post_init__(self) -> None:
        indices = np.array(
            self.selected_voxel_indices, dtype=np.int64, copy=True
        ).reshape(-1)
        scores = np.array(
            self.selected_voxel_scores, dtype=np.float32, copy=True
        ).reshape(-1)
        if len(indices) != int(self.selected_voxels):
            raise ValueError(
                "selected_voxel_indices disagree with selected_voxels"
            )
        if len(scores) != len(indices):
            raise ValueError(
                "selected_voxel_scores must align with selected indices"
            )
        if (
            np.any(indices < 0)
            or np.any(indices >= len(self.base.voxel_batch.features))
        ):
            raise ValueError("selected voxel index is out of range")
        if (
            not np.isfinite(scores).all()
            or np.any(scores < 0.0)
            or np.any(scores > 1.0)
        ):
            raise ValueError("selected voxel scores must lie in [0,1]")
        indices.setflags(write=False)
        scores.setflags(write=False)
        object.__setattr__(self, "selected_voxel_indices", indices)
        object.__setattr__(self, "selected_voxel_scores", scores)
...
class P2OccupancyTopKObserver(P1ResidualProposalObserver):
    ...
    def observe(self, **kwargs: Any) -> ResidualObservation:
        ...
        p2_observation = OccupancyTopKObservation(
            ...
            selected_voxel_scores=probabilities[
