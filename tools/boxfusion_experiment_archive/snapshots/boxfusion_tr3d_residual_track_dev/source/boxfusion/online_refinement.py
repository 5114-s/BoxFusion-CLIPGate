...
class OnlineRefinementController:
    ...
    def reset_scene(self, scene_id: str) -> None:
        ...
        if self.proposal_observer is not None:
            self.proposal_observer.reset_scene(self.scene_id)
    ...
    def process_keyframe(
        ...
        *,
        ...
        if self.proposal_observer is not None:
            # A read-only tap after provider/appearance processing and before
            # lifting.  The observer receives the same runtime masks, metric
            # depth, intrinsics, and pose as the active online controller.
            self.proposal_observer.observe_keyframe(
                scene_id=self.scene_id,
                frame_id=(
                    int(cache_frame_id)
                    if cache_frame_id is not None
                    and cache_frame_id.isdigit()
                    else int(frame_id)
                ),
