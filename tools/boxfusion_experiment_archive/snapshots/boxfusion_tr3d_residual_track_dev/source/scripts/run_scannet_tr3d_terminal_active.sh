...
if [[ "$C3_ONLINE_ENABLED" == "1" ]]; then
    for immutable_root in \
        "$C3_ONLINE_C2_CACHE_ROOT" \
        "$C3_ONLINE_PARENT_CACHE_ROOT"; do
...
run_worker() {
    ...
    while IFS= read -r scene || [[ -n "$scene" ]]; do
        ...
        for immutable_scene_artifact in \
            "$r3_parent_cache" \
            "$R3_FROZEN_G0_ROOT/${scene}_boxes.pkl" \
            "$R3_SHADOW_GOLD_ROOT/${scene}_boxes.pkl"; do
...
PY
        ...
        if [[ "$C3_ONLINE_ENABLED" == "1" ]]; then
            optional_args+=(
                --tr3d-c3-online-c2-cache-root "$C3_ONLINE_C2_CACHE_ROOT"
        ...
        local summary
        summary="$(
            grep -E 'Online refinement summary|Boxer lifting summary|TR3D terminal active summary|C3 online identity summary' "$scene_log" \
                | tail -n 4 \
                | tr '\n' ' ' \
                || true
        )"
...
if [[ "$expected_scene_count" -ne "$total" \
      ...
      || "$c3_online_diagnostic_count" -ne "$total" ]]; then
    echo "Paired artifact count mismatch: expected=$total active=$active_prediction_count baseline=$baseline_prediction_count active_markers=$active_marker_count baseline_markers=$baseline_marker_count r3_diagnostics=$r3_diagnostic_count c3_online_diagnostics=$c3_online_diagnostic_count" >&2
    exit 1
fi
if [[ -n "$BOXER_DIAGNOSTICS_ROOT" ]]; then
