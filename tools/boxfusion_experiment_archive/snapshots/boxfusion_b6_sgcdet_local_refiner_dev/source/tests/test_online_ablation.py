...
@pytest.mark.parametrize(
    "profile,enabled,mutate_geometry",
    [
        ("sgcdet_sparse_observer", False, False),
        ("sgcdet_sparse_identity", True, False),
        ("sgcdet_sparse_active", True, True),
    ],
)
def test_sgcdet_sparse_profiles_freeze_b6_and_isolate_geometry(
    profile, enabled, mutate_geometry
):
    source = complete_config()
    original = deepcopy(source)
    ...
    sparse = online["sgcdet_sparse_refiner"]
    ...
    # B6 remains the only scoring path; the new module can change geometry
    # only in the active profile.
    assert online["refit"]["enabled"] is False
    assert online["box_refiner"]["enabled"] is False
    ...
    assert sparse["enabled"] is enabled
    assert sparse["mutate_geometry"] is mutate_geometry
    assert sparse["collect_diagnostics"] is True
    assert sparse["max_views"] == 5
    assert sparse["points_per_view"] == 128
    assert sparse["improvement_threshold"] == 0.55
    assert sparse["max_candidate_uncertainty"] == 0.50
