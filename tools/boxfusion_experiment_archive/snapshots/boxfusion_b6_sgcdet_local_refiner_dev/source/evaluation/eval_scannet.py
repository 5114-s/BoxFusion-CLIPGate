...
# Resolve evaluator-local modules from this file's directory, not from the
# caller's working directory.  The evaluation utilities are vendored in this
# isolated branch so a run never falls back to a sibling experiment tree.
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
UTILS_DIR = os.path.join(BASE_DIR, 'utils')
# Put the package parent first so ``utils`` resolves to the vendored package,
# while retaining the utility directory for its legacy intra-module imports
# (for example ``from eval_det import ...``).
for local_dir in (UTILS_DIR, BASE_DIR):
...
# Optionally evaluate only a fixed scene subset.
if FLAGS.scene_name != 'None' and FLAGS.scene_list is not None:
    raise ValueError('--scene_name and --scene_list are mutually exclusive')
if FLAGS.scene_list is not None:
    if not os.path.isfile(FLAGS.scene_list):
        raise FileNotFoundError(FLAGS.scene_list)
    with open(FLAGS.scene_list, 'r') as scene_file:
        requested_scenes = [
            line.strip()
    ...
    available_scenes = set(TEST_DATASET.scan_names)
    missing_scenes = [
        scene for scene in requested_scenes if scene not in available_scenes
    ]
    if missing_scenes:
        raise ValueError(
    ...
    TEST_DATASET.scan_names = requested_scenes
