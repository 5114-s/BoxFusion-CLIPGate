...
# Strict B3-v2 + B6 ablation. B6 uses the same zero detector blend and the
# same original-geometry feature distribution as the validated B6-only run;
# only the exported geometry differs.
#
# Usage:
...
RUN_TAG="${BOXFUSION_B3V2_RUN_TAG:-b3v2_oriented_pair_b6_blend000_extent040_ablation10}"
...
export BOXFUSION_ONLINE_ABLATION_PROFILE="b3v2_b6"
export BOXFUSION_QUALITY_MODE="iou_mlp"
export BOXFUSION_QUALITY_CHECKPOINT="$CHECKPOINT"
export BOXFUSION_QUALITY_DETECTOR_BLEND="${BOXFUSION_B3V2_B6_DETECTOR_BLEND:-0.0}"
...
echo "B3-v2+B6: tag=$RUN_TAG, scenes=$SCENE_LIST, GPUs=$GPU_SPEC"
