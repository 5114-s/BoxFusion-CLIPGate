...
_SGCDET_SPARSE_PROFILE_DEFAULTS = {
    "enabled": False,
    "checkpoint": None,
    "device": None,
    "max_views": 5,
...
def _validate_config(config: Mapping[str, Any]) -> None:
    ...
    sparse = online.get("sgcdet_sparse_refiner")
    if sparse is not None:
        if not isinstance(sparse, Mapping):
            raise TypeError(
                "online_refinement.sgcdet_sparse_refiner must be a mapping"
            )
        _validate_enabled(
            sparse, "online_refinement.sgcdet_sparse_refiner"
        )
...
def apply_online_ablation_profile(
    ...
    ``sgcdet_sparse_observer``
        Build the exact K=5 object-local sparse-volume inputs and diagnostics
        while preserving the frozen B6 output byte-for-byte.
    ``sgcdet_sparse_identity``
        Run a strictly loaded sparse-refiner checkpoint but forbid geometry
        mutation. This proves inference and diagnostics are output identity.
    ``sgcdet_sparse_active``
        Add only the gated SGCDet-inspired local sparse geometry refiner to
        frozen B6 ranking. Scores, count, order and stable IDs stay fixed.
    ...
    sparse = online.get("sgcdet_sparse_refiner")
    if sparse is not None and not isinstance(sparse, MutableMapping):
        sparse = dict(sparse)
        online["sgcdet_sparse_refiner"] = sparse
    if profile in sparse_profiles and sparse is None:
        sparse = deepcopy(_SGCDET_SPARSE_PROFILE_DEFAULTS)
        online["sgcdet_sparse_refiner"] = sparse
    if sparse is not None:
        sparse["enabled"] = profile in {
            "sgcdet_sparse_identity",
            "sgcdet_sparse_active",
        }
        sparse["mutate_geometry"] = profile == "sgcdet_sparse_active"
        sparse["collect_diagnostics"] = profile in sparse_profiles
    ...
    local_neural_profiles = b5v2_profiles | joint_profiles | sparse_profiles
    ...
    if (
        profile == "b5v2_memory_observer"
        or profile in joint_profiles
        or profile in sparse_profiles
    ):
    ...
    if profile == "b5v2_b6" or profile in sparse_profiles:
