...
SUPPLEMENTAL_CACHE_DIRECTORY="${BOXFUSION_SUPPLEMENTAL_CACHE_DIRECTORY:-}"
...
if [[ -n "$PROPOSAL_CACHE_MODE_OVERRIDE" \
      && "$PROPOSAL_CACHE_MODE_OVERRIDE" != "disabled" ]]; then
    echo "BOXFUSION_PROPOSAL_CACHE_MODE_OVERRIDE may only be disabled" >&2
    exit 1
fi
...
if [[ "$SKIP_EVALUATION" == "0" && ! -d "$GT_ROOT" ]]; then
...
CODE_FINGERPRINT="$(
    {
        sha256sum \
            ...
            "$ROOT/boxfusion/object_memory.py" \
            "$ROOT/boxfusion/sgcdet_local_sparse_refiner.py" \
        ...
        if [[ -n "$SGCDET_SPARSE_CHECKPOINT" ]]; then
            sha256sum "$SGCDET_SPARSE_CHECKPOINT"
        fi
        printf '%s\n' \
            ...
            "supplemental_cache_directory=$SUPPLEMENTAL_CACHE_DIRECTORY" \
...
if [[ "$CACHE_MODE" == "disabled" ]]; then
    echo "[$(date '+%F %T')] Proposal cache: disabled"
else
    echo "[$(date '+%F %T')] Proposal cache: $CACHE_MODE:$CACHE_NAMESPACE"
fi
echo "[$(date '+%F %T')] Supplemental cache override: ${SUPPLEMENTAL_CACHE_DIRECTORY:-config-default}"
echo "[$(date '+%F %T')] Skip evaluation: $SKIP_EVALUATION"
...
PY
        ...
        if [[ -n "$PROPOSAL_CACHE_MODE_OVERRIDE" ]]; then
            optional_args+=(
                --proposal-cache-mode "$PROPOSAL_CACHE_MODE_OVERRIDE"
            )
        fi
        if [[ -n "$SUPPLEMENTAL_CACHE_DIRECTORY" ]]; then
            optional_args+=(
                --online-supplemental-cache-directory \
                "$SUPPLEMENTAL_CACHE_DIRECTORY"
            )
        fi
        if [[ -n "$BOXER_DIAGNOSTICS_ROOT" ]]; then
            optional_args+=(
...
if [[ "$SKIP_EVALUATION" == "1" ]]; then
    echo "[$(date '+%F %T')] Completed train-only diagnostic collection; ScanNet evaluation was explicitly skipped"
    exit 0
fi
