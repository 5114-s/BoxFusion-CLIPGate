...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Override lifting.proposal_cache.mode. This is used by the "
            "train-only G0 collector, whose scenes are absent from the "
            "frozen validation replay cache."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Override the YOLOE supplemental-proposal cache directory",
    )
    ...
    parser.add_argument("--online-sgcdet-sparse-checkpoint", default=None, type=str, help="Load the SGCDet-inspired object-local sparse geometry refiner")
    ...
    if dataset_path.lower() in ["scannet", "ca1m", 'online']:
        ...
        if args.proposal_cache_mode is not None:
            ...
            print("Proposal cache mode override:", args.proposal_cache_mode)
        boxer_gate_values = (
        ...
        if args.online_supplemental_cache_directory is not None:
            online_cfg.setdefault("supplemental_proposals", {}).setdefault(
                "cache", {}
            )["directory"] = args.online_supplemental_cache_directory
        ...
        if args.online_sgcdet_sparse_checkpoint is not None:
            if (
                args.online_refiner_checkpoint is not None
                or args.online_joint_checkpoint is not None
            ):
                parser.error(
                    "--online-sgcdet-sparse-checkpoint is mutually exclusive "
                    "with legacy/joint geometry checkpoints"
                )
            sparse_cfg = online_cfg.setdefault(
                "sgcdet_sparse_refiner", {}
            )
            sparse_cfg["enabled"] = True
            sparse_cfg["checkpoint"] = (
                args.online_sgcdet_sparse_checkpoint
            )
            # Profile application below is the only place that may enable
            # mutation. A checkpoint without an active profile is identity.
            sparse_cfg["mutate_geometry"] = False
            sparse_cfg["collect_diagnostics"] = True
