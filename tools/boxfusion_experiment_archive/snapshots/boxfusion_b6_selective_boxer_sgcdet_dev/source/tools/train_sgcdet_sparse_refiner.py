...
def train_sgcdet_sparse_refiner(
    ...
    *,
    ...
    minimum_validation_tp50_proxy: float = float("-inf"),
    minimum_validation_cross_success: int = 0,
    maximum_validation_drop50_rate: float = 1.0,
    cross_oversample_factor: int = 1,
    patience: int = 0,
    collection_manifest_path: str | os.PathLike[str] | None = None,
    **loss_weights: float,
) -> Dict[str, Any]:
    dataset = Path(dataset_path)
    dataset_sha256 = _sha256_file(dataset)
    collection_manifest_sha256: str | None = None
    collection_manifest_schema: str | None = None
    if collection_manifest_path is not None:
        manifest_path = Path(collection_manifest_path)
        if not manifest_path.is_file():
            raise FileNotFoundError(manifest_path)
        collection_manifest_sha256 = _sha256_file(manifest_path)
        with manifest_path.open("r", encoding="utf-8") as handle:
            manifest = json.load(handle)
        collection_manifest_schema = manifest.get("schema")
        if (
            collection_manifest_schema
            != "boxfusion.g0_sgcdet_train_collection_manifest.v1"
        ):
    ...
    for name, value in (
        ...
        ("seed", seed),
        ("minimum_validation_cross_success", minimum_validation_cross_success),
        ("cross_oversample_factor", cross_oversample_factor),
        ("patience", patience),
    ):
    ...
    if minimum_validation_cross_success < 0:
        raise ValueError("minimum_validation_cross_success must be non-negative")
    if cross_oversample_factor < 1:
        raise ValueError("cross_oversample_factor must be at least one")
    if patience < 0:
        raise ValueError("patience must be non-negative")
    if selection_metric not in {"loss", "ap50_proxy"}:
        raise ValueError("selection_metric must be loss or ap50_proxy")
    if (
        not np.isscalar(minimum_validation_tp50_proxy)
        or np.isnan(float(minimum_validation_tp50_proxy))
    ):
        raise ValueError("minimum_validation_tp50_proxy must not be NaN")
    if (
        not np.isscalar(maximum_validation_drop50_rate)
        or not np.isfinite(maximum_validation_drop50_rate)
        or not 0.0 <= float(maximum_validation_drop50_rate) <= 1.0
    ):
        raise ValueError("maximum_validation_drop50_rate must lie in [0,1]")
    ...
    if not identity_only:
        ...
        selection: Optional[tuple[float, ...]] = None
        stale_epochs = 0
        for epoch in range(epochs):
            ...
            if cross_oversample_factor > 1:
                cross_indices = train_indices[data.cross_iou50[train_indices]]
                if len(cross_indices):
                    extras = np.tile(
                        cross_indices, cross_oversample_factor - 1
                    )
                    epoch_indices = np.concatenate(
                        (epoch_indices, extras.astype(np.int64, copy=False))
                    )
                    generator = np.random.default_rng(seed + 100000 + epoch)
            ...
            validation_proxy = float(
                current_validation["local_net_tp50_proxy"]
            )
            cross_success = int(
                round(current_validation.get("cross50_success_count", 0.0))
            )
            eligible = float(
                current_validation.get("eligible_matched_count", 0.0)
            )
            drop_rate = (
                float(current_validation.get("drop50_count", 0.0)) / eligible
                if eligible > 0.0
                else 1.0
            )
            qualified = bool(
                validation_proxy >= float(minimum_validation_tp50_proxy)
                and cross_success >= minimum_validation_cross_success
                and drop_rate <= float(maximum_validation_drop50_rate)
            )
            if selection_metric == "ap50_proxy":
                candidate = (
                    -validation_proxy,
                    drop_rate,
            ...
            epoch_history.append(
                {
                    "epoch": int(epoch),
                    "validation_loss": float(current_validation["loss"]),
                    "validation_tp50_proxy": validation_proxy,
                    "validation_cross50_success_count": cross_success,
                    "validation_drop50_rate": drop_rate,
                    "qualified": qualified,
                }
            )
            if qualified and (selection is None or candidate < selection):
                selection = candidate
                best_epoch = epoch
                best_validation_loss = float(current_validation["loss"])
                ...
                stale_epochs += 1
            if patience > 0 and best_state is not None and stale_epochs >= patience:
                break
        if best_state is None:
            raise RuntimeError(
                "training produced no AP50-qualified checkpoint; active "
    ...
    if (
        collection_manifest_path is not None
        and collection_manifest_sha256
        != _sha256_file(Path(collection_manifest_path))
    ):
        raise RuntimeError("collection manifest changed during training")
    metadata: Dict[str, Any] = {
        ...
        "collection_manifest_schema": collection_manifest_schema,
        ...
        "minimum_validation_tp50_proxy": float(
            minimum_validation_tp50_proxy
        ),
        "minimum_validation_cross_success": int(
            minimum_validation_cross_success
        ),
        "maximum_validation_drop50_rate": float(
            maximum_validation_drop50_rate
        ),
        "cross_oversample_factor": int(cross_oversample_factor),
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--selection-metric",
        choices=["loss", "ap50_proxy"],
        default="loss",
    )
    parser.add_argument(
        "--minimum-validation-tp50-proxy",
        type=float,
        default=float("-inf"),
    )
    parser.add_argument(
        "--minimum-validation-cross-success", type=int, default=0
    )
    parser.add_argument(
        "--maximum-validation-drop50-rate", type=float, default=1.0
    )
    parser.add_argument("--cross-oversample-factor", type=int, default=1)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    result = train_sgcdet_sparse_refiner(
        ...
        minimum_validation_tp50_proxy=(
            args.minimum_validation_tp50_proxy
        ),
        minimum_validation_cross_success=(
            args.minimum_validation_cross_success
        ),
        maximum_validation_drop50_rate=(
            args.maximum_validation_drop50_rate
        ),
        cross_oversample_factor=args.cross_oversample_factor,
