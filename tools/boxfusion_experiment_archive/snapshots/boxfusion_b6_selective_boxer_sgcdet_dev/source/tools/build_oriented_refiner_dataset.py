...
# Strict, named contracts only.  The default preserves every historical B5
# dataset byte, while the G0 contract accepts diagnostics collected in the
# exact B6 + Selective-Boxer distribution used by the combined runtime.
STRICT_PROVENANCE_PROFILES = {
    "b5v2_memory_observer": dict(_STRICT_PROVENANCE_EXPECTED),
...
def strict_provenance_for_profile(profile: str) -> dict[str, object]:
    """Return one explicit runtime-provenance contract, never a wildcard."""

    if profile not in STRICT_PROVENANCE_PROFILES:
        raise ValueError(
            "strict_provenance_profile must be one of "
            f"{tuple(STRICT_PROVENANCE_PROFILES)}"
        )
    return dict(STRICT_PROVENANCE_PROFILES[profile])
...
class BuildConfig:
    ...
    strict_provenance_profile: str = "b5v2_memory_observer"
    ...
    def validated(self) -> "BuildConfig":
        ...
        expected_provenance = strict_provenance_for_profile(
            self.strict_provenance_profile
        )
        if strict_schema:
...
def build_oriented_refiner_dataset(config: BuildConfig) -> BuildSummary:
    ...
    for scene_id in scenes:
        diagnostics = load_scene_diagnostics(
            ...
            strict_provenance_profile=config.strict_provenance_profile,
        )
    ...
    if strict_schema:
        expected_provenance = strict_provenance_for_profile(
            config.strict_provenance_profile
        )
        provenance_metadata = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--strict-provenance-profile",
        choices=tuple(STRICT_PROVENANCE_PROFILES),
        default="b5v2_memory_observer",
        help=(
            "exact runtime contract encoded by every diagnostic; no "
            "wildcard/relaxed provenance is permitted"
        ),
    )
...
def main() -> int:
    ...
    summary = build_oriented_refiner_dataset(
        BuildConfig(
            ...
            strict_provenance_profile=args.strict_provenance_profile,
