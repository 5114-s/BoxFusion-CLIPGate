...
[[ -n "$RUN_TAG" ]] || tr3d_die \
  "provide a unique run tag: $0 0,1 tr3d_fg_full_v1"
tr3d_require_tag "$RUN_TAG"
tr3d_parse_gpus "$GPU_SPEC"
tr3d_select_env "${BOXFUSION_TR3D_ENV:-$ROOT/.conda/boxfusion-tr3d}"

if [[ -n "${BOXFUSION_TR3D_CONFIG:-}" ]]; then
  CONFIG="$BOXFUSION_TR3D_CONFIG"
  CUSTOM_CONFIG=1
else
  CONFIG="$ROOT/config/tr3d/tr3d_scannet_foreground_from_official_init.py"
  CUSTOM_CONFIG=0
fi
...
if [[ -n "${BOXFUSION_TR3D_BATCH_PER_GPU:-}" ]]; then
  ...
  (( 16 % ${#TR3D_GPUS[@]} == 0 )) || tr3d_die \
    "official global batch 16 is not divisible by world size; set BOXFUSION_TR3D_BATCH_PER_GPU"
  BATCH_SIZE="$((16 / ${#TR3D_GPUS[@]}))"
fi
NUM_WORKERS="${BOXFUSION_TR3D_NUM_WORKERS:-4}"
PORT="${BOXFUSION_TR3D_PORT:-$((24000 + $$ % 10000))}"
AMP="${BOXFUSION_TR3D_AMP:-1}"
if [[ "$CUSTOM_CONFIG" == "0" ]]; then
  INIT_CHECKPOINT="${BOXFUSION_TR3D_INIT_CHECKPOINT:-$ROOT/models/tr3d_1xb16_scannet-3d-foreground-init.pth}"
else
  INIT_CHECKPOINT="${BOXFUSION_TR3D_INIT_CHECKPOINT:-}"
fi
...
if [[ "$RESUME" == "0" && -n "$INIT_CHECKPOINT" ]]; then
...
if [[ "$RESUME" == "0" && "$CUSTOM_CONFIG" == "0" ]]; then
...
resume_args=()
[[ "$RESUME" == "1" ]] && resume_args=(--resume auto)
amp_args=()
[[ "$AMP" == "1" ]] && amp_args=(--amp)
cfg_options=(
...
if [[ "$RESUME" == "0" && -n "$INIT_CHECKPOINT" ]]; then
  cfg_options+=("load_from=$INIT_CHECKPOINT")
fi

echo "Genuine TR3D T1 full-scene foreground training"
echo "  run tag: $RUN_TAG"
echo "  GPUs: $TR3D_GPU_SPEC (${#TR3D_GPUS[@]} processes)"
echo "  config: $CONFIG"
echo "  annotation: $ANNOTATION"
echo "  init checkpoint: ${INIT_CHECKPOINT:-none (explicit custom-config scratch)}"
