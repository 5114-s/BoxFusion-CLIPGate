...
def _validate_alignment_provenance(
    ...
    """Validate the cached inverse without a cross-runtime byte hash.

    The immutable cache loader already checks that ``axis_alignment_sha256``
    exactly hashes the matrix stored in the cache.  Recomputing an inverse in
    another NumPy/LAPACK build can differ by a few ULPs, so external
    provenance is the numerical inverse relationship in both directions.
    """
