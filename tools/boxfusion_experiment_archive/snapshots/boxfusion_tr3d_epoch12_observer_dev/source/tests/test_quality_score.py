...
def test_iou_aware_mlp_exposes_monotonic_heads_and_ranking():
    weight = np.zeros((QUALITY_FEATURE_DIM, 4), dtype=np.float32)
    bias = np.asarray([0.0, -1.0, 2.0, 1.0], dtype=np.float32)
    ...
    prediction = scorer.predict(np.zeros(QUALITY_FEATURE_DIM))
    assert set(prediction) == set(IOU_AWARE_OUTPUT_NAMES) | {
        "ranking_score"
    }
    assert prediction["predicted_iou"] == pytest.approx(0.5)
    assert (
        prediction["prob_iou_015"]
        >= prediction["prob_iou_025"]
        >= prediction["prob_iou_050"]
    )
    components = np.asarray(
        [prediction[name] for name in IOU_AWARE_OUTPUT_NAMES]
    )
    assert scorer(np.zeros(QUALITY_FEATURE_DIM)) == pytest.approx(
        float(components @ ranking_weights)
    )


def test_iou_aware_checkpoint_loads_with_strict_versioned_schema(tmp_path):
    path = tmp_path / "iou_mlp.npz"
    np.savez(
        path,
        format_version=np.asarray(1),
        feature_names=np.asarray(QUALITY_FEATURE_NAMES),
        output_names=np.asarray(IOU_AWARE_OUTPUT_NAMES),
        iou_thresholds=np.asarray(IOU_AWARE_THRESHOLDS),
        ranking_weights=np.asarray([0.1, 0.2, 0.3, 0.4]),
        feature_mean=np.zeros(QUALITY_FEATURE_DIM),
        feature_scale=np.ones(QUALITY_FEATURE_DIM),
        num_layers=np.asarray(1),
        weight_0=np.zeros((QUALITY_FEATURE_DIM, 4)),
        bias_0=np.zeros(4),
    )
    loaded = load_quality_scorer(path, method="iou_mlp")
    assert isinstance(loaded, IoUAwareMLPQualityScorer)
    assert loaded(np.ones(QUALITY_FEATURE_DIM)) == pytest.approx(0.5)
