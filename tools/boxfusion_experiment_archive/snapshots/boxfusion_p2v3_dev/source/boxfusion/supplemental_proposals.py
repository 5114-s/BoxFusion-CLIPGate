...
DEFAULT_SUPPLEMENTAL_PROPOSAL_CONFIG: Dict[str, Any] = {
    ...
    "cache": {
        ...
        "directory": None,
        "write": True,
        # ``None`` preserves the historical configuration-derived namespace
        # for YOLOE read-through caches.  Offline teacher caches must provide
        # an explicit namespace so their provenance cannot be confused with
        # a detector cache.
        "namespace": None,
        # Used only by the strict cache-only provider.  It never falls back to
        # an online model when an entry is absent or has the wrong image shape.
...
def proposal_cache_key(
    ...
    """Return the content-addressed key shared by cache writers and readers.

    A logical frame identifier alone is not sufficient: changing
    ``data.start``, RGB channel handling, resizing, or the source extraction
    may otherwise silently reuse proposals for different pixels.  Keeping the
    RGB digest in this public helper lets an offline teacher writer produce
    exactly the same keys as the online replay provider without importing or
    constructing any online detector.
    """

    if not isinstance(namespace, str):
        raise TypeError("proposal cache namespace must be a string")
    identifiers = _validate_frame_ids([frame_id], 1)
    validated = _validate_image(image, 0)
    contiguous = np.ascontiguousarray(validated)
    digest = hashlib.sha256()
    digest.update(str(contiguous.shape).encode("ascii"))
    digest.update(contiguous.dtype.str.encode("ascii"))
    digest.update(contiguous.tobytes())
    return (
        f"{namespace}:{identifiers[0]}:"
...
class StrictCacheProposalProvider:
    """Read-only proposal replay with an explicit cache-miss policy.

    Unlike :class:`CachedProposalProvider`, this provider has no backend and
    therefore cannot hide an incomplete offline-teacher cache by running an
    online detector.  Cache misses either raise immediately or produce an
    empty proposal list.  Both absent files and image-shape mismatches count as
    misses.
    """

    _MISSING_POLICIES = frozenset(("error", "empty"))
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(namespace, str) or not namespace.strip():
            raise ValueError(
                "strict proposal cache namespace must be a non-empty string"
            )
        policy = str(missing_policy).strip().lower()
        if policy not in self._MISSING_POLICIES:
            raise ValueError(
                "strict proposal cache missing_policy must be 'error' or "
                "'empty'"
            )
        # Re-open the same directory through a write-disabled cache object.
        # This guarantees that even a caller-provided writable cache cannot be
        # mutated through the strict provider.
        self.cache = NpzProposalCache(
            cache.directory,
            write_enabled=False,
        )
    ...
    def predict(
        ...
        *,
        ...
        if identifiers is None:
            raise ValueError(
                "strict proposal cache replay requires frame_ids"
            )

        output: ProposalBatch = []
        for image, frame_id in zip(validated, identifiers):
            key = proposal_cache_key(self.namespace, frame_id, image)
            proposals = self.cache.load(
                key,
                expected_image_shape=image.shape[:2],
            )
            ...
            if self.missing_policy == "error":
                ...
                raise FileNotFoundError(
                    "Missing or image-incompatible strict proposal cache "
                    f"entry for frame {frame_id!r}: {path}"
                )
            output.append([])
        return output
...
def resolve_supplemental_proposal_config(
    ...
    cache_cfg["enabled"] = bool(cache_cfg["enabled"])
    cache_cfg["write"] = bool(cache_cfg["write"])
    namespace = cache_cfg.get("namespace")
    if namespace is not None:
        if not isinstance(namespace, str):
            raise TypeError(
                "supplemental_proposals.cache.namespace must be a string "
                "or None"
            )
        namespace = namespace.strip()
        if not namespace:
            raise ValueError(
                "supplemental_proposals.cache.namespace cannot be empty"
            )
    cache_cfg["namespace"] = namespace
    missing_policy = str(
        cache_cfg.get("missing_policy", "error")
    ).strip().lower()
    if missing_policy not in StrictCacheProposalProvider._MISSING_POLICIES:
        raise ValueError(
            "supplemental_proposals.cache.missing_policy must be 'error' "
            "or 'empty'"
        )
    cache_cfg["missing_policy"] = missing_policy
    return resolved
...
def build_provider(
    ...
    """Build a disabled, YOLOE, cached YOLOE, or strict-cache provider.
    ...
    cache_cfg = resolved["cache"]
    if resolved["provider"] == "cache_only":
        if not cache_cfg["enabled"]:
            raise ValueError(
                "supplemental_proposals.cache.enabled must be true for "
                "provider='cache_only'"
            )
        directory = cache_cfg.get("directory")
        if directory is None or not os.fspath(directory):
            raise ValueError(
                "supplemental_proposals.cache.directory is required for "
                "provider='cache_only'"
            )
        namespace = cache_cfg.get("namespace")
        if namespace is None:
            raise ValueError(
                "supplemental_proposals.cache.namespace is required for "
                "provider='cache_only'"
            )
        return StrictCacheProposalProvider(
            NpzProposalCache(directory, write_enabled=False),
            namespace=namespace,
            missing_policy=cache_cfg["missing_policy"],
        )
    if resolved["provider"] != "yoloe":
        raise ValueError(
            "Unsupported supplemental proposal provider "
            f"{resolved['provider']!r}; expected 'yoloe' or 'cache_only'"
        )
    ...
    if cache_cfg["enabled"]:
        ...
        provider = CachedProposalProvider(
            ...
            namespace=(
                cache_cfg["namespace"]
                if cache_cfg["namespace"] is not None
                else _provider_namespace(resolved)
            ),
        )
    return provider
