# BoxFusion P 路线：P2V3 Reliability Geometry Fusion Observer
...
P2V2 + P2 anchor 约束的局部 Mask-RGBD 连通组件几何 observer
P2V3 + reliability-adaptive parent/component geometry fusion observer
```

P0/P1/P2/P2V2/P2V3 的正式 prediction 均走同一条冻结 B6 输出路径。P2 只选择
P1 residual voxels 并产生诊断候选；P2V2 只消费同一次 provider call
已经 lift 的 unmatched mask 真实深度点，并用 P2 selected voxels 作为
anchor；P2V3 只对冻结的 P2 parent box 与 P2V2 component box 进行
可靠性自适应融合。它们都不能修改框、分数、类别、数量或顺序。
...
```text
同一次 provider call 已有的 unmatched mask + sensor real depth
          ▼
确定性 3D voxel connected components
          +
冻结 P2 selected residual voxels（anchor）
          ▼
只保留含 anchor 且满足点数/尺度/父框距离约束的组件
          ▼
robust quantile AABB + stable class-agnostic NMS
          ▼
P2V2 diagnostics（不读标签/GT，不调用新 provider，不进入正式输出）
```

P2V2 是 P2 的候选几何替换实验，不是额外训练头；P1/P2 checkpoint
仍然完全冻结。它针对 P2 固定 10 场 `ΔRecall@0.50=0` 的定位瓶颈，
是否有效必须由独立的 P2V2 candidate recall 审计决定。

## P2V3 数据流

```text
冻结 P2 parent box + 冻结 P2V2 Mask-RGBD component box
          ▼
由 depth 有效率、组件支持度、anchor 支持度和两框一致性估计 reliability
          ▼
分别对 center 与 log-extent 做有界 precision-weighted fusion
          ▼
stable class-agnostic NMS
          ▼
P2V3 diagnostics（不读类别/GT，不进入正式输出）
```

P2V3 不新增 checkpoint。P1、P2、P2V2 全部保持启用且只读，相比
P2V2 唯一新增模块是 `p2_reliability_fusion`。固定常数
`75% component + 25% parent` 只属于事后诊断，不能作为正式参数；
正式实验必须使用冻结的可靠性规则，并在固定 10 场重新验证。
...
需要明确：P1/P2 使用 ScanNet train GT 构造残差监督，因此 P2V2 方法从
...
```bash
cd /data/ZhaoX/OVM3D-Dett/boxfusion_p2v3_dev
...
BOXFUSION_P1_RESIDUAL_CHECKPOINT="$PWD/models/scannet_p1_residual.pt" \
BOXFUSION_P2_OCCUPANCY_CHECKPOINT="$PWD/models/scannet_p2_occupancy_topk.pt" \
BOXFUSION_P_RUN_TAG=p2v2_ablation10_b6frozen_v1 \
  bash scripts/run_scannet_p_ablation.sh P2V2 0,1

BOXFUSION_P1_RESIDUAL_CHECKPOINT="$PWD/models/scannet_p1_residual.pt" \
BOXFUSION_P2_OCCUPANCY_CHECKPOINT="$PWD/models/scannet_p2_occupancy_topk.pt" \
BOXFUSION_P_RUN_TAG=p2v3_ablation10_b6frozen_v1 \
  bash scripts/run_scannet_p_ablation.sh P2V3 0,1
```
...
每次 P0/P1/P2/P2V2/P2V3 启动都会在对应 log 目录写入不可变
...
```bash
bash scripts/audit_scannet_p2v2.sh
```

该审计必须先验证同一运行内
`observer_only=true`、`mutation_enabled=false`、`applied_count=0`，
再单独评估 P2V2 candidate stream。跨运行 formal prediction 允许的
数值漂移只能来自事先测量并冻结的 P0 repeat 非确定性包络，不能把
任意 pickle 差异当成模块输出。

相对 `B6∪P1∪P2` 的建议继续门槛仍为：
...
固定 10 场未通过时，不运行 100 场，也不继续叠加 P3。

P2V3 完成后运行独立安全与召回审计：

```bash
bash scripts/audit_scannet_p2v3.sh
```

P2V3 沿用同一停止门槛：相对 `B6∪P1∪P2`，
`ΔRecall@0.25 >= 3pp` 且 `ΔRecall@0.50 >= 1pp`。未同时通过时
不激活输出、不跑 100 场，也不进入 P3。

#### 2026-07-30 固定 10 场结果

冻结实验已完成且 artifact validator 通过。P2V3 共消费 381 个
P2V2 step candidates，场景 NMS 后保留 332 个候选，128 个 provider
step 全部成功；新增运行时间为 0.0568 秒/10 场（约 0.0057 秒/场）。
正式输出始终为 `mutation=false`、`applied_count=0`。

| 相对 `B6∪P1∪P2` | ΔR@0.15 | ΔR@0.25 | ΔR@0.50 |
|---|---:|---:|---:|
| P2V2 paired component | +8.05 pp | +4.03 pp | +0.67 pp |
| P2V3 reliability fused | +5.37 pp | +2.01 pp | +1.34 pp |

因此正式决定为 `STOP_P2V3`：P2V3 达到 R@0.50 门槛，但未达到
R@0.25 门槛。不得将 observer 同运行的标准 AP
`42.12 / 36.78 / 18.04` 解释为 P2V3 的 AP 增益，也不应直接运行
100 场。

只读上限诊断显示，同时保留 component 和 fused 两种几何时，
ΔR@0.25/0.50 可达 `+6.04/+2.01 pp`，但需要 664 个候选，候选
precision 仅约 `1.36%/0.45%`。这只证明两种几何具有互补性，不构成
可激活的输出方案；在没有 train-only selector 和 source-aware
score gate 前，禁止把双假设直接追加到正式检测。

### 7. P2 安全与召回审计（旧 P2 对照）
...
### 8. 100 场（仅在固定 10 场与 train-only held-out 通过后）
...
# 只有固定 10 场 P2V2 gate 通过后才运行这一条
BOXFUSION_P_FULL100=1 \
BOXFUSION_P1_RESIDUAL_CHECKPOINT="$PWD/models/scannet_p1_residual.pt" \
BOXFUSION_P2_OCCUPANCY_CHECKPOINT="$PWD/models/scannet_p2_occupancy_topk.pt" \
BOXFUSION_P_RUN_TAG=p2v2_full100_b6frozen_v1 \
  bash scripts/run_scannet_p_ablation.sh P2V2 0,1

# 只有固定 10 场 P2V3 gate 通过后才运行这一条
BOXFUSION_P_FULL100=1 \
BOXFUSION_P1_RESIDUAL_CHECKPOINT="$PWD/models/scannet_p1_residual.pt" \
BOXFUSION_P2_OCCUPANCY_CHECKPOINT="$PWD/models/scannet_p2_occupancy_topk.pt" \
BOXFUSION_P_RUN_TAG=p2v3_full100_b6frozen_v1 \
  bash scripts/run_scannet_p_ablation.sh P2V3 0,1
