...
print(f"Num scenes in test dataset: {len(TEST_DATASET)}", flush=True)
...
def evaluate_one_epoch():
    ...
    for batch_idx, batch_data_label in enumerate(TEST_DATALOADER):
        ...
        print(f'Eval batch: {batch_idx} scan_idx {scan_idx}', flush=True)
        print(f'pred_path {pred_path}', flush=True)
    ...
    for i, ap_calculator in enumerate(ap_calculator_list):
        print('-' * 10, f'iou_thresh: {AP_IOU_THRESHOLDS[i]:.6f}', '-' * 10, flush=True)
