...
OBSERVER_PROFILE="${BOXFUSION_TRAIN_OBSERVER_PROFILE:-trifusion_plus10_observer}"
...
path, expected_scene, profile = sys.argv[1:4]
with np.load(path, allow_pickle=False) as archive:
    common = {
        "scene_id", "c4_diagnostics_schema", "c4_enabled",
        "c4_mutation_enabled", "c4_applied",
    }
    if profile == "trifusion_plus10_observer":
        required = common | {
    ...
    elif profile == "yidu_a5_raw_fused_query_observer":
        required = common | {
            "yidu_diagnostics_schema", "yidu_ablation_schema",
            "yidu_stage", "yidu_profile",
            "yidu_enabled", "yidu_mutation_enabled",
            "yidu_applied_count", "yidu_applied",
            "yidu_modules_json", "yidu_gate_feature_names",
            "yidu_gate_features", "yidu_gate_evaluated",
            "yidu_gate_accepted",
            "yidu_result_indices", "yidu_stable_ids",
            "yidu_attempted", "yidu_valid",
            "yidu_selected_source", "yidu_original_corners",
            "yidu_selected_candidate_corners",
        }
    else:
        raise SystemExit(f"unsupported train observer profile: {profile}")
    ...
    if profile == "trifusion_plus10_observer":
        ...
        expected_schemas["yidu_diagnostics_schema"] = (
            "boxfusion.yidu.local_observer.v1"
        )
    for name, expected in expected_schemas.items():
        if scalar_text(name) != expected:
            raise SystemExit(f"{path}: unsupported {name}")
    enabled_names = ["c4_enabled"]
    if profile == "trifusion_plus10_observer":
        enabled_names += ["trifusion_enabled", "trifusion_missing_enabled"]
    else:
        enabled_names += ["yidu_enabled"]
    for name in enabled_names:
        if not scalar_bool(name):
            raise SystemExit(f"{path}: {name} is false")
    mutation_names = ["c4_mutation_enabled"]
    if profile == "trifusion_plus10_observer":
        mutation_names += [
            "trifusion_mutation_enabled",
            "trifusion_missing_mutation_enabled",
        ...
        mutation_names += ["yidu_mutation_enabled"]
    for name in mutation_names:
        if scalar_bool(name):
            raise SystemExit(f"{path}: forbidden train observer flag {name}")
    applied_names = ["c4_applied"]
    if profile == "trifusion_plus10_observer":
        applied_names += ["trifusion_applied", "trifusion_missing_applied"]
    else:
        applied_names += ["yidu_applied"]
    for name in applied_names:
        value = np.asarray(archive[name])
        if value.dtype != np.bool_ or bool(np.any(value)):
            raise SystemExit(f"{path}: {name} contains applied rows")
    if profile == "yidu_a5_raw_fused_query_observer":
        if scalar_text("yidu_stage") != "A5":
            raise SystemExit(f"{path}: expected Yidu A5 diagnostics")
        if scalar_text("yidu_profile") != profile:
            raise SystemExit(f"{path}: Yidu profile mismatch")
        if scalar_text("yidu_ablation_schema") != (
            "boxfusion.yidu.incremental_observer.v1"
        ):
            raise SystemExit(f"{path}: Yidu ablation schema mismatch")
        applied_count = np.asarray(archive["yidu_applied_count"])
        if (
            applied_count.shape != ()
            or applied_count.dtype.kind not in "iu"
            or int(applied_count.item()) != 0
        ):
            raise SystemExit(f"{path}: invalid Yidu applied count")
        expected_modules = {
            "adaptive_erosion": True,
            "dfu_filter": True,
            "voxel_components": True,
            "occupancy_msr": True,
            "raw_fused_query": True,
            "quality_gate": False,
        }
        if json.loads(scalar_text("yidu_modules_json")) != expected_modules:
            raise SystemExit(f"{path}: invalid Yidu A5 module matrix")
        names = np.asarray(archive["yidu_gate_feature_names"])
        features = np.asarray(archive["yidu_gate_features"])
        ...
        result_indices = np.asarray(archive["yidu_result_indices"])
        stable_ids = np.asarray(archive["yidu_stable_ids"])
        rows = len(result_indices)
        aligned_vectors = (
            "yidu_stable_ids", "yidu_attempted", "yidu_valid",
            "yidu_applied", "yidu_selected_source",
            "yidu_gate_evaluated", "yidu_gate_accepted",
        )
        if (
            names.shape != (91,)
            ...
            != YIDU_GATE_FEATURE_NAMES
            or result_indices.shape != (rows,)
            or result_indices.dtype.kind not in "iu"
            or np.any(result_indices < 0)
            or len(np.unique(result_indices)) != rows
            or stable_ids.shape != (rows,)
            or stable_ids.dtype.kind not in "iu"
            or np.any(stable_ids < 0)
            or len(np.unique(stable_ids)) != rows
            or any(
                np.asarray(archive[name]).shape != (rows,)
                for name in aligned_vectors
            )
            or any(
                np.asarray(archive[name]).dtype != np.bool_
                for name in (
                    "yidu_attempted", "yidu_valid", "yidu_applied",
                    "yidu_gate_evaluated", "yidu_gate_accepted",
                )
            )
            or np.asarray(archive["yidu_selected_source"]).dtype.hasobject
            or np.asarray(archive["yidu_original_corners"]).shape
            != (rows, 8, 3)
            or np.asarray(
                archive["yidu_selected_candidate_corners"]
            ).shape != (rows, 8, 3)
            or not np.isfinite(
                np.asarray(archive["yidu_original_corners"])
            ).all()
            or features.ndim != 2
            or features.shape != (rows, 91)
            or not np.isfinite(features).all()
        ):
            raise SystemExit(f"{path}: invalid Yidu 91-D gate features")
        for name in ("yidu_gate_evaluated", "yidu_gate_accepted"):
            value = np.asarray(archive[name])
            if value.dtype != np.bool_ or bool(np.any(value)):
                raise SystemExit(f"{path}: A5 unexpectedly evaluated a gate")
' "$path" "$scene" "$OBSERVER_PROFILE" "$ROOT" \
...
case "$OBSERVER_PROFILE" in
    trifusion_plus10_observer|yidu_a5_raw_fused_query_observer) ;;
    *) die "unsupported train observer profile: $OBSERVER_PROFILE" ;;
esac
...
echo "Train-only/no-eval observer"
echo "  mode: $([[ "$EXECUTE" == "1" ]] && echo execute || echo check-only)"
echo "  profile: $OBSERVER_PROFILE"
...
mkdir -p \
    ...
    "$LOG_ROOT/mplconfig" \
    "$DIAGNOSTICS_ROOT"
if [[ "$OBSERVER_PROFILE" == "yidu_a5_raw_fused_query_observer" ]]; then
    MANIFEST="$LOG_ROOT/run_manifest.json"
    if [[ "$ALLOW_RESUME" == "1" && ! -f "$MANIFEST" ]]; then
        die "YiDu train resume requires an existing manifest: $MANIFEST"
    fi
    ...
    manifest_args=(
        ...
        --teacher-namespace "$TRAIN_NAMESPACE"
        --cache-missing-policy error
        --live-root "$LIVE_ROOT"
        --frames-root "$FRAMES_ROOT"
        --prediction-root "$PRED_ROOT"
        --log-root "$LOG_ROOT"
        --diagnostics-root "$DIAGNOSTICS_ROOT"
        --evaluation-root "$ROOT/evaluation/train_forbidden/$RUN_TAG"
        --minimum-extent 0.40
    ...
    "$PYTHON" "$ROOT/tools/build_yidu_run_manifest.py" \
        "${manifest_args[@]}"
fi
    ...
    while IFS= read -r scene || [[ -n "$scene" ]]; do
        ...
        if ! (
            ...
            "$PYTHON" demo.py scannet \
                ...
                --online-ablation-profile "$OBSERVER_PROFILE" \
...
echo "$OBSERVER_PROFILE train-only observer collection completed."
