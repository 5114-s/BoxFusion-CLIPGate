#!/usr/bin/env python3
"""Extract posed RGB-D frames for the BoxFusion ScanNetV2 val split.

This uses the ScanNet `SensorData.py` reader that already exists in
`/extra/ZhaoX/scannet_data` on this machine. It only processes scenes listed in
`evaluation/data_util/meta_data/scannetv2_val.txt` and skips scenes whose
`color/`, `depth/`, `pose/`, and `intrinsic/` directories already contain files.
"""
...
def main() -> int:
    ...
    print("ScanNet val frame extraction complete.")
    return 0
