...
RESIDUAL_TRACK_SOURCE_MODE="${BOXFUSION_RESIDUAL_TRACK_SOURCE_MODE:-}"
RESIDUAL_MIN_SEMANTIC_SCORE="${BOXFUSION_RESIDUAL_MIN_SEMANTIC_SCORE:-}"
...
if [[ -n "$RESIDUAL_TRACK_SOURCE_MODE" ]]; then
    if [[ "$ABLATION_PROFILE" != "residual_track_observer" ]]; then
        echo "BOXFUSION_RESIDUAL_TRACK_SOURCE_MODE is valid only with residual_track_observer" >&2
        exit 1
    fi
    case "$RESIDUAL_TRACK_SOURCE_MODE" in
        sam3|yoloe|dual) ;;
        *)
            echo "BOXFUSION_RESIDUAL_TRACK_SOURCE_MODE must be sam3, yoloe, or dual" >&2
            exit 1
            ;;
    esac
fi
if [[ -n "$RESIDUAL_MIN_SEMANTIC_SCORE" ]]; then
    if [[ "$ABLATION_PROFILE" != "residual_track_observer" ]]; then
        echo "BOXFUSION_RESIDUAL_MIN_SEMANTIC_SCORE is valid only with residual_track_observer" >&2
        exit 1
    fi
    if [[ ! "$RESIDUAL_MIN_SEMANTIC_SCORE" =~ ^([0-9]+([.][0-9]*)?|[.][0-9]+)$ ]]; then
        echo "BOXFUSION_RESIDUAL_MIN_SEMANTIC_SCORE must be numeric" >&2
        exit 1
    fi
    if ! "$PYTHON" -c \
        "import math; value=float('$RESIDUAL_MIN_SEMANTIC_SCORE'); assert math.isfinite(value) and 0.0 <= value <= 1.0" \
        >/dev/null 2>&1; then
...
echo "[$(date '+%F %T')] Residual track source mode: ${RESIDUAL_TRACK_SOURCE_MODE:-config-default}"
echo "[$(date '+%F %T')] Residual minimum semantic score: ${RESIDUAL_MIN_SEMANTIC_SCORE:-config-default}"
...
run_worker() {
    ...
    while IFS= read -r scene || [[ -n "$scene" ]]; do
        ...
        if [[ -n "$RESIDUAL_TRACK_SOURCE_MODE" ]]; then
            optional_args+=(
                --residual-track-source-mode "$RESIDUAL_TRACK_SOURCE_MODE"
            )
        fi
        if [[ -n "$RESIDUAL_MIN_SEMANTIC_SCORE" ]]; then
            optional_args+=(
                --residual-track-min-semantic-score "$RESIDUAL_MIN_SEMANTIC_SCORE"
            )
        fi
        if [[ -n "$YIDU_GATE_CHECKPOINT" ]]; then
            optional_args+=(
