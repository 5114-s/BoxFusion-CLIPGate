...
def test_global_overlap_rejection_happens_before_track_creation_and_at_readout():
    ...
    before_readout = observer.summary()
    assert observer.candidates(candidate_box[None, :]) == ()
    decisions = observer.candidate_decisions(candidate_box[None, :])
    assert len(decisions) == 1
    assert decisions[0].confirmed is True
    assert decisions[0].accepted is False
    assert decisions[0].reason == "global_overlap"
    assert decisions[0].maximum_global_iou == pytest.approx(1.0)
    assert decisions[0].maximum_global_containment == pytest.approx(1.0)
    assert observer.summary() == before_readout
