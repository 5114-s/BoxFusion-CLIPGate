...
from tools.export_residual_track_candidates import (
    export_directory as export_residual_directory,
)
...
class _FailAfterOneSecondaryProvider:
    ...
    def predict(self, images, *, frame_ids=None):
        ...
        raise RuntimeError("synthetic partial SAM3 cache failure")
...
def test_c4_observer_is_bit_exact_to_quality_only_b6(tmp_path):
    ...
    assert c4.summary["observer_zero_write_check_enabled"] is True
    assert c4.summary["observer_zero_write_verified"] is True
    assert (
        c4.summary["observer_zero_write_pre_sha256"]
        == c4.summary["observer_zero_write_post_sha256"]
    )
...
def test_trifusion_observer_is_bit_exact_to_c4_and_quality_only_b6(
    ...
    assert trifusion.summary["observer_zero_write_check_enabled"] is True
    assert trifusion.summary["observer_zero_write_verified"] is True
    assert (
        trifusion.summary["observer_zero_write_pre_sha256"]
        == trifusion.summary["observer_zero_write_post_sha256"]
    )
...
def test_residual_sam3_track_is_isolated_from_c4_m3_and_b6_output(
    tmp_path,
):
    mask = np.zeros((16, 16), dtype=bool)
    mask[3:13, 3:13] = True
    ...
    b6 = OnlineRefinementController(
        _config(tmp_path / "residual_b6", "quality_only"),
        provider=_NoopProvider(),
        quality_scorer=scorer,
    )
    residual = OnlineRefinementController(
        _config(
            tmp_path / "residual_sam3",
            "residual_track_observer",
            diagnostics=True,
        ),
        provider=_NoopProvider(),
    ...
    for frame_id in (0, 1):
        b6.process_keyframe(**frame, frame_id=frame_id)
        residual.process_keyframe(**frame, frame_id=frame_id)

    b6_result = b6.finalize(**inputs)
    result = residual.finalize(
        **inputs, scene_id="scene0000_00"
    )
    _assert_same_bytes(result, b6_result)
    assert result.summary["residual_track_source_mode"] == "sam3"
    assert result.summary["residual_track_provider_calls"] == 2
    assert result.summary["residual_track_primary_unmatched"] == 0
    assert result.summary["residual_track_secondary_unmatched"] == 2
    assert result.summary["residual_track_candidates"] == 1
    assert result.summary["residual_track_associations_evaluated"] == 1
    assert result.summary["residual_track_associations_accepted"] == 1
    assert result.summary["residual_track_associations_selected"] == 1
    assert result.summary["residual_track_tracks_seeded"] == 1
    assert result.summary["residual_track_final_decisions"] == 1
    assert result.summary["residual_track_final_confirmed"] == 1
    assert result.summary["residual_track_final_accepted"] == 1
    assert result.summary[
        "residual_track_final_decision_reasons"
    ] == {"confirmed_multiview": 1}
    assert result.summary["trifusion_attempted"] == 0
    assert result.summary["c4_attempted"] == 0
    assert result.summary["observer_zero_write_verified"] is True
    ...
    with np.load(path, allow_pickle=False) as payload:
        assert (
            str(payload["residual_track_diagnostics_schema"].item())
            == "boxfusion.residual_mask_track_observer.v1"
        )
        assert payload["residual_track_candidate_ids"].shape == (1,)
        assert payload["residual_track_graph_confirmed"].tolist() == [True]
        assert payload["residual_track_applied"].tolist() == [False]
        assert payload["residual_track_primary_nodes"].tolist() == [0]
        assert payload["residual_track_secondary_nodes"].tolist() == [2]
        assert payload["residual_track_mixed_provider"].tolist() == [False]
        assert json.loads(
            str(
                payload[
                    "residual_track_final_decision_reasons_json"
                ].item()
            )
        ) == {"confirmed_multiview": 1}
        final_decisions = json.loads(
            str(
                payload[
                    "residual_track_final_decisions_json"
                ].item()
            )
        )
        assert final_decisions == [
            {
                ...
                "maximum_global_containment": 0.0,
                "maximum_global_iou": 0.0,
        ...
        assert payload[
            "residual_track_final_decision_track_ids"
        ].tolist() == [0]
        assert payload[
            "residual_track_final_decision_accepted"
        ].tolist() == [True]
        assert payload[
            "residual_track_final_decision_maximum_global_iou"
        ].tolist() == [0.0]
        assert int(
            payload["residual_track_associations_evaluated"].item()
        ) == 1
        assert int(
            payload["residual_track_associations_accepted"].item()
        ) == 1
        assert int(
            payload["residual_track_associations_selected"].item()
        ) == 1
        assert json.loads(
            str(
                payload[
                    "residual_track_last_association_reasons_json"
                ].item()
            )
        ) == {"accepted": 1}
    scene_list = tmp_path / "residual_one_scene.txt"
    scene_list.write_text("scene0000_00\n", encoding="utf-8")
    export_root = tmp_path / "residual_supplemental_export"
    export_summary = export_residual_directory(
        diagnostics_root=tmp_path / "residual_sam3",
        scene_list=scene_list,
        output_root=export_root,
    )
    assert export_summary["candidates"] == 1
    ...
    assert exported.candidate_ids == (
        "scene0000_00:residual_track:track:0",
    )


def test_residual_dual_sources_share_one_graph_clock_and_track(
    tmp_path,
):
    mask = np.zeros((16, 16), dtype=bool)
    mask[3:13, 3:13] = True
    yoloe_proposal = SupplementalProposal(
        bbox=np.asarray([3.0, 3.0, 13.0, 13.0], dtype=np.float32),
        score=0.91,
        mask=mask,
        label="chair",
    ...
    sam3_proposal = SupplementalProposal(
        bbox=np.asarray([3.0, 3.0, 13.0, 13.0], dtype=np.float32),
        score=0.89,
        mask=mask,
        label="chair",
    ...
    config = _config(
        tmp_path / "residual_dual",
        "residual_track_observer",
        diagnostics=True,
    )
    config["online_refinement"]["residual_track_observer"][
        "source_mode"
    ] = "dual"
    controller = OnlineRefinementController(
        config,
        provider=_SequenceSecondaryProvider([[yoloe_proposal], []]),
        generic_geometry_provider=_SequenceSecondaryProvider(
            [[], [sam3_proposal]]
        ),
        quality_scorer=lambda mapping: float(
            mapping["detector_score"]
        ),
    )
    inputs, _ = _inputs()
    frame = {
    ...
    for frame_id in (0, 1):
        controller.process_keyframe(**frame, frame_id=frame_id)
    result = controller.finalize(
        **inputs, scene_id="scene0000_00"
    )
    assert result.summary["residual_track_provider_calls"] == 2
    assert controller.residual_track_graph.last_provider_call_index == 1
    assert result.summary["residual_track_candidates"] == 1
    candidate = controller._last_residual_track_candidates[0]
    ...
    with np.load(path, allow_pickle=False) as payload:
        assert payload["residual_track_primary_nodes"].tolist() == [1]
        assert payload["residual_track_secondary_nodes"].tolist() == [1]
        assert payload["residual_track_unknown_nodes"].tolist() == [0]
        assert payload["residual_track_mixed_provider"].tolist() == [True]


def test_residual_sam3_partial_cache_failure_discards_the_whole_graph(
    tmp_path,
):
    mask = np.zeros((16, 16), dtype=bool)
    mask[3:13, 3:13] = True
    ...
    controller = OnlineRefinementController(
        _config(
            tmp_path / "residual_fail_closed",
            "residual_track_observer",
            diagnostics=True,
        ),
        provider=_NoopProvider(),
        generic_geometry_provider=_FailAfterOneSecondaryProvider(
            [proposal]
        ),
        quality_scorer=lambda mapping: float(
            mapping["detector_score"]
    ...
    controller.process_keyframe(**frame, frame_id=0)
    assert controller.residual_track_graph.active_track_ids == (0,)
    controller.process_keyframe(**frame, frame_id=1)
    assert controller.residual_track_graph.active_track_ids == ()
    result = controller.finalize(
        **inputs, scene_id="scene0000_00"
    )
    assert result.summary["residual_track_fail_closed"] is True
    assert result.summary["residual_track_candidates"] == 0
    assert result.summary["observer_zero_write_verified"] is True
    path = (
        tmp_path
        / "residual_fail_closed"
        / "scene0000_00_tracks.npz"
    )
    with np.load(path, allow_pickle=False) as payload:
        assert bool(payload["residual_track_failed"].item()) is True
        assert payload["residual_track_candidate_ids"].shape == (0,)
...
def _yidu_pair(
    ...
    *,
    ...
    if profile == "yidu_a6_quality_gate_observer":
        yidu_config["online_refinement"]["yidu_ablation"][
            "quality_gate"
        ]["checkpoint"] = str(tmp_path / "injected-gate.npz")
    yidu = OnlineRefinementController(
        yidu_config,
        provider=_NoopProvider(),
        generic_geometry_provider=_NoopProvider(),
        quality_scorer=scorer(captured_yidu),
        yidu_ap50_gate=gate,
    )
    inputs, boxes = _inputs()
    for controller in (b6, yidu):
        _install_primary_evidence(controller, 7, boxes[0], "chair")
        _install_primary_evidence(controller, 8, boxes[1], "table")
    _install_c4_multiview_memory(yidu, 7, boxes[0])
    b6_result = b6.finalize(**inputs)
    yidu_result = yidu.finalize(
        **inputs,
        scene_id="scene0000_00" if diagnostics else None,
    )
    return (
...
@pytest.mark.parametrize(
    "profile,stage",
    [
        ("yidu_a1_adaptive_erosion_observer", "A1"),
        ("yidu_a2_dfu_filter_observer", "A2"),
        ("yidu_a3_voxel_components_observer", "A3"),
        ("yidu_a4_occupancy_msr_observer", "A4"),
        ("yidu_a5_raw_fused_query_observer", "A5"),
    ],
)
def test_yidu_incremental_observers_are_bit_exact_to_frozen_b6(
    tmp_path, profile, stage
):
    b6, result, controller, captured_b6, captured_yidu = _yidu_pair(
        tmp_path, profile
    )
    _assert_same_bytes(result, b6)
    assert captured_yidu == captured_b6
    assert result.summary["yidu_stage"] == stage
    assert result.summary["mutation_yidu_enabled"] is False
    assert result.summary["yidu_applied"] == 0
    assert result.summary["yidu_attempted"] == 1
    assert result.summary["observer_zero_write_check_enabled"] is True
    assert result.summary["observer_zero_write_verified"] is True
    assert len(result.summary["observer_zero_write_pre_sha256"]) == 64
    assert (
        result.summary["observer_zero_write_pre_sha256"]
        == result.summary["observer_zero_write_post_sha256"]
    )
    assert result.summary["observer_zero_write_changed_fields"] == []
    assert b6.summary["observer_zero_write_check_enabled"] is False
    runtime = controller._last_yidu_runtime[7]
    assert runtime["attempted"] is True
    assert runtime["valid"] is True
    assert runtime["applied"] is False
    np.testing.assert_array_equal(
        result.corners[0], runtime["original_corners"]
    )


def test_yidu_a5_diagnostics_align_and_remain_non_mutating(tmp_path):
    b6, result, _, _, _ = _yidu_pair(
        tmp_path,
        "yidu_a5_raw_fused_query_observer",
        diagnostics=True,
    )
    _assert_same_bytes(result, b6)
    path = tmp_path / "yidu" / "scene0000_00_tracks.npz"
    with np.load(path, allow_pickle=False) as payload:
        assert str(payload["yidu_stage"].item()) == "A5"
        assert bool(payload["yidu_enabled"].item()) is True
        assert bool(payload["yidu_mutation_enabled"].item()) is False
        assert int(payload["yidu_applied_count"].item()) == 0
        assert not np.any(payload["yidu_applied"])
        assert bool(
            payload["yidu_zero_write_check_enabled"].item()
        ) is True
        assert bool(payload["yidu_zero_write_verified"].item()) is True
        pre_sha256 = str(
            payload["yidu_zero_write_pre_sha256"].item()
        )
        post_sha256 = str(
            payload["yidu_zero_write_post_sha256"].item()
        )
        assert len(pre_sha256) == 64
        assert pre_sha256 == post_sha256
        assert payload["yidu_zero_write_array_names"].tolist() == [
            "corners",
            "boxes",
            "scores",
            "source_indices",
        ...
        assert payload["yidu_zero_write_changed_fields"].size == 0
        # Diagnostics remain aligned with every final output row.  The first
        # row has local evidence; the second is an explicit unobserved row.
        assert payload["yidu_stable_ids"].tolist() == [7, 8]
        assert payload["yidu_attempted"].tolist() == [True, False]
        assert payload["yidu_component_features"].shape == (2, 12)
        assert payload["yidu_occupancy_features"].shape == (2, 48)
        assert payload["yidu_raw_fused_features"].shape == (2, 30)
        assert payload["yidu_gate_features"].shape == (
            2,
            YIDU_GATE_FEATURE_DIM,
        )


def test_yidu_zero_write_guard_rejects_in_process_observer_mutation(
    tmp_path,
    monkeypatch,
):
    config = _config(
    ...
    controller = OnlineRefinementController(
        ...
        provider=_NoopProvider(),
        generic_geometry_provider=_NoopProvider(),
        quality_scorer=lambda mapping: (
            0.15 + 0.70 * float(mapping["detector_score"])
        ),
    )
    original_observer = controller._observe_yidu_candidates

    def mutating_observer(**kwargs):
        original_observer(**kwargs)
        kwargs["scores"][0] = np.nextafter(
            kwargs["scores"][0],
            np.float32(np.inf),
        )
    ...
    with pytest.raises(
        RuntimeError,
        match=(
            "observer zero-write contract violated; "
            "formal output array bytes changed: scores"
        ),
    ):
        controller.finalize(**inputs)

    audit = controller._last_observer_zero_write_audit
    assert audit["enabled"] is True
    assert audit["verified"] is False
    assert audit["changed_fields"] == ("scores",)
    assert len(audit["pre_sha256"]) == 64
    assert len(audit["post_sha256"]) == 64
    assert audit["pre_sha256"] != audit["post_sha256"]


def test_yidu_a6_gate_can_accept_but_cannot_change_output(tmp_path):
    bias = np.asarray(
        [
            np.arctanh(0.20),
            np.log(0.01**2),
    ...
    gate = AP50SafetyGate(
        ...
        weights=(
            np.zeros(
                (YIDU_GATE_FEATURE_DIM, len(bias)),
                dtype=np.float32,
            ),
        ),
        biases=(bias,),
    ...
    b6, result, controller, _, _ = _yidu_pair(
        tmp_path,
        "yidu_a6_quality_gate_observer",
        diagnostics=True,
        gate=gate,
    ...
    assert runtime["gate_evaluated"] is True
    assert runtime["applied"] is False
    assert result.summary["yidu_gate_evaluated"] == 1
    assert result.summary["yidu_applied"] == 0
