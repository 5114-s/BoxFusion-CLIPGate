...
def test_residual_track_profile_is_a_single_module_frozen_b6_child():
    online = apply_online_ablation_profile(
        complete_config(), "residual_track_observer"
    )["online_refinement"]
    assert online["appearance_memory"]["enabled"] is True
    ...
    assert online["box_refiner"]["enabled"] is False
    assert online["supplemental_output"]["enabled"] is False
    assert online["trifusion_observer"]["enabled"] is False
    assert (
        online["generic_local_geometry_refiner"][
    ...
    residual = online["residual_track_observer"]
    ...
    assert residual["source_mode"] == "sam3"
    assert residual["missing_instance_graph"]["enabled"] is True
