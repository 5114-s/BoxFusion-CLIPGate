...
- 每次 observer finalize 都对正式输出数组执行同进程、逐字节的
  pre/post SHA256 零写回检查；任一字段变化都会立即终止，不保存被污染输出。
- B0 与 observer 是独立 GPU 运行。由于 BoxFusion 的 PyCUDA 浮点原子归约
  不能保证跨运行 bit-exact，跨运行审计采用“结构严格一致 + 显式数值包络”；
  默认包络沿用既有 P0-repeat/P2 协议，不根据当前 A1/A2 结果反向设定。
...
A1/A2 的输出是清理前后点集与保留率诊断，不是候选框。因此对这两个阶段运行
通用 geometry candidate exporter 时，`candidates=0` 是预期行为，不能把它
解释为模块失效，也不能用 AP25/AP50 crossing 作为 A1/A2 的单独通过条件。
第一个产生可做 IoU/oracle 审计的几何框阶段是 A3。若要严格区分 A1 与 A2
各自对最终候选框的贡献，需要额外运行共享同一 A3 组件生成器的 paired
controls（raw+A3、A1+A3、A1+A2+A3），不能仅比较当前累计阶段的正式 AP。
...
B0 不启用 SAM3/C4，因此 v3 manifest 中 B0 的 teacher cache、namespace 和
cache policy 均记录为 `null`；A1–A6 则必须记录并校验实际启用的 immutable
teacher cache。
...
A1 到 A6 是 observer，因此使用两层互补检查：

1. **同一次运行的零写回检查**：正式数组组装后、observer 调用前，对 corners、
   boxes、scores、stable/source IDs、quality features 和全部 refit 数组做
   C-order 字节快照与 SHA256；observer 结束后必须逐字节相同。任何变化立即
   终止推理，且不会导出被污染的预测。
2. **独立运行之间的比较**：框数量、标签、顺序、dtype、stable/source IDs 和
   score 排名必须严格相同；box/score 数值使用显式、预注册的 CUDA 重跑漂移
   包络。原因是 BoxFusion 的 PyCUDA 浮点原子归约即使固定 seed 也不能保证
   不同进程间 bit-exact。

统一脚本当前显式采用此前独立 P0/P2 协议冻结的保守上界：
`corner <= 0.02 m`、`score <= 0.02`、same-index `1-IoU <= 0.05`。
这些值不是根据当前 A1/A2 结果反向选择的。需要检查跨运行 bit-exact 时，可设置
`BOXFUSION_YIDU_IDENTITY_MODE=strict_bitwise`。
...
`yidu_mutation_enabled=false`、`yidu_applied_count=0`、阶段模块矩阵，以及
`yidu_zero_write_verified=true` 且 pre/post SHA256 相同。结构变化、超过数值
包络或缺少同进程证明都视为实现错误，不能继续 100 场。
...
A1 和 A2 只改变 mask-depth 点清理过程，尚未定义 box candidate；因此这两个
阶段导出的 `candidates=0` 是预期行为，不表示代码没有执行。A1/A2 应检查
attempted/valid、输入点保留量和过滤原因。真正的 box candidate 从 A3/A4 开始，
届时才使用 ΔIoU 和 crossing 作为继续条件。
...
1. 同进程 observer 零写回检查 bit-exact，并且跨运行结构严格一致、数值在冻结
   漂移包络内；
...
- A1 到 A6 的最终 AP 应与 B0 指标一致；独立 CUDA 重跑可能出现包络内数值漂移，
  但同一运行的 observer 前后数组必须 bit-exact。超过包络或结构变化才说明输出
  被污染或实验输入不一致。
