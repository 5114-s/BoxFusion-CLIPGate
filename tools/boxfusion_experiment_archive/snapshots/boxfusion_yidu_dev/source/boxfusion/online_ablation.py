...
def apply_online_ablation_profile(
    ...
    ``residual_track_observer``
        Preserve the historical B6 quality-only output bit-for-bit while
        unmatched SAM3 and/or YOLOE masks are lifted with real depth and
        associated in one provider-call-clocked multi-view graph.  This
        profile disables C4 per-global refinement and TriFusion occupancy/MSR;
        graph candidates are diagnostics only and can never enter output.
    ``b3_memory_observer``
    ...
    if profile == "residual_track_observer":
        generic_geometry["observe_existing_globals"] = False
    elif "observe_existing_globals" in generic_geometry:
        generic_geometry["observe_existing_globals"] = True
    if is_c4_observer:
    ...
    residual = online.get("residual_track_observer")
    if not isinstance(residual, MutableMapping):
        residual = {} if residual is None else dict(residual)
        online["residual_track_observer"] = residual
    is_residual_observer = profile == "residual_track_observer"
    residual["enabled"] = is_residual_observer
    residual["observer_only"] = True
    residual["mutate"] = False
    residual["collect_diagnostics"] = is_residual_observer
    # The profile supports ``sam3``, ``yoloe`` and ``dual``.  The dedicated
    # runner defaults to ``sam3`` for a one-source teacher-ceiling ablation;
    # a CLI override may select either of the other modes after profile
    # application without changing any mutation bit.
    residual.setdefault("source_mode", "sam3")
    residual_graph = residual.get("missing_instance_graph")
    if not isinstance(residual_graph, MutableMapping):
        residual_graph = (
            {} if residual_graph is None else dict(residual_graph)
        )
        residual["missing_instance_graph"] = residual_graph
    residual_graph["enabled"] = is_residual_observer
