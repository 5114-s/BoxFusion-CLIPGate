...
def build_parser():
    """Build the command-line parser without parsing arguments on import."""
    ...
    parser.add_argument('--gt_root', type=str, default='./data_util/scannet_train_detection_data', help='Prepared ScanNet detection GT root')
    ...
    parser.add_argument('--scene_list', type=str, default=None, help='Optional text file containing one ScanNet scene ID per line.')
    ...
    parser.add_argument('--num_workers', type=int, default=0, help='DataLoader workers for evaluation.')
    parser.add_argument('--seed', type=int, default=0, help='NumPy seed used by deterministic evaluation sampling.')
    return parser
...
def dataset_split_for_scene_list(scene_list):
    """Return the GT split needed for a requested evaluation scope.

    Default evaluation intentionally remains on ScanNet ``val``.  An explicit
    scene list is resolved against every prepared scan under ``--gt_root`` so
    train-only diagnostic lists can be evaluated without weakening missing
    scene validation.
    """

    return 'all' if scene_list is not None else 'val'


def read_requested_scenes(scene_list):
    """Read and validate one explicit ScanNet scene-list file."""

    if not os.path.isfile(scene_list):
        raise FileNotFoundError(scene_list)
    with open(scene_list, 'r') as scene_file:
        requested_scenes = [
            line.strip()
    ...
    if not requested_scenes:
        raise ValueError('--scene_list must contain at least one scene')
    if len(requested_scenes) != len(set(requested_scenes)):
        raise ValueError('--scene_list contains duplicate scene IDs')
    return requested_scenes


def validate_requested_scenes(requested_scenes, available_scenes):
    """Validate an ordered request against scans present under ``gt_root``."""

    available = set(available_scenes)
    missing_scenes = [
        scene for scene in requested_scenes if scene not in available
    ]
    if missing_scenes:
        raise ValueError(
    ...
    return list(requested_scenes)


def log_string(out_str):
    if DUMP_FOUT is None:
        raise RuntimeError('evaluation runtime has not been configured')
...
def configure_runtime(flags):
    """Create evaluation state after CLI parsing, never during import."""

    from torch.utils.data import DataLoader

    # Load the evaluation ``utils`` namespace before the legacy dataset adds
    # ``evaluation/utils`` itself to sys.path (where ``utils.py`` would
    # otherwise shadow the namespace package).
    from utils import utils as _evaluation_utils  # noqa: F401
    from data_util.dataset import ScannetDetectionDataset
    from data_util.model_util_scannet import ScannetDatasetConfig

    global FLAGS
    global BATCH_SIZE, NUM_POINT
    global DUMP_DIR, DUMP_FOUT, AP_IOU_THRESHOLDS
    global DATASET_CONFIG, TEST_DATASET, TEST_DATALOADER, CONFIG_DICT

    FLAGS = flags
    BATCH_SIZE = FLAGS.batch_size
    NUM_POINT = FLAGS.num_point

    # Respect the caller's dump directory so paired ablations stay isolated.
    time_string = datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
    DUMP_DIR = os.path.join(FLAGS.dump_dir, time_string)
    FLAGS.DUMP_DIR = DUMP_DIR
    AP_IOU_THRESHOLDS = [
        float(x) for x in FLAGS.ap_iou_thresholds.split(',')
    ]
    ...
    dataset_split = dataset_split_for_scene_list(FLAGS.scene_list)
    TEST_DATASET = ScannetDetectionDataset(
        ...
        data_path=FLAGS.gt_root
    )

    if FLAGS.scene_name != 'None' and FLAGS.scene_list is not None:
        raise ValueError(
            '--scene_name and --scene_list are mutually exclusive'
        )
    if FLAGS.scene_list is not None:
        requested_scenes = read_requested_scenes(FLAGS.scene_list)
        TEST_DATASET.scan_names = validate_requested_scenes(
            requested_scenes,
            TEST_DATASET.scan_names,
        )
    elif FLAGS.scene_name != 'None':
    ...
    print("FLAGS.shuffle_dataset:", FLAGS.shuffle_dataset, flush=True)
...
def evaluate_one_epoch():
    ...
    """
    from utils.ap_helper import APCalculator, parse_groundtruths
    from utils.utils import (
        flip_axis_to_camera,
        load_data,
        obb_to_aabb_corners,
        reorganize_obb_to_aabb,
    )
    ...
    for batch_idx, batch_data_label in enumerate(TEST_DATALOADER):
        ...
        if not os.path.exists(pred_path):
            raise FileNotFoundError(
                'Missing prediction for requested scene: {}'.format(pred_path)
            )
        else:
            batch_pred_map_cls = load_data(pred_path)
            bbox = [i[1] for i in batch_pred_map_cls[0]]
            scores = [float(i[2]) for i in batch_pred_map_cls[0]]
            ...
            batch_pred_map_cls = [
                (int(0), batch_pred_map_cls[i], scores[i])
        ...
        print("****************************************************", flush=True)
        print("scan_idx", scan_idx, flush=True)
        print("pred_labels", Counter(pred_labels), flush=True)
        gt_labels = [i[0] for i in batch_gt_map_cls[0]]
        print("gt_labels", Counter(gt_labels), flush=True)
    ...
    for i, ap_calculator in enumerate(ap_calculator_list):
        print('-' * 10, 'iou_thresh: %f' % (AP_IOU_THRESHOLDS[i]), '-' * 10, flush=True)
