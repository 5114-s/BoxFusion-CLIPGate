...
if __name__ == "__main__":
    ...
    parser.add_argument("--residual-track-source-mode", choices=["sam3", "yoloe", "dual"], default=None, help="Mask source used by residual_track_observer")
    parser.add_argument("--residual-track-min-semantic-score", default=None, type=float, help="Override only residual_track_observer missing-instance semantic gate in [0, 1]")
    ...
    if dataset_path.lower() in ["scannet", "ca1m", 'online']:
        ...
        if args.residual_track_source_mode is not None:
            if args.online_ablation_profile != "residual_track_observer":
                parser.error(
                    "--residual-track-source-mode is valid only with "
                    "residual_track_observer"
                )
            cfg.setdefault("online_refinement", {}).setdefault(
                "residual_track_observer", {}
            )["source_mode"] = args.residual_track_source_mode
        if args.residual_track_min_semantic_score is not None:
            try:
                configure_residual_min_semantic_score(
                    cfg,
                    args.residual_track_min_semantic_score,
