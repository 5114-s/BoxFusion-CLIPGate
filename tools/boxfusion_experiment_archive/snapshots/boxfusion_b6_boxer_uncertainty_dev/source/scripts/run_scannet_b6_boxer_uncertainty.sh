...
# Isolated B6 + Selective Boxer G0 + uncertainty-fusion ablation.
#
# Fixed 10 scenes (default):
...
#
# Full 100 scenes must be explicitly requested after the fixed-10 gate:
...
case "$PROFILE" in
    ...
    u2_active)
        ...
        ;;
    *)
        echo "Profile must be u0_control, u1_observer, or u2_active" >&2
        exit 2
        ;;
esac
...
artifact_root="$ROOT/results/b6_boxer_uncertainty/$PROFILE/$list_scope"
log_root="$ROOT/logs/b6_boxer_uncertainty/$PROFILE/$list_scope"
diagnostics_root="$ROOT/diagnostics/b6_boxer_uncertainty/$PROFILE/$list_scope/online"
boxer_diagnostics_root="$ROOT/diagnostics/b6_boxer_uncertainty/$PROFILE/$list_scope/boxer"
uncertainty_diagnostics_root="$ROOT/diagnostics/b6_boxer_uncertainty/$PROFILE/$list_scope/uncertainty"
eval_root="$ROOT/evaluation/b6_boxer_uncertainty/$PROFILE/$list_scope"
...
# A residual shell setting must not silently enable a second learned refiner.
unset BOXFUSION_REFINER_CHECKPOINT

# Freeze the already validated Selective Boxer G0 gate.
...
export BOXFUSION_BOXER_UNCERTAINTY_MODE="$UNCERTAINTY_MODE"
...
if [[ "$UNCERTAINTY_MODE" == "disabled" ]]; then
    unset BOXFUSION_BOXER_UNCERTAINTY_DIAGNOSTICS_ROOT
else
    export BOXFUSION_BOXER_UNCERTAINTY_DIAGNOSTICS_ROOT="$uncertainty_diagnostics_root"
fi
export BOXFUSION_EVAL_ROOT="$eval_root"

echo "B6 + Selective Boxer G0 + uncertainty-fusion ablation"
echo "  profile/mode: $PROFILE/$UNCERTAINTY_MODE"
echo "  scenes: $SCENE_LIST"
echo "  scene-list SHA256: $list_sha"
echo "  GPUs: $GPU_SPEC"
echo "  score/min-extent: 0.40/0.40"
echo "  Selective Boxer G0: center<=0.10 m, volume=[0.50,2.00]"
echo "  reliable views: Top-K=3; Boxer confidence power=1.0"
echo "  CuTR fallback uncertainty factor: 1.0"
echo "  predictions: $artifact_root"
echo "  uncertainty diagnostics: ${BOXFUSION_BOXER_UNCERTAINTY_DIAGNOSTICS_ROOT:-disabled}"
