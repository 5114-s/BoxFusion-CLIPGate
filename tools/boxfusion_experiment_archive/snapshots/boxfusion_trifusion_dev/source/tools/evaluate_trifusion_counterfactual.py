...
def load_m3_diagnostics(
    ...
    *,
    ...
    c4_result_indices = _integer_rows(
        payload["c4_result_indices"],
        name="c4_result_indices",
        path=path,
        nonnegative=True,
    )
    c4_rows = len(c4_result_indices)
    if len(np.unique(c4_result_indices)) != c4_rows or np.any(
        c4_result_indices[1:] <= c4_result_indices[:-1]
    ):
        raise ValueError(
            f"{path}: c4_result_indices must be unique and increasing"
        )
    if c4_rows and int(c4_result_indices[-1]) >= len(frozen.scores):
        raise ValueError(
            f"{path}: C4 result index exceeds frozen predictions"
        )
    c4_stable_ids = _integer_rows(
        payload["c4_stable_ids"],
        name="c4_stable_ids",
        path=path,
        rows=c4_rows,
    )
    if len(np.unique(c4_stable_ids)) != c4_rows:
        raise ValueError(f"{path}: c4_stable_ids must be unique")

    # TriFusion consumes the independent C4/SAM3 geometry-memory table, not
    # the historical B3/B6 ObjectGeometryMemory table represented by
    # ``result_indices`` above.  A final prediction may therefore have valid
    # C4 evidence while being absent from that legacy table.  Align the
    # observer rows with their actual C4 parent contract.
    positions = np.searchsorted(c4_result_indices, result_indices)
    if np.any(positions >= c4_rows) or not np.array_equal(
        c4_result_indices[positions], result_indices
    ):
        raise ValueError(
            f"{path}: TriFusion rows are absent from c4_result_indices"
        )
    if not np.array_equal(c4_stable_ids[positions], stable_ids):
        raise ValueError(
            f"{path}: TriFusion stable IDs do not align with C4 result indices"
        )
