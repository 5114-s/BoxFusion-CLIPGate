...
# Pure B3 ablation: conservative robust geometry refit from Top-K Mask-RGBD
# memory.  Original detector scores and detection count are preserved.
#
# Usage:
#   bash scripts/run_scannet_b3_topk_refit.sh 0,1

GPU_SPEC="${1:-0}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
RUN_TAG="${BOXFUSION_B3_RUN_TAG:-b3_topk_refit_extent040_ablation10}"
...
export BOXFUSION_ONLINE_ABLATION_PROFILE="b3_topk_refit_only"
...
echo "B3 Top-K refit run: tag=$RUN_TAG, scenes=$SCENE_LIST, GPUs=$GPU_SPEC"
