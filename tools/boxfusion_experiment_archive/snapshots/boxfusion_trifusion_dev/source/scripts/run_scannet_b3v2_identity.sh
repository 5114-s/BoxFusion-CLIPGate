...
# B3-v2 identity control: construct the exact K=5 visibility-memory inputs,
# but disable every output mutation.  Its predictions must match the paired
# score-preserving control element by element.

GPU_SPEC="${1:-0}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
RUN_TAG="${BOXFUSION_B3V2_RUN_TAG:-b3v2_memory_identity_extent040_ablation10}"
...
export BOXFUSION_ONLINE_ABLATION_PROFILE="b3v2_memory_observer"
...
echo "B3-v2 identity: tag=$RUN_TAG, scenes=$SCENE_LIST, GPUs=$GPU_SPEC"
