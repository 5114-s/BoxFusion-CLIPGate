...
@pytest.mark.parametrize("stage", ("P0", "P1", "P2", "P2V2"))
def test_p_stages_are_deep_copied_children_of_quality_only_b6(stage):
    ...
    assert residual["enabled"] is (stage in {"P1", "P2", "P2V2"})
    ...
    assert occupancy["enabled"] is (stage in {"P2", "P2V2"})
    ...
    assert local_geometry["enabled"] is (stage == "P2V2")
    assert local_geometry["observer_only"] is True
    assert local_geometry["mutate"] is False
    assert local_geometry["collect_diagnostics"] is (stage == "P2V2")
