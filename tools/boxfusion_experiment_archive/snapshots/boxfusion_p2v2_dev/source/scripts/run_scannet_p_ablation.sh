...
# Frozen-B6 P0/P1/P2/P2V2 protocol.
#
# Fixed ten scenes:
...
case "${STAGE^^}" in
    ...
    *) echo "Stage must be P0, P1, P2, or P2V2" >&2; exit 2 ;;
esac
...
if [[ "$STAGE" != "P0" && ! -f "$P1_CHECKPOINT" ]]; then
    echo "P1/P2/P2V2 require BOXFUSION_P1_RESIDUAL_CHECKPOINT" >&2
    exit 1
fi
if [[ ( "$STAGE" == "P2" || "$STAGE" == "P2V2" ) \
      && ! -f "$P2_CHECKPOINT" ]]; then
    echo "P2/P2V2 require BOXFUSION_P2_OCCUPANCY_CHECKPOINT" >&2
    exit 1
fi
if [[ "$STAGE" == "P0" ]]; then
    P1_CHECKPOINT=""
...
for output in outputs:
    try:
        ...
        raise SystemExit("P output roots must remain inside boxfusion_p2v2_dev")
PY
...
if [[ "$STAGE" == "P1" || "$STAGE" == "P2" || "$STAGE" == "P2V2" ]]; then
