...
PY
        ...
        if (( paired_artifact_exists )); then
            ...
            if [[ "$recorded_schema" != "boxfusion.tr3d_terminal_completion.v2" \
                  ...
                  || "$recorded_boxer_sha" != "$actual_boxer_sha" ]]; then
        ...
        local prediction_sha baseline_sha r3_sha boxer_sha
        ...
        local summary
        summary="$(
            grep -E 'Online refinement summary|Boxer lifting summary|TR3D terminal active summary' "$scene_log" \
                | tail -n 3 \
                | tr '\n' ' ' \
                || true
        )"
...
if [[ "$expected_scene_count" -ne "$total" \
      ...
      || "$r3_diagnostic_count" -ne "$total" ]]; then
    echo "Paired artifact count mismatch: expected=$total active=$active_prediction_count baseline=$baseline_prediction_count active_markers=$active_marker_count baseline_markers=$baseline_marker_count r3_diagnostics=$r3_diagnostic_count" >&2
    exit 1
fi
if [[ -n "$BOXER_DIAGNOSTICS_ROOT" ]]; then
