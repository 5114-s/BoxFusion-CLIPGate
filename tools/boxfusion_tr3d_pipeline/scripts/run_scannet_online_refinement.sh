...
C3_PARENT_CACHE_ROOT="${BOXFUSION_C3_ONLINE_PARENT_CACHE_ROOT:-}"
C3_DIAGNOSTICS_ROOT="${BOXFUSION_C3_ONLINE_DIAGNOSTICS_ROOT:-}"
C3_CANDIDATE_SOURCE="${BOXFUSION_C3_ONLINE_CANDIDATE_SOURCE:-parent_score}"
...
if [[ -n "$C3_PARENT_CACHE_ROOT" || -n "$C3_DIAGNOSTICS_ROOT" ]]; then
    if [[ -z "$C3_PARENT_CACHE_ROOT" || -z "$C3_DIAGNOSTICS_ROOT" ]]; then
        echo "Both BOXFUSION_C3_ONLINE_PARENT_CACHE_ROOT and BOXFUSION_C3_ONLINE_DIAGNOSTICS_ROOT are required" >&2
        exit 1
    fi
    if [[ ! -d "$C3_PARENT_CACHE_ROOT" || -L "$C3_PARENT_CACHE_ROOT" ]]; then
        echo "Missing C3 parent cache root: $C3_PARENT_CACHE_ROOT" >&2
        exit 1
    fi
    if [[ "$C3_CANDIDATE_SOURCE" != "parent_score" ]]; then
        echo "Direct train collection requires parent_score candidate source" >&2
        exit 1
    fi
fi
...
run_worker() {
    ...
    while IFS= read -r scene || [[ -n "$scene" ]]; do
        ...
        if [[ -n "$C3_DIAGNOSTICS_ROOT" ]]; then
            c3_diagnostic="$C3_DIAGNOSTICS_ROOT/${scene}_c3_online_identity.json"
        fi
        if [[ "$CACHE_MODE" == "replay" ]]; then
...
PY
        ...
        if [[ -s "$prediction" ]]; then
            ...
            if [[ -n "$c3_diagnostic" && ! -s "$c3_diagnostic" ]]; then
                echo "Prediction exists but C3 diagnostic is missing: $c3_diagnostic" >&2
                return 1
            fi
        ...
        if [[ -n "$c3_diagnostic" && -e "$c3_diagnostic" ]]; then
            echo "Refusing orphan C3 diagnostic: $c3_diagnostic" >&2
            return 1
        fi
        ...
        if [[ -n "$C3_PARENT_CACHE_ROOT" ]]; then
            optional_args+=(
                --tr3d-c3-online-parent-cache-root "$C3_PARENT_CACHE_ROOT"
                --tr3d-c3-online-diagnostics-root "$C3_DIAGNOSTICS_ROOT"
                --tr3d-c3-online-candidate-source "$C3_CANDIDATE_SOURCE"
            )
        fi
        ...
        if [[ -n "$c3_diagnostic" && ! -s "$c3_diagnostic" ]]; then
            echo "[$(date '+%F %T')] ERROR: C3 diagnostic missing: $c3_diagnostic" >&2
            return 1
        fi
        ...
        local summary
        summary="$(
            grep -E 'Online refinement summary|Boxer lifting summary|C3 online identity summary' "$scene_log" \
                | tail -n 3 \
                | tr '\n' ' ' \
                || true
        )"
...
if [[ "$SKIP_EVALUATION" == "1" ]]; then
    echo "[$(date '+%F %T')] Completed train-only diagnostic collection; evaluation skipped"
    exit 0
fi
