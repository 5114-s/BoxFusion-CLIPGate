...
C1_RUN="${BOXFUSION_C2_C1_RUN:-$ROOT/artifacts/tr3d_c1_track/c1_r3active_full100_v1}"
...
PARENT_ROOT="${BOXFUSION_C2_PARENT_CACHE_ROOT:-$R2_ROOT/cache/tr3d_prefix_boxfusion_causal_p100_full100_v3}"
TEACHER_ROOT="${BOXFUSION_C2_TEACHER_ROOT:-/data/ZhaoX/OVM3D-Dett/boxfusion_maskgraph_dev/cache/sam3_teacher/sam3_teacher_full100_c050_frozen_v1}"
SCENE_LIST="${BOXFUSION_C2_SCENE_LIST:-$ROOT/evaluation/data_util/meta_data/scannetv2_val.txt}"
EXCLUDE_SCENE_LIST="${BOXFUSION_C2_EXCLUDE_SCENE_LIST:-$ROOT/evaluation/data_util/meta_data/scannetv2_val_ablation10_even.txt}"
...
ACTIVE_ROOT="${BOXFUSION_C2_ACTIVE_ROOT:-/extra/ZhaoX/codex_artifacts/boxfusion_r3_20260805/boxfusion_tr3d_terminal_paired_full100/results/b6_g0_tr3d_terminal/g0_tr3d_terminal_paired_full100_v1/scannetv2_val-4b18fc586f7a}"
...
for path in "$PYTHON_BIN" "$SCENE_LIST" "$EXCLUDE_SCENE_LIST"; do
...
exclude_count="$(awk 'NF && $1 !~ /^#/ {count += 1} END {print count + 0}' "$EXCLUDE_SCENE_LIST")"
[[ "$scene_count" == "100" ]] || { echo "C2 full100 requires exactly 100 scenes" >&2; exit 2; }
[[ "$exclude_count" == "10" ]] || { echo "C2 held-out audit requires exactly 10 excluded development scenes" >&2; exit 2; }
[[ ! -e "$RUN_ROOT" ]] || { echo "Refusing immutable run overwrite: $RUN_ROOT" >&2; exit 2; }
mkdir -p "$CACHE_ROOT" "$REPORT_ROOT"

echo "C2 multi-view SAM3 Mask-RGBD confirmation observer (full100)"
echo "  source: C1 depth-feature ranking, Top-$SOURCE_BUDGET per scene"
echo "  teacher: $TEACHER_ROOT"
echo "  all/held-out scenes: 100/90"
...
env -u PYTHONPATH TMPDIR="${BOXFUSION_C2_TMPDIR:-/dev/shm}" PYTHONDONTWRITEBYTECODE=1 \
  "$PYTHON_BIN" tools/audit_tr3d_c2_maskrgbd_observer.py \
    --scene-list "$SCENE_LIST" --exclude-scene-list "$EXCLUDE_SCENE_LIST" \
...
echo "C2 full100 complete: $REPORT_ROOT/gt_audit.json"
