...
CANDIDATE_SOURCE="${BOXFUSION_C3_ONLINE_CANDIDATE_SOURCE:-c2}"
...
if [[ "$CANDIDATE_SOURCE" == "c2" ]]; then
    required_roots+=("$C2_CACHE_ROOT")
elif [[ "$CANDIDATE_SOURCE" != "parent_score" ]]; then
    echo "Invalid C3 candidate source: $CANDIDATE_SOURCE" >&2
    exit 1
fi
for required_root in "${required_roots[@]}"; do
...
export BOXFUSION_C3_ONLINE_CANDIDATE_SOURCE="$CANDIDATE_SOURCE"
