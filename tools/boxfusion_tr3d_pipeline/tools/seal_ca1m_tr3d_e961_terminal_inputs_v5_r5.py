#!/usr/bin/env python3
"""Create-only R4 invalidation, R5 preregistration, or R5 ready bundle."""
...
from boxfusion.ca1m_tr3d_e961_terminal_inputs_v5_r5 import (  # noqa: E402
    seal_preregistration, seal_r4_invalidation, seal_ready_authorization_bundle,
    sha256_file,
)
...
def parser() -> argparse.ArgumentParser:
    ...
    mode.add_argument("--invalidate-r4", action="store_true")
...
def main() -> int:
    ...
    if args.invalidate_r4:
        if any(dynamic): raise ValueError("R4 invalidation forbids dynamic receipts")
        path = seal_r4_invalidation(); result = {"status": "INVALIDATED_R4", "path": str(path), "sha256": sha256_file(path)}
    elif args.seal_preregistration:
        if any(dynamic): raise ValueError("static preregistration forbids dynamic receipts")
        path = seal_preregistration(); result = {"status": "SEALED_R5_STATIC_PREREGISTRATION", "path": str(path), "sha256": sha256_file(path)}
    else:
        ...
        result = {"status": "SEALED_R5_READY_BUNDLE", "ready": str(ready), "ready_sha256": sha256_file(ready), "authorization": str(auth), "authorization_sha256": sha256_file(auth), "bundle": str(bundle), "bundle_sha256": sha256_file(bundle), "gpu_started": False}
