...
from boxfusion.tr3d_worker_client import PersistentTR3DWorker
...
def run(cfg, model, dataset, clip_model, preprocess, tokenized_text, text_features, augmentor, preprocessor, score_thresh=0.0, viz_on_gt_points=False, gap=25, re_vis=True):
    ...
    if c3_online_observer.enabled and not cfg.get(
        "online_refinement", {}
    ).get("enabled", False):
        raise ValueError(
            "C3 online identity observer requires online_refinement.enabled"
        )
    c3_active_appender = None
    c3_active_output_root = None
    c3_active_diagnostics_root = None
    c3_active_cfg = cfg.get("tr3d_c3_online_active", {})
    if bool(c3_active_cfg.get("enabled", False)):
        if not c3_online_observer.enabled or terminal_r3 is None:
            raise ValueError(
                "C3 online active requires both the identity observer and "
                "terminal R3"
            )
        policy_path = c3_active_cfg.get("policy_checkpoint")
        output_root = c3_active_cfg.get("output_root")
        diagnostics_root = c3_active_cfg.get("diagnostics_root")
        if not all(
            isinstance(value, (str, os.PathLike))
            and os.fspath(value).strip()
            for value in (policy_path, output_root, diagnostics_root)
        ):
            raise ValueError(
                "C3 online active requires policy/output/diagnostics paths"
            )
        c3_active_appender = C3OnlineActiveAppender.from_checkpoint(policy_path)
        c3_active_output_root = Path(output_root).resolve()
        c3_active_diagnostics_root = Path(diagnostics_root).resolve()
        reserved_roots = {
            Path(cfg["data"]["output_dir"]).resolve(),
            terminal_r3_baseline_root,
            Path(c3_online_observer.config.diagnostics_root).resolve(),
        }
        if (
            c3_active_output_root in reserved_roots
            or c3_active_diagnostics_root in reserved_roots
            or c3_active_output_root == c3_active_diagnostics_root
        ):
            raise ValueError("C3 online active roots must be isolated")

    incremental_tr3d = None
    incremental_worker = None
    incremental_diagnostics_root = None
    incremental_cfg = cfg.get("tr3d_incremental_observer", {})
    if bool(incremental_cfg.get("enabled", False)):
        required = (
            "worker_python", "worker_script", "runtime_root", "config",
            "checkpoint", "project_root", "vendor_root", "prefix_manifest",
            "diagnostics_root",
        )
        missing = [name for name in required if not incremental_cfg.get(name)]
        if missing:
            raise ValueError(
                "incremental TR3D observer missing: " + ", ".join(missing)
            )
        incremental_worker = PersistentTR3DWorker(
            python=incremental_cfg["worker_python"],
            worker_script=incremental_cfg["worker_script"],
            runtime_root=incremental_cfg["runtime_root"],
            config=incremental_cfg["config"],
            checkpoint=incremental_cfg["checkpoint"],
            project_root=incremental_cfg["project_root"],
            vendor_root=incremental_cfg["vendor_root"],
            device=incremental_cfg.get("device", "cuda:0"),
        )
        base_incremental_config = IncrementalTR3DConfig(
                voxel_size_m=float(incremental_cfg.get("voxel_size_m", 0.03)),
                pixel_stride=int(incremental_cfg.get("pixel_stride", 4)),
                warmup_keyframes=int(incremental_cfg.get("warmup_keyframes", 3)),
                inference_interval_keyframes=int(
                    incremental_cfg.get("inference_interval_keyframes", 5)
                ),
                max_memory_voxels=int(
                    incremental_cfg.get("max_memory_voxels", 300000)
                ),
                max_snapshot_points=int(
                    incremental_cfg.get("max_snapshot_points", 200000)
                ),
                min_track_hits=int(incremental_cfg.get("min_track_hits", 2)),
            )
        if bool(incremental_cfg.get("lightweight_fusion", False)):
            incremental_tr3d = LightweightAsyncTR3DObserver(
                base_incremental_config,
                incremental_worker,
                LightweightFusionConfig(
                    stage=int(incremental_cfg.get("lightweight_stage", 6)),
                    top_k_views=int(incremental_cfg.get("top_k_views", 5)),
                    diversity_weight=float(
                        incremental_cfg.get("diversity_weight", 0.30)
                    ),
                    min_view_angle_deg=float(
                        incremental_cfg.get("min_view_angle_deg", 12.0)
                    ),
                    depth_pixel_stride=int(
                        incremental_cfg.get("depth_evidence_stride", 6)
                    ),
                    drain_on_finalize=bool(
                        incremental_cfg.get("drain_on_finalize", False)
                    ),
                ),
            )
        else:
            incremental_tr3d = IncrementalTR3DObserver(
                base_incremental_config, incremental_worker
            )
        incremental_prefixes = load_prefix_manifest(
            incremental_cfg["prefix_manifest"]
        )
        incremental_diagnostics_root = Path(
            incremental_cfg["diagnostics_root"]
        ).resolve()
    ...
    def observe_online_keyframe(
        ...
        if incremental_tr3d is not None:
            if incremental_tr3d.scene_id != str(scene_identifier):
                prefix = incremental_prefixes.get(str(scene_identifier))
                if prefix is None:
                    raise ValueError(
                        f"incremental prefix manifest has no {scene_identifier}"
                    )
                incremental_tr3d.reset_scene(
                    str(scene_identifier), prefix.axis_alignment
                )
            incremental_tr3d.process_keyframe(
                scene_id=str(scene_identifier), depth=depth, image=image,
                intrinsics=depth_intrinsics, camera_to_world=pose_matrix,
                source_timestamp=int(source_frame_identifier),
            )
        ...
        if count == len(dataset)-1 or (count+gap)>len(dataset)-1:
            ...
            if cfg['data']['output_dir'] is not None and cfg["eval"]:
                ...
                if c3_online_observer.enabled:
                    c3_online_summary = c3_online_observer.finalize(
                        scene_id=scene_identifier,
                        prediction_corners=boxes_3d,
                        prediction_scores=scores,
                    )
                    print(
                        c3_online_observer.summary_text(c3_online_summary)
                    )
                    if c3_active_appender is not None:
                        assert c3_active_output_root is not None
                        assert c3_active_diagnostics_root is not None
                        c3_active_result = c3_active_appender.apply(
                            scene_id=scene_identifier,
                            identity_summary=c3_online_summary,
                            anchor_corners=boxes_3d,
                            anchor_scores=scores,
                        )
                        active_prediction_path = (
                            c3_active_output_root
                            / f"{scene_identifier}_boxes.pkl"
                        )
                        active_report_path = (
                            c3_active_diagnostics_root
                            / f"{scene_identifier}_c3_online_active.json"
                        )
                        save_prediction_create_only(
                            c3_active_result.corners,
                            c3_active_result.scores,
                            active_prediction_path,
                        )
                        write_active_summary_create_only(
                            active_report_path, c3_active_result.summary
                        )
                        print(
                            "C3 online active summary | "
                            f"applied={c3_active_result.summary['applied_count']}, "
                            f"anchors/output="
                            f"{c3_active_result.summary['anchor_count']}/"
                            f"{c3_active_result.summary['output_count']}, "
                            f"checkpoint="
                            f"{c3_active_result.summary['policy_checkpoint_sha256'][:12]}"
                        )

                if incremental_tr3d is not None:
                    incremental_summary = incremental_tr3d.finalize(
                        anchor_corners_world=boxes_3d,
                        anchor_scores=scores,
                    )
                    assert incremental_diagnostics_root is not None
                    incremental_diagnostics_root.mkdir(
                        parents=True, exist_ok=True
                    )
                    incremental_report = (
                        incremental_diagnostics_root
                        / f"{scene_identifier}_tr3d_incremental.json"
                    )
                    if incremental_report.exists():
                        raise FileExistsError(
                            f"refusing incremental diagnostic overwrite: {incremental_report}"
                        )
                    incremental_report.write_text(
                        json.dumps(
                            incremental_summary, indent=2, sort_keys=True
                        ) + "\n",
                        encoding="utf-8",
                    )
                    print(
                        "TR3D incremental observer summary | "
                        f"keyframes/calls={incremental_summary['keyframes']}/"
                        f"{incremental_summary['provider_calls']}, "
                        f"voxels={incremental_summary['memory_voxels']}, "
                        f"tracks/confirmed={incremental_summary['tracks']}/"
                        f"{incremental_summary['confirmed_tracks']}, "
                        f"amortized_ms/keyframe="
                        f"{incremental_summary['amortized_ms_per_keyframe']:.3f}"
                    )
            ...
            if incremental_worker is not None:
                incremental_worker.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--proposal-cache-mode", choices=["disabled", "replay"], default=None, help="Override the immutable proposal-cache mode")
    ...
    parser.add_argument("--tr3d-c3-online-c2-cache-root", default=None, type=str, help="Enable the C3 online identity observer with this immutable C2 cache root")
    parser.add_argument("--tr3d-c3-online-parent-cache-root", default=None, type=str, help="Immutable terminal TR3D parent-cache root for the C3 online observer")
    ...
    parser.add_argument("--tr3d-c3-online-prefix-id", default="p100", type=str, help="Frozen C3 prefix identity (must remain p100 for the registered run)")
    parser.add_argument("--tr3d-c3-online-candidate-source", choices=["c2", "parent_score"], default="c2", help="C3 candidate ranking source")
    parser.add_argument("--tr3d-c3-online-active-policy", default=None, type=str, help="Train-only authorized C3 source-gate checkpoint")
    parser.add_argument("--tr3d-c3-online-active-output-root", default=None, type=str, help="Create-only C3 active prediction root")
    parser.add_argument("--tr3d-c3-online-active-diagnostics-root", default=None, type=str, help="Create-only C3 active report root")
    parser.add_argument("--tr3d-incremental-observer", action="store_true", help="Enable causal voxel memory and periodic live TR3D observer")
    parser.add_argument("--tr3d-incremental-diagnostics-root", default=None, type=str)
    parser.add_argument("--tr3d-incremental-prefix-manifest", default=None, type=str, help="Manifest used only for ScanNet axis alignment")
    parser.add_argument("--tr3d-worker-python", default=None, type=str)
    parser.add_argument("--tr3d-worker-runtime-root", default=None, type=str)
    parser.add_argument("--tr3d-worker-config", default=None, type=str)
    parser.add_argument("--tr3d-worker-checkpoint", default=None, type=str)
    parser.add_argument("--tr3d-worker-project-root", default=None, type=str)
    parser.add_argument("--tr3d-worker-vendor-root", default=None, type=str)
    parser.add_argument("--tr3d-worker-device", default="cuda:0", type=str)
    parser.add_argument("--tr3d-incremental-every-keyframes", default=5, type=int)
    parser.add_argument("--tr3d-lightweight-fusion", action="store_true", help="Enable isolated OV-SCAN/Zoo3D/async/SMOV/InsFusion observer")
    parser.add_argument("--tr3d-lightweight-stage", default=6, type=int, choices=range(1, 7), metavar="{1..6}")
    parser.add_argument("--tr3d-lightweight-top-k", default=5, type=int)
    parser.add_argument("--tr3d-lightweight-diversity-weight", default=0.30, type=float)
    parser.add_argument("--tr3d-lightweight-min-view-angle-deg", default=12.0, type=float)
    parser.add_argument("--tr3d-lightweight-depth-stride", default=6, type=int)
    parser.add_argument("--tr3d-lightweight-drain-finalize", action="store_true", help="Wait for pending TR3D requests at scene end (slower, higher coverage)")
    ...
    if dataset_path.lower() in ["scannet", "ca1m", 'online']:
        ...
        if args.proposal_cache_mode is not None:
            cfg.setdefault("lifting", {}).setdefault("proposal_cache", {})[
                "mode"
            ] = args.proposal_cache_mode
        ...
        if any(value is not None for value in c3_online_values):
            if not all(value is not None for value in c3_online_values):
                parser.error("C3 parent-cache and diagnostics roots are required together")
            if (
                args.tr3d_c3_online_candidate_source == "c2"
                and args.tr3d_c3_online_c2_cache_root is None
            ):
                parser.error("C3 c2 candidate source requires the C2 cache root")
            if args.tr3d_c3_online_prefix_id != "p100":
                parser.error(
                    "the registered C3 online identity observer requires "
                    "--tr3d-c3-online-prefix-id=p100"
                )
            cfg["tr3d_c3_online_observer"] = {
                "enabled": True,
                "c2_cache_root": args.tr3d_c3_online_c2_cache_root,
                "parent_cache_root": (
                    args.tr3d_c3_online_parent_cache_root
                ),
                "diagnostics_root": (
                ...
                "prefix_id": args.tr3d_c3_online_prefix_id,
                "source_rank_max": 5,
                "gate_name": "mask2_depth",
                "candidate_source": args.tr3d_c3_online_candidate_source,
            }
        c3_active_values = (
            args.tr3d_c3_online_active_policy,
            args.tr3d_c3_online_active_output_root,
            args.tr3d_c3_online_active_diagnostics_root,
        )
        if any(value is not None for value in c3_active_values):
            if not all(value is not None for value in c3_active_values):
                parser.error(
                    "all three --tr3d-c3-online-active-* arguments are "
                    "required together"
                )
            if not all(value is not None for value in c3_online_values):
                parser.error(
                    "C3 online active requires the C3 online identity observer"
                )
            cfg["tr3d_c3_online_active"] = {
                "enabled": True,
                "policy_checkpoint": args.tr3d_c3_online_active_policy,
                "output_root": args.tr3d_c3_online_active_output_root,
                "diagnostics_root": (
                    args.tr3d_c3_online_active_diagnostics_root
                ),
            }
        if args.tr3d_incremental_observer:
            incremental_values = (
                args.tr3d_incremental_diagnostics_root,
                args.tr3d_incremental_prefix_manifest,
                args.tr3d_worker_python,
                args.tr3d_worker_runtime_root,
                args.tr3d_worker_config,
                args.tr3d_worker_checkpoint,
                args.tr3d_worker_project_root,
                args.tr3d_worker_vendor_root,
            )
            if not all(value is not None for value in incremental_values):
                parser.error(
                    "incremental TR3D observer requires diagnostics, manifest, "
                    "worker Python/runtime/config/checkpoint/project/vendor"
                )
            if args.tr3d_incremental_every_keyframes < 1:
                parser.error("--tr3d-incremental-every-keyframes must be positive")
            if args.tr3d_lightweight_top_k < 1:
                parser.error("--tr3d-lightweight-top-k must be positive")
            if not 0.0 <= args.tr3d_lightweight_diversity_weight <= 1.0:
                parser.error(
                    "--tr3d-lightweight-diversity-weight must lie in [0,1]"
                )
            if not 0.0 <= args.tr3d_lightweight_min_view_angle_deg <= 180.0:
                parser.error(
                    "--tr3d-lightweight-min-view-angle-deg must lie in [0,180]"
                )
            if args.tr3d_lightweight_depth_stride < 1:
                parser.error("--tr3d-lightweight-depth-stride must be positive")
            cfg["tr3d_incremental_observer"] = {
                "enabled": True,
                "diagnostics_root": args.tr3d_incremental_diagnostics_root,
                "prefix_manifest": args.tr3d_incremental_prefix_manifest,
                "worker_python": args.tr3d_worker_python,
                "worker_script": str(
                    Path(__file__).resolve().parent
                    / "tools" / "tr3d_online_worker.py"
                ),
                "runtime_root": args.tr3d_worker_runtime_root,
                "config": args.tr3d_worker_config,
                "checkpoint": args.tr3d_worker_checkpoint,
                "project_root": args.tr3d_worker_project_root,
                "vendor_root": args.tr3d_worker_vendor_root,
                "device": args.tr3d_worker_device,
                "inference_interval_keyframes": (
                    args.tr3d_incremental_every_keyframes
                ),
                "voxel_size_m": 0.03,
                "pixel_stride": 4,
                "warmup_keyframes": 3,
                "max_memory_voxels": 300000,
                "max_snapshot_points": 200000,
                "min_track_hits": 2,
                "lightweight_fusion": bool(args.tr3d_lightweight_fusion),
                "lightweight_stage": int(args.tr3d_lightweight_stage),
                "top_k_views": int(args.tr3d_lightweight_top_k),
                "diversity_weight": float(
                    args.tr3d_lightweight_diversity_weight
                ),
                "min_view_angle_deg": float(
                    args.tr3d_lightweight_min_view_angle_deg
                ),
                "depth_evidence_stride": int(
                    args.tr3d_lightweight_depth_stride
                ),
                "drain_on_finalize": bool(
                    args.tr3d_lightweight_drain_finalize
                ),
            }
        elif args.tr3d_lightweight_fusion:
            parser.error(
                "--tr3d-lightweight-fusion requires --tr3d-incremental-observer"
            )
        boxer_gate_values = (
