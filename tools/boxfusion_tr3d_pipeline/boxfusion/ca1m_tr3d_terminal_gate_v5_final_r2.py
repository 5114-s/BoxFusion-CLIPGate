"""Final R2, R6-only execution boundary for the CA-1M terminal gate v5.
...
DEFAULT_PENDING_CONFIG = ROOT / "config/ca1m_tr3d_terminal_gate_v5_final_r2_pending.json"
MANIFEST_ROOT = ROOT / "manifests/ca1m_tr3d_terminal_gate_v5_final_r2"
...
PENDING_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_pending_config.v5.final.r2"
PROTOCOL_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_preregistration_protocol.v5.final.r2"
PREREGISTRATION_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_preregistration.v5.final.r2"
READY_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_ready_config.v5.final.r2"
AUTHORIZATION_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_run_authorization.v5.final.r2"
RUN_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_run.v5.final.r2"
STOP_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_stop.v5.final.r2"
MATERIALIZATION_COLLECTION_SCHEMA = (
    "boxfusion.ca1m_tr3d_geometry_materialization_collection.v5.final.r2"
)
...
NAMESPACE = "ca1m_tr3d_terminal_gate_v5_final_r2"
...
IMPLEMENTATION_PATHS = {
    ...
    "final_gate_boundary": ROOT / "boxfusion/ca1m_tr3d_terminal_gate_v5_final_r2.py",
    "static_preflight": ROOT / "tools/preflight_ca1m_tr3d_terminal_gate_v5_final_r2.py",
    "scientific_sealer": ROOT / "tools/seal_ca1m_tr3d_terminal_gate_v5_final_r2.py",
    "runner": ROOT / "tools/run_ca1m_tr3d_terminal_gate_v5_final_r2.py",
    "regression_tests": ROOT / "tests/test_ca1m_tr3d_terminal_gate_v5_final_r2.py",
}
GENERIC_GATE_SHA256 = (
...
def _write_runtime_bytes_fuse(
    ...
    if capability is not None:
        _guard_run_capability(capability)
    output = _exclusive_bytes_fuse(target, payload, name)
    if capability is not None:
        _guard_run_capability(capability)
    return output
...
def ap_parity_fixture() -> dict[str, Any]:
    ...
    return {
        "schema": "boxfusion.ca1m_tr3d_terminal_gate_ap_parity.v5.final.r2",
...
def validate_pending_config(
    ...
    if cfg.get("invalidated_predecessor") != _legacy_invalidation_pending_record():
        raise ValueError("pending legacy final invalidation binding differs")
    validate_legacy_final_invalidation()
...
def static_preflight(
    ...
    return {
        "schema": "boxfusion.ca1m_tr3d_terminal_gate_static_preflight.v5.final.r2",
...
def build_static_protocol_payload(
    ...
    legacy_invalidation = validate_legacy_final_invalidation()
    ...
    return {
        ...
        "invalidated_predecessor": legacy_invalidation,
        "future_r6": {
...
def seal_ready_authorization(
    *, preregistration_path: Path = PREREGISTRATION_PATH,
    ...
    commit_material = {
        "schema": "boxfusion.ca1m_tr3d_terminal_gate_commit_material.v5.final.r2",
...
def _claim_bytes(context: ExecutionContext, creator_pid: int) -> bytes:
    return canonical_json({
        "schema": "boxfusion.ca1m_tr3d_terminal_gate_run_claim.v5.final.r2",
...
def _claim_runtime(context: ExecutionContext) -> _RunCapability:
    """Consume the fixed one-use claim before any GT open/runtime mkdir."""
    ...
    try:
        ...
        return _RunCapability(token=token, writer_fd=writer_fd, parent_fd=parent_fd)
    except BaseException:
        if writer_fd >= 0:
            os.close(writer_fd)
        os.close(parent_fd)
...
def _guard_run_capability(capability: _RunCapability) -> _RunAuthority:
    ...
    if capability.writer_fd < 0 or capability.parent_fd < 0:
...
def _release_run_capability(capability: _RunCapability) -> None:
    ...
    for descriptor in (capability.writer_fd, capability.parent_fd):
        try:
            os.close(descriptor)
        except OSError:
            pass
