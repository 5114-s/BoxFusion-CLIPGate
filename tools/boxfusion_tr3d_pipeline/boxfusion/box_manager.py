...
class BoxManager:
    ...
    def __init__(self,cfg):
        ...
        # Disabled by default so the native path has no diagnostic overhead.
        # Moon-QIM-lite enables and clears this bounded per-keyframe trace.
        self.record_merge_events = False
    ...
    def record(self, cur_id, fusion_inds, init_id, cam_poses, box_size, keep, box_centers):
        ...
        for idx in fusion_inds:
            if self.record_merge_events:
                self.merge_log.append(
                    {
                        "stage": "spatial",
                        "winner_members": tuple(self.fusion_list[cur_id]),
                        "loser_members": tuple(self.fusion_list[idx]),
                    }
                )
