"""Final R3, R6-only execution boundary for the CA-1M terminal gate v5.
...
DEFAULT_PENDING_CONFIG = ROOT / "config/ca1m_tr3d_terminal_gate_v5_final_r3_pending.json"
MANIFEST_ROOT = ROOT / "manifests/ca1m_tr3d_terminal_gate_v5_final_r3"
...
R2_PROTOCOL_SHA256 = (
    "25a3c25ab93786f8c880c63082609c418cb1a86094e73c4192a03a35154d2a31"
)
R2_PROTOCOL_SCHEMA = (
    "boxfusion.ca1m_tr3d_terminal_gate_preregistration_protocol.v5.final.r2"
)
R2_PROTOCOL_INVALID_PATH = (
    ROOT
    / "manifests/ca1m_tr3d_terminal_gate_v5_final_r2/"
    "PREREGISTRATION_PROTOCOL_INVALID.json"
)
R2_PROTOCOL_INVALID_SHA256 = (
    "616ac9db9d48c14fc05628510a5acabbcb7ce7044ee110ea1878f4bfb35e530e"
)
R2_PROTOCOL_INVALID_SCHEMA = (
    "boxfusion.ca1m_tr3d_terminal_gate_preregistration_protocol_invalid.v5.final.r2"
)
R2_RUNNER_PATH = ROOT / "tools/run_ca1m_tr3d_terminal_gate_v5_final_r2.py"
R2_RUNNER_TOMBSTONE_SHA256 = (
    "e018da39f2ad2d301f78801deacb51ec113ff9bd949194c3a9fd4b2e2abeedba"
)

PENDING_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_pending_config.v5.final.r3"
PROTOCOL_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_preregistration_protocol.v5.final.r3"
PREREGISTRATION_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_preregistration.v5.final.r3"
READY_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_ready_config.v5.final.r3"
AUTHORIZATION_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_run_authorization.v5.final.r3"
RUN_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_run.v5.final.r3"
STOP_SCHEMA = "boxfusion.ca1m_tr3d_terminal_gate_stop.v5.final.r3"
MATERIALIZATION_COLLECTION_SCHEMA = (
    "boxfusion.ca1m_tr3d_geometry_materialization_collection.v5.final.r3"
)
...
NAMESPACE = "ca1m_tr3d_terminal_gate_v5_final_r3"
...
IMPLEMENTATION_PATHS = {
    ...
    "final_gate_boundary": ROOT / "boxfusion/ca1m_tr3d_terminal_gate_v5_final_r3.py",
    "static_preflight": ROOT / "tools/preflight_ca1m_tr3d_terminal_gate_v5_final_r3.py",
    "scientific_sealer": ROOT / "tools/seal_ca1m_tr3d_terminal_gate_v5_final_r3.py",
    "runner": ROOT / "tools/run_ca1m_tr3d_terminal_gate_v5_final_r3.py",
    "regression_tests": ROOT / "tests/test_ca1m_tr3d_terminal_gate_v5_final_r3.py",
}
GENERIC_GATE_SHA256 = (
...
def _predecessor_invalidation_pending_record() -> dict[str, Any]:
    return {
        "state": "invalidated", "path": os.fspath(R2_PROTOCOL_INVALID_PATH),
        "sha256": R2_PROTOCOL_INVALID_SHA256,
        "schema": R2_PROTOCOL_INVALID_SCHEMA, "operational_authority": False,
    }


def validate_r2_protocol_invalidation() -> dict[str, Any]:
    path, value, data, identity = stable_json(
        R2_PROTOCOL_INVALID_PATH, "final-R2 protocol invalidation",
        schema=R2_PROTOCOL_INVALID_SCHEMA,
    )
    protocol = value.get("invalidated_protocol") or {}
    if (
        sha256_bytes(data) != R2_PROTOCOL_INVALID_SHA256
        or set(value) != {
        ...
        or value.get("invalid") is not True
        or value.get("operational_authority") is not False
        or value.get("namespace") != "ca1m_tr3d_terminal_gate_v5_final_r2"
        or value.get("superseded_by_namespace") != NAMESPACE
        or value.get("never_instance_preregistered") is not True
        or value.get("never_ready_authorized") is not True
        or (value.get("reason") or {}).get("code")
        != "RUNTIME_ROOT_TOCTOU_NOT_ATOMICALLY_CLAIMED"
        or any((value.get("access_before_invalidation") or {}).values())
        or protocol != {
            "path": os.fspath(R2_PROTOCOL_PATH), "sha256": R2_PROTOCOL_SHA256,
            "schema": R2_PROTOCOL_SCHEMA,
        }
        or value.get("runner_before_tombstone") != {
            "path": os.fspath(R2_RUNNER_PATH),
            "sha256": "872cce086472159d5fbca3887e9b1a132f19e57c39be9741f3026d386a67d412",
        }
    ):
        raise PermissionError("final-R2 protocol invalidation differs")
    validate_artifact_record(
        protocol, "invalidated final-R2 static protocol",
        schema=R2_PROTOCOL_SCHEMA, canonical_path=R2_PROTOCOL_PATH,
    )
    _, runner_data, _ = stable_bytes(
        R2_RUNNER_PATH, "final-R2 runner tombstone", immutable=False,
    )
    if sha256_bytes(runner_data) != R2_RUNNER_TOMBSTONE_SHA256:
        raise PermissionError("final-R2 runner tombstone SHA differs")
    return {
        "path": os.fspath(path), "sha256": sha256_bytes(data),
        "schema": R2_PROTOCOL_INVALID_SCHEMA, "identity": identity,
        "operational_authority": False,
    }
...
def ap_parity_fixture() -> dict[str, Any]:
    ...
    return {
        "schema": "boxfusion.ca1m_tr3d_terminal_gate_ap_parity.v5.final.r3",
...
def validate_pending_config(
    ...
    if cfg.get("invalidated_predecessor") != _predecessor_invalidation_pending_record():
        raise ValueError("pending final-R2 invalidation binding differs")
    validate_r2_protocol_invalidation()
...
def static_preflight(
    ...
    return {
        "schema": "boxfusion.ca1m_tr3d_terminal_gate_static_preflight.v5.final.r3",
...
def build_static_protocol_payload(
    ...
    predecessor_invalidation = validate_r2_protocol_invalidation()
...
def seal_ready_authorization(
    *, preregistration_path: Path = PREREGISTRATION_PATH,
    ...
    commit_material = {
        "schema": "boxfusion.ca1m_tr3d_terminal_gate_commit_material.v5.final.r3",
...
def _host_target_probe(parent: Path) -> None:
    """Verify create/fsync/hardlink/unlink semantics without trusting modes."""

    parent = _canonical_absolute(_absolute_lexical(parent), "output host parent")
    parent_fd, parent_chain = _open_dir_chain(parent, "output host parent")
    ...
    try:
        ...
        os.close(parent_fd)
    _verify_dir_chain(parent, parent_chain, "output host parent")


def _claim_bytes(context: ExecutionContext, creator_pid: int) -> bytes:
    return canonical_json({
        "schema": "boxfusion.ca1m_tr3d_terminal_gate_run_claim.v5.final.r3",
...
def _assert_lock_still_owned(writer_fd: int) -> None:
    probe = os.open(
        RUN_CLAIM_PATH, os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0),
    )
    blocked = False
    try:
        try:
...
def _claim_runtime(context: ExecutionContext) -> _RunCapability:
    """Consume claim, then atomically own/bind runtime root before GT."""

    if (
        _absolute_lexical(RUNTIME_ROOT) != _absolute_lexical(OUTPUT_PARENT_PATH / NAMESPACE)
    ...
    _host_target_probe(OUTPUT_PARENT_PATH)
    creator_pid = os.getpid()
    payload = _claim_bytes(context, creator_pid)
    try:
        _exclusive_bytes_fuse(RUN_CLAIM_PATH, payload, "final-gate run claim")
    ...
    try:
        ...
        authority = _RunAuthority(
            ...
            claim_identity=_identity(claim_info),
...
def _guard_run_capability(capability: _RunCapability) -> _RunAuthority:
    ...
    _, claim_data, claim_identity = stable_bytes(
        RUN_CLAIM_PATH, "final-gate fixed run claim", immutable=False,
    )
    ...
    _assert_lock_still_owned(capability.writer_fd)
    if (
...
def load_execution_context(
    ...
    *, require_outputs_absent: bool = True,
    ...
    return ExecutionContext(
        ...
        prereg_path, prereg, ready_path, ready, r6, inventory, outputs,
    )
...
def revalidate_execution_inputs(context: ExecutionContext) -> None:
    ...
    _, prereg, r6, inventory = validate_scientific_preregistration(
        context.preregistration_path
    )
    if (
        prereg != context.preregistration
        or r6.wrapper_sha256 != context.r6.wrapper_sha256
        or r6.collection_sha256 != context.r6.collection_sha256
        or r6.authorization_commit_id != context.r6.authorization_commit_id
...
def run_final_gate(*, context: ExecutionContext) -> Path:
    """Execute one formal attempt using only the canonical internal GT loader."""

    capability = _claim_runtime(context)
    try:
        _stage_guard(context, capability)
