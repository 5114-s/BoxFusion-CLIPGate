...
PARENT_SCORE_ROUTE = "parent_score_rank<=5 AND online_yoloe_mask2_depth"
...
class C3OnlineIdentityConfig:
    ...
    def from_mapping(cls, value: Mapping[str, Any] | None) -> "C3OnlineIdentityConfig":
        ...
        config = cls(
            ...
            c2_cache_root=os.fspath(raw.get("c2_cache_root") or ""),
            parent_cache_root=os.fspath(raw.get("parent_cache_root") or ""),
            diagnostics_root=os.fspath(raw.get("diagnostics_root") or ""),
            ...
            candidate_source=str(raw.get("candidate_source", "c2")),
        )
        ...
        if config.candidate_source not in {"c2", "parent_score"}:
            raise ValueError("C3 candidate_source must be c2 or parent_score")
        required_roots = ["parent_cache_root"]
        if config.candidate_source == "c2":
            required_roots.append("c2_cache_root")
...
class C3OnlineIdentityObserver:
    ...
    def _load_scene(self, scene_id: str) -> _SceneState:
        ...
        if self.config.candidate_source == "c2":
            ...
            parent_rows = np.asarray(c2.parent_rows[source], dtype=np.int64)
            proposal_ids = _readonly(c2.proposal_ids[source], np.int64)
            source_ranks = _readonly(c2.source_ranks[source], np.int32)
            track_scores = _readonly(c2.c1_track_scores[source], np.float32)
            ...
            order = np.argsort(-np.asarray(parent.scores_3d), kind="stable")
            parent_rows = np.asarray(order[: self.config.source_rank_max], dtype=np.int64)
            proposal_ids = _readonly(parent.proposal_ids[parent_rows], np.int64)
            source_ranks = _readonly(
                np.arange(1, len(parent_rows) + 1, dtype=np.int32), np.int32
            )
            track_scores = _readonly(parent.scores_3d[parent_rows], np.float32)
            frozen_selected = _readonly(np.zeros(len(parent_rows)), np.bool_)
            c2_config = C2MaskRGBDConfig(source_budget=self.config.source_rank_max)
            c2_path = parent_path
            c2_sha = parent_sha
        boxes = _readonly(parent.boxes_world[parent_rows], np.float32)
    ...
    def finalize(
        ...
        *,
        ...
        summary: dict[str, Any] = {
            ...
            "scene_id": scene_id,
            "route": (
                ROUTE if self.config.candidate_source == "c2"
                else PARENT_SCORE_ROUTE
            ),
            "comparison_scope": (
            ...
            "frozen_c1_depth_dino_ranking_reused": (
                self.config.candidate_source == "c2"
            ),
