"""R6 canonical-authority boundary for CA-only E961 terminal inputs.

R6 supersedes the never-authorized R5 preregistration.  Every stage rebuilds
...
from . import ca1m_tr3d_e961_terminal_inputs_v5_r5 as r5


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_CONFIG = ROOT / "config/ca1m_tr3d_e961_terminal_inputs_v5_r6_pending.json"
MANIFEST_ROOT = ROOT / "manifests/ca1m_tr3d_e961_terminal_inputs_v5_r6"
...
R5_INVALID_PATH = (
    ROOT / "manifests/ca1m_tr3d_e961_terminal_inputs_v5_r5/PREREGISTRATION_INVALID.json"
)

CONFIG_SCHEMA = "boxfusion.ca1m_tr3d_e961_terminal_inputs_config.v5.r6"
PREREGISTRATION_SCHEMA = "boxfusion.ca1m_tr3d_e961_terminal_inputs_preregistration.v5.r6"
R5_INVALID_SCHEMA = "boxfusion.ca1m_tr3d_e961_terminal_inputs_preregistration_invalid.v5.r5"
AUTH_SCHEMA = "boxfusion.ca1m_tr3d_e961_terminal_inputs_run_authorization.v5.r6"
BUNDLE_SCHEMA = "boxfusion.ca1m_tr3d_e961_terminal_inputs_authorization_bundle.v5.r6"
EXACT80_RECEIPT_SCHEMA = "boxfusion.ca1m_tr3d_e961_terminal_inputs_exact80_receipt.v5.r6"
STATIC_REPORT_SCHEMA = "boxfusion.ca1m_tr3d_e961_terminal_inputs_static_preflight.v5.r6"
OPERATIONAL_REPORT_SCHEMA = "boxfusion.ca1m_tr3d_e961_terminal_inputs_operational_preflight.v5.r6"
NAMESPACE = "ca1m_tr3d_e961_terminal_inputs_xfit_r2_v5_r6"
...
def _load_pending() -> tuple[Path, dict[str, Any]]:
    path, cfg = _json(DEFAULT_CONFIG, "R6 frozen pending config")
    if path != DEFAULT_CONFIG or cfg.get("schema") != CONFIG_SCHEMA or cfg.get("namespace") != NAMESPACE:
        raise ValueError("R6 pending config identity differs")
    return path, cfg
...
def _load_base(cfg: Mapping[str, Any]) -> tuple[Path, dict[str, Any]]:
    ...
    if path != r3.DEFAULT_CONFIG:
        raise ValueError("R6 base static contract is noncanonical")
    # The R3 JSON freezes all semantic paths, but downstream pipeline modules
    # can legitimately advance before R6 is sealed.  Validate those exact
    # paths against a live SHA inventory which R6 preregisters independently.
...
def _validate_shape(cfg: Mapping[str, Any], *, require_invalidation: bool = True) -> dict[str, Any]:
    if cfg.get("schema") != CONFIG_SCHEMA or cfg.get("namespace") != NAMESPACE:
        raise ValueError("R6 schema/namespace differs")
    if cfg.get("preregistration") != {"path": os.fspath(PREREGISTRATION_PATH), "schema": PREREGISTRATION_SCHEMA}:
        raise ValueError("R6 preregistration target differs")
    if cfg.get("sealed_dynamic_outputs") != {
        ...
        raise ValueError("R6 dynamic targets differ")
    _, base = _load_base(cfg)
    predecessor = cfg.get("invalidated_predecessor") or {}
    predecessor_path, _ = _file_record(
        predecessor.get("preregistration"), "R5 preregistration", r5.PREREGISTRATION_SCHEMA,
    )
    if predecessor_path != r5.PREREGISTRATION_PATH:
        raise ValueError("R5 predecessor preregistration is noncanonical")
    invalid = predecessor.get("invalidation")
    if require_invalidation:
        invalid_path, invalid_value = _file_record(invalid, "R5 invalidation", R5_INVALID_SCHEMA)
        if invalid_path != R5_INVALID_PATH or invalid_value.get("invalid") is not True:
            raise ValueError("R5 invalidation differs")
    elif invalid != _pending(R5_INVALID_SCHEMA):
        raise ValueError("R5 invalidation must be pending before create-only invalidation")
    receipts = cfg.get("producer_success_receipts") or {}
    if tuple(receipts) != ROLE_ORDER:
        raise ValueError("R6 producer role order differs")
    if cfg.get("outputs") != _expected_outputs():
        raise ValueError("R6 output paths differ")
    implementation = cfg.get("implementation") or {}
    required = {
        "current_core", "current_sealer", "current_preflight", "current_runner",
        "current_tests", "r5_static_helper", "r4_static_helper", "r3_static_helper",
        "r2_execution_core",
    }
    if set(implementation) != required:
        raise ValueError("R6 implementation inventory differs")
    for key, record in implementation.items():
        _file_record(record, f"R6 implementation {key}")
    if cfg.get("access") != base.get("access"):
        raise ValueError("R6 access contract differs")
    return base
...
def _ready_delta(pending: Mapping[str, Any], ready: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if expected != ready:
        raise PermissionError("R6 ready differs outside the exact six dynamic fields")
    return result


def r5_invalidation_payload() -> dict[str, Any]:
    predecessor = r5.PREREGISTRATION_PATH
    expected_sha = "d8f24da8efb5d79837663355d02ef89979e5b8f32518cfb93343484f575485ad"
    if sha256_file(predecessor) != expected_sha:
        raise ValueError("R5 predecessor changed")
    for forbidden in (
        r5.READY_CONFIG_PATH, r5.RUN_AUTHORIZATION_PATH,
        r5.AUTHORIZATION_BUNDLE_PATH, Path("/extra/ZhaoX") / r5.NAMESPACE,
    ):
        if forbidden.exists() or forbidden.is_symlink():
            raise PermissionError("cannot invalidate R5 after operational output")
    return {
        "schema": R5_INVALID_SCHEMA, "complete": True, "create_only": True,
        "invalid": True, "operational_authority": False,
        "predecessor": {"path": os.fspath(predecessor), "sha256": expected_sha, "schema": r5.PREREGISTRATION_SCHEMA},
        "audit_result": "CODE_BLOCK",
        "reasons": [
            "python38_and_some_python310_builds_lack_fcntl_ofd_lock_constants",
            "ofd_constant_import_failure_prevented_critical_context_and_fd_negative_tests_from_executing",
            "creator_pid_was_not_bound_to_the_private_runtime_authority_registry",
        ],
...
def seal_r5_invalidation() -> Path:
    payload = r5_invalidation_payload()
    r3._host_target_probe(R5_INVALID_PATH.parent)
    fd = r3._claim_writer(R5_INVALID_PATH.parent, ".R5_INVALID.writer.claim", sha256_bytes(canonical_json(payload)))
    try:
        return r3._exclusive_bytes(R5_INVALID_PATH, canonical_json(payload))
    finally:
        os.close(fd)
...
def build_preregistration_payload() -> dict[str, Any]:
    ...
    dependency_changes = [{
        ...
        "r6_preregistered_sha256": base["implementation"][name]["sha256"],
    ...
    if [item["name"] for item in dependency_changes] != ["v5_manifest_runtime"]:
        raise ValueError("R3-to-R6 dependency drift inventory differs")
    for role in ROLE_ORDER:
    ...
    if cfg.get("continuation_receipt") != _pending(CONTINUATION_SCHEMA):
        raise ValueError("R6 continuation is not pending")
    if cfg.get("run_authorization") != _pending(BUNDLE_SCHEMA, bundle=True):
        raise ValueError("R6 bundle authorization is not pending")
    inventory = r3.processed_point_inventory(base)
    invalid_path, _ = _file_record(
        cfg["invalidated_predecessor"]["invalidation"], "R5 invalidation", R5_INVALID_SCHEMA,
    )
    return {
        ...
        "inherited_base_dependency_relationship": {
            "r5_predecessor_preregistration_invalid": True,
            "frozen_r3_base_config_sha256": sha256_file(r3.DEFAULT_CONFIG),
            "r6_rehashes_every_canonical_dependency": True,
            "changed_dependencies": dependency_changes,
        },
        "r5_invalidation": {"path": os.fspath(invalid_path), "sha256": sha256_file(invalid_path), "schema": R5_INVALID_SCHEMA},
        ...
        "runtime_invariants": {
            ...
            "creator_pid_matches_private_authority_registry": True,
...
def validate_preregistration() -> tuple[Path, dict[str, Any]]:
    path, value = _json(PREREGISTRATION_PATH, "R6 preregistration")
    expected = build_preregistration_payload()
    if value != expected:
        raise ValueError("R6 preregistration/static inputs drifted")
    return path, value
...
def validate_static_config(path: Path = DEFAULT_CONFIG) -> dict[str, Any]:
    if Path(path) != DEFAULT_CONFIG:
        raise ValueError("R6 static config path is noncanonical")
    source, cfg = _load_pending()
    _validate_shape(cfg)
...
def _commit_id(ready: Mapping[str, Any], continuation: Path, roles: Mapping[str, r3.VerifiedRole]) -> str:
    value = {
        "schema": "boxfusion.ca1m_tr3d_e961_terminal_inputs_commit_material.v5.r6",
...
def seal_ready_authorization_bundle(
    *, outer_receipt: Path, inner_holdout2_receipt: Path,
    ...
    try:
        r3.create_or_verify(READY_CONFIG_PATH, ready_bytes, "R6 ready leaf")
        r3.create_or_verify(RUN_AUTHORIZATION_PATH, auth_bytes, "R6 authorization leaf")
        ...
        r3.create_or_verify(AUTHORIZATION_BUNDLE_PATH, bundle_bytes, "R6 authorization bundle")
    finally:
        os.close(claim_fd)
...
def _load_committed_ready(path: Path) -> tuple[dict[str, Any], bytes, dict[str, Any], bytes, dict[str, Any], bytes]:
    if path != READY_CONFIG_PATH:
        raise PermissionError("R6 operational ready path is noncanonical")
    ready_bytes = stable_bytes(path, "R6 ready config")
    ready = json.loads(ready_bytes)
    try:
        auth_bytes = stable_bytes(RUN_AUTHORIZATION_PATH, "R6 authorization")
        bundle_bytes = stable_bytes(AUTHORIZATION_BUNDLE_PATH, "R6 authorization bundle")
    except FileNotFoundError as error:
        raise PendingOperationalInputs("R6 ready/auth leaves are not bundle-committed") from error
    ...
    if (
        ...
        raise PermissionError("R6 authorization bundle/leaf binding differs")
...
def _fresh_r2_module(authority: DerivedAuthority) -> Any:
    ...
    name = f"boxfusion._r6_frozen_r2_{authority.r2_module_sha256[:12]}_{secrets.token_hex(12)}"
...
def _assert_lock_still_owned(writer_fd: int) -> None:
    # Check with an independent description first, before any operation could
    # reacquire an accidentally released lock on writer_fd.
    ...
    if not probe_blocked:
        raise PermissionError("R6 writer descriptor no longer holds the exclusive lock")
    try:
        # Re-locking succeeds only for the same open-file-description (or a
        # dup of it).  An independently opened FD for the same inode blocks.
        fcntl.flock(writer_fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
    except OSError as error:
        if error.errno in (errno.EACCES, errno.EAGAIN):
            raise PermissionError(
                "R6 writer FD is not the registered lock-owning open description"
            ) from error
        raise
...
def _runtime_claim_bytes(bundle_sha256: str) -> bytes:
    return canonical_json({
        "schema": "boxfusion.ca1m_tr3d_e961_single_writer_claim.v5.r6",
...
def _claim_runtime_writer(bundle_sha256: str) -> int:
    if WRITER_CLAIM_PATH.parent != OUTPUT_PARENT_PATH or WRITER_CLAIM_PATH.name != f".{NAMESPACE}.writer.claim":
        raise PermissionError("R6 runtime writer claim path differs from the fixed namespace path")
    payload = _runtime_claim_bytes(bundle_sha256)
    try:
        r3._exclusive_bytes(WRITER_CLAIM_PATH, payload)
    except FileExistsError:
        if stable_bytes(WRITER_CLAIM_PATH, "R6 runtime writer claim") != payload:
            raise PermissionError("R6 runtime writer claim belongs to another authorization")
    parent_fd, _ = r3._open_dir_chain(OUTPUT_PARENT_PATH, "R6 runtime writer parent")
    try:
        writer_fd = os.open(
    ...
    try:
        ...
        os.close(writer_fd)
        raise PermissionError("another R6 writer currently owns the namespace")
    return writer_fd


def _authority_record(ctx: ReadyContext) -> CanonicalAuthority:
    if type(ctx) is not ReadyContext:
        raise PermissionError("R6 context type differs")
    authority = _RUNTIME_AUTHORITIES.get(ctx.authority_token)
    if authority is None:
        raise PermissionError("R6 context has no registered canonical authority")
    return authority


def _guard_persistent_handles(ctx: ReadyContext, authority: CanonicalAuthority) -> None:
    if os.getpid() != authority.creator_pid:
        raise PermissionError("R6 context is being used outside its creator process")
    if ctx.writer_fd < 0 or ctx.parent_fd < 0:
        raise PermissionError("R6 persistent authority descriptor is closed")
    try:
        writer = os.fstat(ctx.writer_fd)
        parent = os.fstat(ctx.parent_fd)
    except OSError as error:
        raise PermissionError("R6 persistent authority descriptor is invalid") from error
    ...
    if writer_identity != authority.writer_claim_identity or writer.st_nlink != 1:
        raise PermissionError("R6 writer FD differs from registered canonical claim")
    try:
        canonical_claim = _identity(WRITER_CLAIM_PATH)
    except (FileNotFoundError, NotADirectoryError, OSError) as error:
        raise PermissionError("R6 fixed writer claim no longer names the registered inode") from error
    if canonical_claim != authority.writer_claim_identity:
        raise PermissionError("R6 fixed writer claim no longer names the registered inode")
    expected_claim_bytes = _runtime_claim_bytes(authority.bundle_sha256)
    if stable_bytes(WRITER_CLAIM_PATH, "R6 fixed writer claim") != expected_claim_bytes:
        raise PermissionError("R6 fixed writer claim payload is not bound to the canonical bundle")
    ...
    if not stat.S_ISDIR(parent.st_mode) or (parent.st_dev, parent.st_ino) != authority.parent_identity:
        raise PermissionError("R6 held output-parent dirfd differs from registered canonical parent")
    reopened_fd, reopened_chain = r3._open_dir_chain(OUTPUT_PARENT_PATH, "R6 fixed output parent")
    try:
    ...
    if (
        ...
        raise PermissionError("R6 fixed output-parent chain changed")
...
def _derive_canonical_payloads(expected: Any = None) -> DerivedAuthority:
    ...
    preregistration_bytes = stable_bytes(PREREGISTRATION_PATH, "R6 canonical preregistration")
    ...
    if expected is not None and (
        ...
        raise PermissionError("R6 canonical sealed authority bytes changed")
    ...
    if (
        ...
        raise PermissionError("R6 preregistration/ready/authorization binding differs")
    for section in ("implementation", "base_execution_dependencies"):
        records = preregistration.get(section)
        if not isinstance(records, Mapping) or not records:
            raise PermissionError(f"R6 preregistration {section} inventory is missing")
        for name, dependency_record in records.items():
            try:
                _file_record(dependency_record, f"R6 preregistered {section}.{name}")
            except (OSError, ValueError) as error:
                raise PermissionError(f"R6 preregistered {section}.{name} changed") from error
    try:
        invalid_path, invalid_value = _file_record(
            preregistration.get("r5_invalidation"), "R6 preregistered R5 invalidation", R5_INVALID_SCHEMA,
        )
    except (OSError, ValueError) as error:
        raise PermissionError("R6 preregistered R5 invalidation changed") from error
    if invalid_path != R5_INVALID_PATH or invalid_value.get("invalid") is not True:
        raise PermissionError("R6 preregistered R5 invalidation differs")
    ...
    if auth != expected_auth or bundle != expected_bundle:
        raise PermissionError("R6 committed authorization payload differs")
    ...
    if auth.get("canonical_r2_config_sha256") != r2_sha or r2_config.get("outputs") != _expected_outputs():
        raise PermissionError("R6 authorization R2 config/output binding differs")

    r2_record = ready["implementation"]["r2_execution_core"]
    if preregistration["implementation"].get("r2_execution_core") != r2_record:
        raise PermissionError("R6 R2 module record differs between ready and preregistration")
    r2_path, _ = _file_record(r2_record, "R6 canonical R2 execution core")
    if r2_path != R2_CANONICAL_PATH:
        raise PermissionError("R6 R2 execution path is noncanonical")
...
def validate_operational_ready(path: Path = None) -> ReadyContext:
    ...
    if path == DEFAULT_CONFIG:
        raise PendingOperationalInputs("formal R6 receipt bundle is pending")
    if path != READY_CONFIG_PATH:
        raise PermissionError("R6 operational ready path is noncanonical")
    validate_static_config()
    try:
        ...
        raise PendingOperationalInputs("formal R6 receipt bundle is pending") from error

    if Path(derived.ready["outputs"]["namespace_root"]).parent != OUTPUT_PARENT_PATH:
        raise PermissionError("R6 ready output parent differs from fixed canonical parent")
    r3._host_target_probe(OUTPUT_PARENT_PATH)
    parent_fd, parent_chain = r3._open_dir_chain(OUTPUT_PARENT_PATH, "R6 fixed output parent")
    ...
    authority = CanonicalAuthority(
        ...
        os.getpid(), _identity(WRITER_CLAIM_PATH), (parent_stat.st_dev, parent_stat.st_ino),
        tuple(parent_chain),
    )
...
def run_stage_p(ctx: ReadyContext, role: str, *, device: str = "cuda:0", **kwargs: Any) -> dict[str, Any]:
    with _fresh_secure_r2(ctx) as (module, derived):
        ...
        if function.__module__ != module.__name__:
            raise PermissionError("R6 stage-P callable source changed")
...
def run_stage_o(ctx: ReadyContext, role: str, **kwargs: Any) -> dict[str, Any]:
    with _fresh_secure_r2(ctx) as (module, derived):
        ...
        if function.__module__ != module.__name__:
            raise PermissionError("R6 stage-O callable source changed")
...
def run_stage_e(ctx: ReadyContext, role: str, **kwargs: Any) -> dict[str, Any]:
    with _fresh_secure_r2(ctx) as (module, derived):
        ...
        if function.__module__ != module.__name__:
            raise PermissionError("R6 stage-E callable source changed")
        result = function(derived.r2_context, role, **kwargs)
    _guard_context(ctx)
    return result
...
def seal_stage_m(ctx: ReadyContext) -> dict[str, Any]:
    with _fresh_secure_r2(ctx) as (module, derived):
        ...
        if function.__module__ != module.__name__:
            raise PermissionError("R6 stage-M callable source changed")
    ...
    wrapper = {
        ...
        "r6_preregistration": {"path": os.fspath(PREREGISTRATION_PATH), "sha256": derived.preregistration_sha256, "schema": PREREGISTRATION_SCHEMA},
        "r6_ready_config": {"path": os.fspath(READY_CONFIG_PATH), "sha256": derived.ready_sha256, "schema": CONFIG_SCHEMA},
        "r6_run_authorization": {"path": os.fspath(RUN_AUTHORIZATION_PATH), "sha256": derived.authorization_sha256, "schema": AUTH_SCHEMA},
        "r6_authorization_bundle": {"path": os.fspath(AUTHORIZATION_BUNDLE_PATH), "sha256": derived.bundle_sha256, "schema": BUNDLE_SCHEMA},
        "ground_truth_access": False, "fold1_access": False,
        "official_validation_access": False,
    }
    target = Path(derived.ready["outputs"]["manifest_root"]) / "M_EXACT80_R6_RECEIPT.json"
    r3.create_or_verify(target, canonical_json(wrapper), "R6 exact80 receipt")
...
def run_all(ctx: ReadyContext, *, device: str = "cuda:0") -> dict[str, Any]:
    ...
    return {
        "schema": "boxfusion.ca1m_tr3d_e961_terminal_inputs_run.v5.r6",
...
__all__ = [
    ...
    "RUN_AUTHORIZATION_PATH", "AUTHORIZATION_BUNDLE_PATH", "R5_INVALID_PATH",
    ...
    "R2ContextSnapshot", "load_bound_producer_verifiers", "r5_invalidation_payload",
    "seal_r5_invalidation", "build_preregistration_payload", "seal_preregistration",
