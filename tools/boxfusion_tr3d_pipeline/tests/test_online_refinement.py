...
class RecordingProposalObserver:
    def __init__(self):
        self.reset_scenes = []
        self.calls = []

    def reset_scene(self, scene_id):
        self.reset_scenes.append(scene_id)

    def observe_keyframe(self, **values):
...
def test_read_only_proposal_observer_receives_runtime_frame_before_lifting(tmp_path):
    image, depth, intrinsics, pose, proposal = synthetic_inputs()
    observer = RecordingProposalObserver()
    controller = OnlineRefinementController(
    ...
    controller.process_keyframe(
        ...
        cache_frame_id="125",
        global_corners=corners,
        global_scores=np.asarray([0.7], dtype=np.float32),
        stable_ids=np.asarray([1], dtype=np.int64),
    )
    assert len(observer.calls) == 1
    call = observer.calls[0]
    assert call["scene_id"] == "scene0000_00"
    assert call["frame_id"] == 125
    assert call["proposals"] == (proposal,)
    np.testing.assert_array_equal(call["depth"], depth)
    np.testing.assert_array_equal(call["intrinsics"], intrinsics)
    np.testing.assert_array_equal(call["camera_to_world"], pose)
