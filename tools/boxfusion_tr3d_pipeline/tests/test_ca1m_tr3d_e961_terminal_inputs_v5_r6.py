...
import boxfusion.ca1m_tr3d_e961_terminal_inputs_v5_r6 as route


ROOT = Path(__file__).resolve().parents[1]
PREFLIGHT = ROOT / "tools/preflight_ca1m_tr3d_e961_terminal_inputs_v5_r6.py"
RUNNER = ROOT / "tools/run_ca1m_tr3d_e961_terminal_inputs_v5_r6.py"
R5_RUNNER = ROOT / "tools/run_ca1m_tr3d_e961_terminal_inputs_v5_r5.py"
...
def test_invalidated_r5_runner_is_tombstoned_and_bound_core_fails_closed() -> None:
    result = subprocess.run(
        [sys.executable, str(R5_RUNNER), "--run-all"], cwd=ROOT, capture_output=True,
    )
    assert result.returncode == 66 and result.stdout == b""
    assert json.loads(result.stderr)["status"] == "INVALIDATED_R5_CODE_BLOCK"
    with pytest.raises((ValueError, PermissionError), match="implementation current_runner|SHA"):
        route.r5.validate_static_config()
...
def _make_context(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[route.ReadyContext, dict[str, object]]:
    ...
    r5_invalid_path = tmp_path / "R5_INVALID.json"
    ...
    monkeypatch.setattr(route, "R5_INVALID_PATH", r5_invalid_path)
    ...
    route.r3.write_bytes_exclusive(r5_invalid_path, route.canonical_json({
        "schema": route.R5_INVALID_SCHEMA, "invalid": True,
    }))
    preregistration = {
        ...
        "r5_invalidation": {
            "path": str(r5_invalid_path), "sha256": route.sha256_file(r5_invalid_path),
            "schema": route.R5_INVALID_SCHEMA,
        },
    }
    ...
    route._RUNTIME_AUTHORITIES[token] = route.CanonicalAuthority(
        ...
        os.getpid(), route._identity(route.WRITER_CLAIM_PATH),
...
def test_creator_pid_mismatch_rejected_before_stage(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ctx, _ = _make_context(tmp_path, monkeypatch)
    creator_pid = route._RUNTIME_AUTHORITIES[ctx.authority_token].creator_pid
    monkeypatch.setattr(route.os, "getpid", lambda: creator_pid + 1)
    try: _assert_stage_guard_rejects(ctx, monkeypatch, "outside its creator process")
    finally: ctx.close()
...
def test_sys_modules_fake_r2_injection_rejected_before_stage(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    name = f"boxfusion._r6_frozen_r2_{derived.r2_module_sha256[:12]}_feedface"
...
def test_post_call_guard_rederives_roles_before_return(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ctx, state = _make_context(tmp_path, monkeypatch)
    module_name = "boxfusion._r6_fault_injection"
...
def test_crash_replay_fills_exact_leaves_then_publishes_bundle_last(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    route.r3.create_or_verify(ready_path, ready_bytes, "R6 ready leaf")
    with pytest.raises(route.PendingOperationalInputs): route._load_committed_ready(ready_path)
    route.r3.create_or_verify(auth_path, auth_bytes, "R6 authorization leaf")
    with pytest.raises(route.PendingOperationalInputs): route._load_committed_ready(ready_path)
    route.r3.create_or_verify(bundle_path, bundle_bytes, "R6 authorization bundle")
...
def test_static_preregistration_binds_r5_invalid_and_r6_invariants() -> None:
    ...
    assert prereg["r5_invalidation"]["sha256"]==route.sha256_file(route.R5_INVALID_PATH)
    assert prereg["inherited_base_dependency_relationship"]["r5_predecessor_preregistration_invalid"] is True
