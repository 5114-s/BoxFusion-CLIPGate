...
from boxfusion import ca1m_tr3d_terminal_gate_v5_final_r2 as final


ROOT = Path(__file__).resolve().parents[1]
PREFLIGHT = ROOT / "tools/preflight_ca1m_tr3d_terminal_gate_v5_final_r2.py"
...
def _synthetic_r6(
    ...
    # Frozen generic science is reused, but its legacy writer calls a pathlib
    # keyword unsupported by Python 3.8.  The fixture uses the production
    # final-R2 FUSE writer so both interpreter runs exercise real I/O.
    monkeypatch.setattr(gate, "write_bytes_create_only", fixture_write)
...
def _claim_context(
    ...
    outputs = {
        key: runtime / ("materialized" if key == "materialization_root" else f"{key}.bin")
    ...
    r6 = final.R6Exact80Binding(
        ...
        wrapper_identity=final.artifact_record(dummy, "dummy")["identity"],
        collection_path=dummy, collection_sha256=gate.sha256_file(dummy),
        collection_identity=final.artifact_record(dummy, "dummy")["identity"],
        authorization_commit_id="a" * 64, scene_folds={}, upstream_records={},
    )
    ...
    context = final.ExecutionContext(
        ...
        prereg, {}, ready, {}, r6, inventory, outputs,
    )
...
def test_run_claim_single_writer_fd_owner_and_restart_fail_closed(
    ...
    with pytest.raises(final.RunClaimConsumed):
        final._claim_runtime(context)
    assert not final.RUNTIME_ROOT.exists()
...
def test_full_final_chain_trains_on_fit60_then_runs_frozen_fold0(
    ...
    gate.write_bytes_create_only(
        protocol, final.canonical_json(final.build_static_protocol_payload(pending)),
    ...
    def recording_loader(scene: str) -> np.ndarray:
        opened.append(scene)
        return np.load(gt_paths[scene], allow_pickle=False)

    with pytest.raises(TypeError):
        final.run_final_gate(  # type: ignore[call-arg]
            context=context, ground_truth_loader=recording_loader,
        )
    assert not runtime_root.exists()

    original_writer = final._exclusive_bytes_fuse

    def fuse_mode777(path: Path, payload: bytes, name: str) -> Path:
        output = original_writer(path, payload, name)
        output.chmod(0o777)
        return output

    monkeypatch.setattr(final, "_exclusive_bytes_fuse", fuse_mode777)
    receipt = final.run_final_gate_nonproduction(
        context=context, ground_truth_loader=recording_loader,
        nonproduction_root=runtime_root,
    )
