...
def test_prediction_identity_link_is_create_only_and_byte_exact(tmp_path):
    source = tmp_path / "formal" / "scene_boxes.pkl"
    identity = tmp_path / "identity" / "scene_boxes.pkl"
    save_prediction_create_only(
        np.stack((_corners([0, 0, 2], [1, 2, 3]),)),
        np.asarray([0.75], dtype=np.float32),
        source,
    )
    saved = link_prediction_create_only(source, identity)
    assert saved == identity.resolve()
    assert source.read_bytes() == identity.read_bytes()
    assert source.stat().st_ino == identity.stat().st_ino
    with pytest.raises(FileExistsError, match="refusing to overwrite"):
        link_prediction_create_only(source, identity)
