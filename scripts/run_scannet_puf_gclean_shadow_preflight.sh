...
CONFIG="$ROOT/config/scannet_puf_gclean_shadow_replay_score05.yaml"
...
PRED_ROOT="$ROOT/results/scannet_puf_gclean_shadow_replay_score05"
LOG_ROOT="$ROOT/logs/scannet_puf_gclean_shadow_replay_score05"
...
if [[ "$FINGERPRINT" != "457c997631cd71a83b6480a6e45e103e273ac5ed2d1488252790549c2e2b3504" ]]; then
  echo "Unexpected sealed producer fingerprint: $FINGERPRINT" >&2
  exit 1
fi
...
if ! flock -n 9; then
  echo "Another PUF-Gclean-shadow preflight holds $LOG_ROOT/run.lock" >&2
  exit 1
fi

echo "[$(date '+%F %T')] Starting PUF-Gclean-shadow preflight on GPU $GPU"
...
while IFS= read -r scene || [[ -n "$scene" ]]; do
  ...
  smov_json="$DIAGNOSTICS_ROOT/${scene}.smov_shadow.json"
  puf_json="$DIAGNOSTICS_ROOT/${scene}.puf_gclean_shadow.json"
  for expected_fresh in "$prediction" "$observer_json" "$smov_json" "$puf_json"; do
    if [[ -e "$expected_fresh" ]]; then
      echo "Fresh PUF-Gclean-shadow artifact already exists: $expected_fresh" >&2
      exit 1
    fi
  done
  ...
  for required in "$prediction" "$observer_json" "$smov_json" "$puf_json"; do
    if [[ ! -s "$required" ]]; then
      echo "Missing PUF-Gclean-shadow artifact: $required; see $scene_log" >&2
      exit 1
    fi
  done
  if ! grep -q 'PUF-Gclean-shadow summary | trace_valid=True' "$scene_log"; then
    echo "PUF-Gclean-shadow trace is invalid for $scene; see $scene_log" >&2
    exit 1
  fi
  echo "[$(date '+%F %T')] Completed $scene"
done < "$SCENE_LIST"

echo "[$(date '+%F %T')] Completed PUF-Gclean-shadow preflight"
