...
CONFIG="$ROOT/config/scannet_t05_boxer_gsa_topk3_real_score05.yaml"
SCENE_LIST="$ROOT/evaluation/data_util/meta_data/scannetv2_val.txt"
EXPERIMENT=scannet_t05_boxer_gsa_topk3_real_score05
PRED_ROOT="$ROOT/results/$EXPERIMENT"
GSA_DIAGNOSTICS="$ROOT/diagnostics/t05_boxer_gsa/gsa"
BOXER_DIAGNOSTICS="$ROOT/diagnostics/t05_boxer_gsa/boxer_lifting"
...
mkdir -p "$PRED_ROOT" "$GSA_DIAGNOSTICS" "$BOXER_DIAGNOSTICS" "$SCENE_LOG_ROOT" "$MPL_ROOT"
exec 9>"$LOG_ROOT/run.lock"
flock -n 9 || { echo "Another Boxer-GSA full100 driver is active" >&2; exit 1; }
...
actual = {
    ...
    "gsa": cfg["lifting"]["boxer_gsa"]["enabled"],
    "low": cfg["lifting"]["boxer_gsa"]["low_score_min"],
    "window": cfg["lifting"]["boxer_gsa"]["evidence_window"],
    "topk": cfg["box_fusion"]["reliable_views"]["top_k"],
    "appearance": cfg["association"]["appearance_gate"]["enabled"],
}
expected = {
    "score": 0.5, "boxer": "active", "gsa": True, "low": 0.4,
    "window": 5, "topk": 3, "appearance": False,
}
...
validate_scene() {
  ...
  local diag="$GSA_DIAGNOSTICS/${scene}.json"
  local log="$SCENE_LOG_ROOT/${scene}.log"
  [[ -s "$pred" && -s "$diag" ]] || return 1
  if [[ -e "$log" ]]; then
    [[ -s "$log" ]] || return 1
    grep -Fq 'Boxer-GSA summary:' "$log" || return 1
...
assert diag["schema"] == "boxfusion.boxer_gsa.v1"
assert diag["scene_id"] == sys.argv[3]
assert diag["stats"]["keyframes"] > 0
assert diag["source_geometry_preserved"] is True
assert diag["dynamic_group_score"] is True
PY
}
...
run_worker() {
  ...
  for index in "${!SCENES[@]}"; do
    ...
    echo "[$(date '+%F %T')] [GPU $gpu] Completed $scene $(grep -F 'Boxer-GSA summary:' "$log" | tail -n 1)"
  done
}

echo "[$(date '+%F %T')] Boxer-GSA Top-K3 real-score official100 started on GPUs=$GPU_SPEC"
child_pids=()
cleanup() {
  local pid
...
echo "[$(date '+%F %T')] Boxer-GSA official100 complete"
