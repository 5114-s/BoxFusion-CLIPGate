...
CONFIG="$ROOT/config/scannet_gclean_shadow_replay_score05.yaml"
...
PRED_ROOT="$ROOT/results/scannet_gclean_shadow_replay_score05"
LOG_ROOT="$ROOT/logs/scannet_gclean_shadow_replay_score05"
...
if ! flock -n 9; then
  echo "Another Gclean-shadow preflight holds $LOG_ROOT/run.lock" >&2
  exit 1
fi

echo "[$(date '+%F %T')] Starting Gclean-shadow preflight on GPU $GPU"
...
while IFS= read -r scene || [[ -n "$scene" ]]; do
  ...
  gclean_json="$DIAGNOSTICS_ROOT/${scene}.gclean_shadow.json"
  for expected_fresh in "$prediction" "$observer_json" "$smov_json" "$gclean_json"; do
    if [[ -e "$expected_fresh" ]]; then
      echo "Fresh Gclean-shadow artifact already exists: $expected_fresh" >&2
      exit 1
    fi
  done
  ...
  for required in "$prediction" "$observer_json" "$smov_json" "$gclean_json"; do
    if [[ ! -s "$required" ]]; then
      echo "Missing Gclean-shadow artifact: $required; see $scene_log" >&2
      exit 1
    fi
  done
  if ! grep -q 'Gclean-shadow summary | trace_valid=True' "$scene_log"; then
    echo "Gclean-shadow trace is invalid for $scene; see $scene_log" >&2
    exit 1
  fi
  echo "[$(date '+%F %T')] Completed $scene"
done < "$SCENE_LIST"

echo "[$(date '+%F %T')] Completed Gclean-shadow preflight"
