...
SCENE_LIST="$ROOT/evaluation/data_util/meta_data/scannetv2_boxer_past3_s1_holdout10.txt"
...
CONTRACT="$ROOT/docs/BOXER_PAST3_S1_H10_PROPOSAL_CONTRACT.md"
LOG_ROOT="$ROOT/logs/scannet_boxer_past3_s1_h10_v1_score05"
OUTPUT_ROOT="$LOG_ROOT/boxer_raw"
EXPECTED_LIST_SHA=8965d0534ed3028f85d8b0ea7227d348a6faa1387b858ddf42c3183bd9ebdf90
...
if [[ "$(sha256sum "$SCENE_LIST" | awk '{print $1}')" != "$EXPECTED_LIST_SHA" ]]; then
  echo "H10 scene list hash mismatch" >&2
  exit 1
fi
if [[ "$(grep -cvE '^[[:space:]]*(#|$)' "$SCENE_LIST")" -ne 10 ]]; then
  echo "H10 scene list must contain exactly ten scenes" >&2
  exit 1
fi
if grep -qE '^(scene0568_00|scene0606_01|scene0377_02)$' "$SCENE_LIST"; then
  echo "S0 development scene leaked into H10" >&2
  exit 1
fi
...
if [[ -e "$LOG_ROOT" ]]; then
  echo "Fresh H10 shadow log root already exists: $LOG_ROOT" >&2
  exit 1
fi
mkdir -p "$LOG_ROOT/scenes" "$LOG_ROOT/mplconfig" "$OUTPUT_ROOT"
exec 9>"$LOG_ROOT/run.lock"
if ! flock -n 9; then
  echo "Another H10 Boxer shadow job holds $LOG_ROOT/run.lock" >&2
  exit 1
fi
...
while IFS= read -r scene || [[ -n "$scene" ]]; do
  ...
  sha256sum "$CACHE_ROOT/$scene/manifest.json" \
    "$ROOT/logs/scannet_topk_fusion_score05/scenes/$scene.log" \
    >>"$LOG_ROOT/frozen_inputs_sha256.txt"
done <"$SCENE_LIST"
...
echo "[$(date '+%F %T')] Starting frozen OWLv2+Boxer H10 shadow on GPU $GPU"
while IFS= read -r scene || [[ -n "$scene" ]]; do
  [[ -z "$scene" || "$scene" == \#* ]] && continue
...
echo "[$(date '+%F %T')] Completed output-inert frozen Boxer H10 shadow"
