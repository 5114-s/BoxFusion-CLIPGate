...
CONFIG="$ROOT/config/scannet_t05_boxer_causal_hungarian_assoc_score05.yaml"
SCENE_LIST="${BOXFUSION_ASSOC_SCENE_LIST:-$ROOT/evaluation/data_util/meta_data/scannetv2_val.txt}"
EXPERIMENT=scannet_t05_boxer_causal_hungarian_assoc_score05
PRED_ROOT="$ROOT/results/$EXPERIMENT"
DIAGNOSTICS_ROOT="$ROOT/diagnostics/t05_boxer/causal_hungarian_assoc_score05"
LOG_ROOT="$ROOT/logs/causal_hungarian_assoc/full100"
...
for required in \
  ...
  "$CLIP_MODEL" "$CLASS_TXT" "$CLASS_FEATURES" "$BOXER_CHECKPOINT" \
  "$DINO_CHECKPOINT" "$EVAL_RUNNER" \
  "$ROOT/boxfusion/causal_hungarian_association.py"; do
...
assert cfg["association"]["causal_hungarian"]["enabled"] is True
assert cfg["association"]["causal_hungarian"]["high_score_threshold"] == 0.60
assert cfg["box_fusion"]["reliable_views"]["enabled"] is True
assert cfg["box_fusion"]["reliable_views"]["top_k"] == 3
assert "edgetam_maskdepth" not in cfg["box_fusion"]
...
FINGERPRINT="$({
  sha256sum \
    ...
    "$ROOT/boxfusion/causal_hungarian_association.py" \
    ...
    "$MODEL" "$CLIP_MODEL" "$CLASS_FEATURES" \
...
echo "[$(date '+%F %T')] causal Hungarian association official run"
...
run_worker() {
  ...
  while IFS= read -r scene || [[ -n "$scene" ]]; do
    ...
    local summary
    summary="$(rg 'Causal Hungarian association summary|Reliable-view fusion summary|Cost:' "$log" | tail -n 3 | tr '\n' ' ' || true)"
