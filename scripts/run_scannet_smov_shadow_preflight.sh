...
CONFIG="$ROOT/config/scannet_smov_shadow_replay_score05.yaml"
...
PRED_ROOT="$ROOT/results/scannet_smov_shadow_replay_score05"
LOG_ROOT="$ROOT/logs/scannet_smov_shadow_replay_score05"
...
if ! flock -n 9; then
  echo "Another SMOV-shadow preflight holds $LOG_ROOT/run.lock" >&2
  exit 1
fi

echo "[$(date '+%F %T')] Starting SMOV-shadow preflight on GPU $GPU"
...
while IFS= read -r scene || [[ -n "$scene" ]]; do
  ...
  for expected_fresh in "$prediction" "$observer_json" "$smov_json"; do
    if [[ -e "$expected_fresh" ]]; then
      echo "Fresh SMOV-shadow artifact already exists: $expected_fresh" >&2
      exit 1
    fi
  done
  ...
  for required in "$prediction" "$observer_json" "$smov_json"; do
    if [[ ! -s "$required" ]]; then
      echo "Missing SMOV-shadow artifact: $required; see $scene_log" >&2
      exit 1
    fi
  done
  if ! grep -q 'SMOV-shadow summary | trace_valid=True' "$scene_log"; then
    echo "SMOV-shadow trace is invalid for $scene; see $scene_log" >&2
    exit 1
  fi
  echo "[$(date '+%F %T')] Completed $scene"
done < "$SCENE_LIST"

echo "[$(date '+%F %T')] Completed SMOV-shadow preflight"
