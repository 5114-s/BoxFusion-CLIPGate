...
class ProposalCacheConfig:
    ...
    def from_mapping(cls, mapping: Mapping[str, Any]) -> "ProposalCacheConfig":
        ...
        expected_fingerprint = str(
            mapping.get("expected_fingerprint", "")
        ).strip()
...
class ProposalCache:
    ...
    def __init__(self, config: ProposalCacheConfig, device: torch.device):
        ...
        fingerprint_in_config = bool(
            self.mode == "replay" and self.config.expected_fingerprint
        )
        if (
            required_environment
            and not fingerprint_in_config
            and not os.environ.get(required_environment, "").strip()
        ):
            raise ProposalCacheError(
    ...
    def _load_manifest(self, scene_id: str) -> Dict[str, Any]:
        ...
        expected_fingerprint = self.config.expected_fingerprint or os.environ.get(
