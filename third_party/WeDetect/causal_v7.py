...
OUT = os.environ.get("CAUSAL_OUT", "/data/ZhaoX/BoxFusion/results/causal_m1m2_v7")
...
for pf in sorted(glob.glob(f'{NATIVE}/scene*_boxes.pkl')):
    ...
    for line in open(f'{KFDIAG}/{scene}_pvq_kfmap.jsonl'):
        rr = json.loads(line)
        if rr.get('type') != 'kf_map':
            continue
        for i2, cid2 in enumerate(rr['init_ids']):
            cur_map[int(cid2)] = np.asarray(rr['boxes'][i2], float)
        timeline.append((int(rr['keyframe_id']), list(cur_map.values())))
    def map_at(f):
        out = []
        for kf, boxes in timeline:
            if kf <= f:
                out = boxes
            else:
                break
    ...
    if cands:
        ...
        for f in sorted(by_f):
            ...
            for fr, c, s, is_wd in by_f[f]:
                tainted = any(aabb_iou(c, nb) >= 0.25 for nb in mnow)
                best, best_v = None, 0.0
                for r in receipts:
                    if tainted:
                        break   # polluted proposal never joins an existing receipt
                    rc = r['obs'][-1]
