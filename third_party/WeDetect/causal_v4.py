...
OUT = os.environ.get("CAUSAL_OUT", "/data/ZhaoX/BoxFusion/results/causal_m1m2_v4")
...
for pf in sorted(glob.glob(f'{NATIVE}/scene*_boxes.pkl')):
    ...
    # per-record index of first/last appearance; stable set at f = boxes with age >= NAGE
    NAGE = 5
    rec_kfs = [kf for kf, _ in timeline]
    first_idx, last_idx = {}, {}
    for i, (kf, mp) in enumerate(timeline):
        for cid in mp:
            first_idx.setdefault(cid, i)
            last_idx[cid] = i

    def stable_map_at(f):
        import bisect
        ri = bisect.bisect_right(rec_kfs, f) - 1   # latest record index at/before f
        if ri < 0:
            return []
        return [timeline[j][1][cid] for j, cid in enumerate([]) ] if False else \
               [corners for cid, corners in timeline[ri][1].items()
                if (last_idx.get(cid, 0) - first_idx.get(cid, ri)) >= NAGE]
        ...
        for f in sorted(by_f):
            ...
            for fr, c, s, is_wd in by_f[f]:
                ...
                if best is not None:
                    ...
                    receipts.append(dict(obs=[c], frames={t}, last_ord=t, scores=[s], n_wd=is_wd))
        # scene-end finalize (all data is past at scene end -> causal)
        cand_births = []
        for r in receipts:
