...
OUT = os.environ.get("CAUSAL_OUT", "/data/ZhaoX/BoxFusion/results/causal_m1m2_v8")
        ...
        for f in sorted(by_f):
            ...
            for fr, c, s, is_wd in by_f[f]:
                ...
                if best is not None:
                    ...
                    best['last_ord'] = t; best['meta'].append((t, s, is_wd))
                else:
                    receipts.append(dict(obs=[c], frames={t}, last_ord=t, meta=[(t, s, is_wd)]))
        # scene-end finalize (all data is past at scene end -> causal)
        native_final = [np.asarray(r[1], float) for r in rows]
        cand_births = []
        for r in receipts:
            clean = [(c, t, s, wd) for c, (t, s, wd) in zip(r['obs'], r['meta'])
                     if not any(aabb_iou(c, nb) >= DEDUP for nb in native_final)]
            n_wd = sum(wd for _, _, _, wd in clean)
            min_views = 3 if 2*n_wd > len(clean) else 2
            cframes = {t for _, t, _, _ in clean}
            if len(cframes) < min_views:
                continue
            obs = [c for c, _, _, _ in clean]
            best_j, best_s = 0, -1.0
            for j, a in enumerate(obs):
                ssum = sum(aabb_iou(a, b) for k, b in enumerate(obs) if k != j)
                if ssum > best_s:
                    best_s, best_j = ssum, j
            cand_births.append((float(np.mean([s for _, _, s, _ in clean])), obs[best_j]))
