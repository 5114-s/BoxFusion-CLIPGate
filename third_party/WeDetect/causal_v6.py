...
OUT = os.environ.get("CAUSAL_OUT", "/data/ZhaoX/BoxFusion/results/causal_m1m2_v6")
    ...
    if cands:
        ...
        for f in sorted(by_f):
            ...
            for fr, c, s, is_wd in by_f[f]:
                mcomp = [(nb.mean(0) + (nb - nb.mean(0)) * 0.75) for nb in mnow]
                if any(aabb_iou(c, nb2) >= 0.25 for nb2 in mcomp):
                    continue
