...
for pf in sorted(glob.glob(f'{FINAL}/scene*_boxes.pkl')):
    ...
    for i in range(len(rows)):
        ...
        good_views = []   # (2D iou, camera azimuth unit vec)
        cc = corners.mean(0)
        for fr, props in by_frame.items():
            ...
            if k >= 0 and iv[k] >= 0.3:
                cam = pose[:3, 3]
                good_views.append((float(iv[k]), cam - cc))
        # angular spread between best two views
        spread = 0.0
        if len(good_views) >= 2:
            good_views.sort(key=lambda x: -x[0])
            v1, v2 = good_views[0][1], good_views[1][1]
            n1, n2 = np.linalg.norm(v1), np.linalg.norm(v2)
            if n1 > 1e-6 and n2 > 1e-6:
                c = float(np.clip(np.dot(v1, v2)/(n1*n2), -1, 1))
                spread = float(np.degrees(np.arccos(c)))
        buckets[cls[i]].append((sup, area_ratio, ious[i][0], len(good_views), spread))

def summarize(name, arr):
    a = np.array([(s, ar if ar is not None else np.nan, i, n, sp) for s, ar, i, n, sp in arr])
    if not len(a):
        return
    sup = a[:, 0]; ar = a[:, 1]; iou = a[:, 2]; nv = a[:, 3]; sp = a[:, 4]
    ...
    m = np.isfinite(ar)
    if m.any():
        print(f'   2D/proj area ratio @best-view: median={np.median(ar[m]):.2f}   <0.85: {(ar[m]<0.85).mean()*100:.0f}%   >1.15: {(ar[m]>1.15).mean()*100:.0f}%')
    print(f'   views with IoU>=0.3: >=3: {(nv>=3).mean()*100:.0f}%   ==2: {(nv==2).mean()*100:.0f}%   <=1: {(nv<=1).mean()*100:.0f}%')
    print(f'   angular spread of top-2 views: median={np.median(sp):.0f} deg   >=30deg: {(sp>=30).mean()*100:.0f}%   >=60deg: {(sp>=60).mean()*100:.0f}%')
